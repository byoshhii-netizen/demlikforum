...
function authMiddleware(req, res, next) {
  ...
  if (!token || !tokens.has(token)) return res.status(401).json({ error: 'Giriþ gerekli' });
  const userId = tokens.get(token);
  const user = db.prepare('SELECT * FROM users WHERE id=?').get(userId);
  if (!user) return res.status(401).json({ error: 'Kullanýcý bulunamadý' });
...
function adminMiddleware(req, res, next) {
  ...
  if (!setting || token !== setting.value) return res.status(403).json({ error: 'Geçersiz admin token' });
  next();
}
...
app.post('/api/auth/register', (req, res) => {
  ...
  if (ipBanned) return res.status(403).json({ error: 'Bu IP adresi yasaklanmýþ' });

  const existing = db.prepare('SELECT id FROM users WHERE username=? OR email=?').get(username, email);
  if (existing) return res.status(400).json({ error: 'Bu kullanýcý adý veya e-posta zaten kullanýlýyor' });
  ...
  try {
    ...
    res.status(400).json({ error: 'Kayýt baþarýsýz: ' + e.message });
  }
});
...
app.get('/api/forum/:slug', optionalAuth, (req, res) => {
  const forum = db.prepare(`
    ...
    WHERE f.slug=?
  `).get(req.params.slug);
  if (!forum) return res.status(404).json({ error: 'Forum bulunamadý' });
  res.json(forum);
});
...
app.post('/api/forums', authMiddleware, (req, res) => {
  const { title, content, banner_image, allow_comments } = req.body;
  ...
  try {
    const result = db.prepare('INSERT INTO forums (user_id, title, content, banner_image, slug, allow_comments) VALUES (?,?,?,?,?,?)').run(
      req.user.id, title, content, banner_image || '', tempSlug, allow_comments !== false ? 1 : 0
    );
    ...
    console.error('Forum oluþturma hatasý:', e);
...
app.put('/api/forum/:slug', authMiddleware, (req, res) => {
  ...
  const { title, content, banner_image, allow_comments } = req.body;
  db.prepare("UPDATE forums SET title=?, content=?, banner_image=?, allow_comments=?, updated_at=datetime('now','localtime') WHERE id=?").run(
    title || forum.title, content || forum.content, banner_image ?? forum.banner_image, allow_comments !== undefined ? (allow_comments ? 1 : 0) : forum.allow_comments, forum.id
  );
...
app.post('/api/forum/:slug/comments/:id/like', authMiddleware, (req, res) => {
  ...
  if (!comment) return res.status(404).json({ error: 'Yorum bulunamadý' });
...
app.post('/api/forum/:slug/comments', authMiddleware, (req, res) => {
  ...
  if (!content || content.trim().length === 0) return res.status(400).json({ error: 'Yorum boþ olamaz' });
...
app.get('/api/book/:slug', (req, res) => {
  ...
  if (!book) return res.status(404).json({ error: 'Kitap bulunamadý' });
...
app.post('/api/books', authMiddleware, (req, res) => {
  ...
  try {
    ...
    console.error('Kitap oluþturma hatasý:', e);
    res.status(400).json({ error: e.message });
  }
});
...
app.get('/api/book/:slug/page/:pageSlug', (req, res) => {
  ...
  if (!page) return res.status(404).json({ error: 'Sayfa bulunamadý' });
...
app.put('/api/book/:slug/chapter/:id', authMiddleware, (req, res) => {
  ...
  if (!ch) return res.status(404).json({ error: 'Bölüm bulunamadý' });
  const { title, order_num } = req.body;
...
app.get('/api/group/:slug', optionalAuth, (req, res) => {
  ...
  if (!group) return res.status(404).json({ error: 'Grup bulunamadý' });
...
app.post('/api/groups', authMiddleware, (req, res) => {
  ...
  try {
    ...
    console.error('Grup oluþturma hatasý:', e);
    res.status(400).json({ error: e.message });
  }
});
...
app.post('/api/group/:slug/leave', authMiddleware, (req, res) => {
  ...
  const existing = db.prepare('SELECT id FROM group_members WHERE group_id=? AND user_id=?').get(group.id, req.user.id);
  if (!existing) return res.status(400).json({ error: 'Üye deðilsiniz' });
...
app.post('/api/group/join-invite', authMiddleware, (req, res) => {
  ...
  if (!invite) return res.status(404).json({ error: 'Geçersiz davet kodu' });
...
app.get('/api/group/:slug/messages', optionalAuth, (req, res) => {
  ...
  if (group.type === 'private') {
    ...
    if (!m) return res.status(403).json({ error: 'Üye deðilsiniz' });
  }
...
app.post('/api/group/:slug/messages', authMiddleware, (req, res) => {
  ...
  if (!member) return res.status(403).json({ error: 'Üye deðilsiniz' });
  const { content, image_url } = req.body;
  if (!content && !image_url) return res.status(400).json({ error: 'Mesaj boþ olamaz' });
...
app.delete('/api/group/:slug/messages/:id', authMiddleware, (req, res) => {
  ...
  if (!msg) return res.status(404).json({ error: 'Mesaj bulunamadý' });
...
app.post('/api/group/:slug/moderator/:userId', authMiddleware, (req, res) => {
  ...
  if (!member) return res.status(404).json({ error: 'Üye bulunamadý' });
...
app.post('/api/group/:slug/upload', authMiddleware, upload.single('image'), (req, res) => {
  const group = db.prepare('SELECT * FROM groups WHERE slug=?').get(req.params.slug);
  if (!group || !group.allow_photos) return res.status(403).json({ error: 'Fotoðraf yükleme kapalý' });
...
app.put('/api/admin/level/:id', adminMiddleware, (req, res) => {
  ...
  if (!lv) return res.status(404).json({ error: 'Seviye bulunamadý' });
  const { name, icon, color, min_forums, min_books, min_comments, order_num } = req.body;
...
app.get('/profil/:username', (req, res) => {
  ...
  let html = fs.readFileSync(path.join(__dirname, 'public', 'index.html'), 'utf8');
  const desc = escapeHtml((user.bio || `${user.username} kullanýcýsýnýn profili`).substring(0, 160));
...
app.listen(PORT, () => console.log(`Demlik Forum çalýþýyor: http://localhost:${PORT}`));
