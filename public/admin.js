...
async function tryLogin() {
  ...
  try {
    ...
    $('#admin-login-err').textContent = 'Hatalý þifre';
  }
}
...
async function renderDashboard(main) {
  ...
  // Her isteði ayrý ayrý çek, biri patlarsa diðerleri etkilenmesin
  ...
  main.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px">
      <div class="card">
        ...
        <div class="card-body" style="display:flex;flex-direction:column;gap:12px">
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:var(--bg4);border-radius:8px">
            <span style="font-size:13px">Banlý Üye</span><span class="badge badge-red">${banned}</span>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:var(--bg4);border-radius:8px">
            <span style="font-size:13px">Admin Sayýsý</span><span class="badge badge-blue">${admins}</span>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:var(--bg4);border-radius:8px">
      ...
      <div class="card">
        <div class="card-header"><span><i class="fas fa-history" style="color:#f97316;margin-right:8px"></i>Son Ýþlemler</span></div>
...
async function renderUsers(main) {
  ...
  try { users = await adminApi('/users'); } catch (e) {
    main.innerHTML = `<div class="adm-section-header"><div class="adm-section-title"><div class="icon-pill"><i class="fas fa-users"></i></div> Kullanýcýlar</div></div><div class="card"><div class="card-body" style="color:var(--red2);padding:20px"><i class="fas fa-exclamation-circle"></i> ${escHtml(e.message)}</div></div>`;
    return;
  }
  main.innerHTML = `
    <div class="adm-section-header">
      <div class="adm-section-title"><div class="icon-pill"><i class="fas fa-users"></i></div> Kullanýcýlar <span style="font-size:13px;font-weight:400;color:var(--text2)">(${users.length})</span></div>
      <div class="adm-search"><i class="fas fa-search"></i><input type="text" id="user-search" placeholder="Kullanýcý, e-posta, IP ara..." style="min-width:240px" /></div>
    </div>
    <div class="card">
      <div class="table-wrap">
        <table>
          <thead><tr><th>ID</th><th>Kullanýcý</th><th>E-posta</th><th>Seviye</th><th>Ýstatistik</th><th>IP</th><th>Kayýt</th><th>Durum</th><th>Ýþlem</th></tr></thead>
...
function renderUsersTable(users) {
  ...
  if (!users.length) { tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;color:var(--text3);padding:32px">Kullanýcý bulunamadý</td></tr>'; return; }
  tbody.innerHTML = users.map(u => `<tr>
    ...
    <td>${u.banned ? '<span class="badge badge-red"><i class="fas fa-ban"></i> Banlý</span>' : '<span class="badge badge-green"><i class="fas fa-check"></i> Aktif</span>'}</td>
    <td>
      <div style="display:flex;gap:4px;flex-wrap:wrap">
        ...
        ${u.banned
          ? `<button class="btn btn-green btn-xs unban-user-btn" data-id="${u.id}" title="Ban Kaldýr"><i class="fas fa-unlock"></i></button>`
  ...
  tbody.addEventListener('click', async e => {
    ...
    if (unban) { if (!confirm('Ban kaldýrýlsýn mý?')) return; try { await adminApi('/user/'+unban.dataset.id+'/unban',{method:'POST'}); toast('Ban kaldýrýldý'); loadSection('users'); } catch(e){toast(e.message,'error');} }
    if (del) { if (!confirm('Kullanýcý kalýcý silinsin mi?')) return; try { await adminApi('/user/'+del.dataset.id,{method:'DELETE'}); toast('Silindi'); loadSection('users'); } catch(e){toast(e.message,'error');} }
    if (perm) { const u = users.find(x => x.id == perm.dataset.id); if (u) showPermModal(u); }
  });
}

function showEditUserModal(user) {
  showModal('Kullanýcý Düzenle — ' + user.username, `
    <div class="form-row">
      <div class="form-group"><label>Kullanýcý Adý</label><input id="eu-username" value="${escHtml(user.username)}" /></div>
    ...
    <div class="form-row">
      ...
      <div class="form-group"><label>Yeni Þifre (boþ=deðiþme)</label><input id="eu-pw" type="password" placeholder="••••••" /></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label>Seviye ID</label><input id="eu-level" type="number" value="${user.level_id||1}" /></div>
      <div class="form-group"><label>Ünvan</label><input id="eu-title" value="${escHtml(user.title||'')}" placeholder="Örn: Yazýlýmcý" /></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label>Ünvan</label><input id="eu-title" value="${escHtml(user.title||'')}" placeholder="Örn: Yazýlýmcý" /></div>
      <div class="form-group"><label>Ýsim Rengi</label><input id="eu-color" type="color" value="${user.name_color||'#f5f5f5'}" style="height:38px;cursor:pointer" /></div>
    </div>
  ...
  $('#eu-submit').addEventListener('click', async () => {
    ...
    try {
      ...
      toast('Kullanýcý güncellendi'); hideModal(); loadSection('users');
    } catch (e) { $('#eu-error').textContent = e.message; }
  });
}

function showBanModal(userId) {
  showModal('Kullanýcýyý Banla', `
  ...
  $('#ban-submit').addEventListener('click', async () => {
    try { await adminApi('/user/'+userId+'/ban',{method:'POST',body:JSON.stringify({ban_type:$('#ban-type').value})}); toast('Banlandý'); hideModal(); loadSection('users'); }
...
async function showPermModal(user) {
  ...
  const permDefs = [
    ...
    { key:'can_ban_users', label:'Üye Yasakla/Kaldýr', desc:'Ban atabilir, kaldýrabilir', icon:'fas fa-ban' },
    { key:'can_delete_content', label:'Ýçerik Sil', desc:'Forum, kitap, yorum silebilir', icon:'fas fa-trash' },
    { key:'can_edit_content', label:'Ýçerik Düzenle', desc:'Forum ve kitap düzenleyebilir', icon:'fas fa-edit' },
    ...
    { key:'can_manage_announcements', label:'Duyuru Yönet', desc:'Duyuru oluþtur/düzenle/sil', icon:'fas fa-bullhorn' },
    { key:'can_view_logs', label:'Log Görüntüle', desc:'Sistem loglarýný okuyabilir', icon:'fas fa-history' },
    { key:'can_manage_settings', label:'Site Ayarlarý', desc:'Site ayarlarýný deðiþtirebilir', icon:'fas fa-cog' },
  ...
  showModal(`Yetki Düzenleme — ${user.username}`, `
    ...
    <div style="margin-bottom:16px;padding:12px 14px;background:rgba(234,179,8,0.1);border:1px solid rgba(234,179,8,0.3);border-radius:10px;font-size:12px;color:#facc15">
      ...
      <strong>Bu kullanýcý þu an SÜPERADMÝN.</strong> Yetki kaydý yokken tüm yetkilere sahiptir.
      Aþaðýdan kýsýtlý yetki kaydý oluþturabilirsin — bu durumda sadece seçtiðin yetkiler geçerli olur.
    </div>` : `
    <div style="margin-bottom:16px;padding:10px 14px;background:rgba(88,101,242,0.1);border:1px solid rgba(88,101,242,0.2);border-radius:10px;font-size:12px">
      ...
      Kaydet'e basýnca kullanýcýya <strong>is_admin=1</strong> atanýr ve sadece iþaretli yetkiler verilir.
      Tüm yetkiler verirsen süperadmin gibi çalýþýr.
    </div>`}
    ...
    <button class="btn btn-blue" id="perm-save-btn" style="width:100%;justify-content:center;margin-top:8px"><i class="fas fa-save"></i> Kaydet &amp; Adminliði Etkinleþtir</button>
...
async function renderForums(main) {
  ...
  main.innerHTML = `
    <div class="adm-section-header">
      ...
      <div class="adm-search"><i class="fas fa-search"></i><input type="text" id="forum-search" placeholder="Baþlýk veya kullanýcý ara..." style="min-width:240px" /></div>
    </div>
    <div class="card">
      <div class="table-wrap">
        <table>
          <thead><tr><th>ID</th><th>Baþlýk</th><th>Yazar</th><th>Görüntülenme</th><th>Beðeni</th><th>Tarih</th><th>Ýþlem</th></tr></thead>
  ...
  const renderTable = (list) => {
    ...
    if (!list.length) { tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--text3);padding:32px">Konu bulunamadý</td></tr>'; return; }
    ...
    tbody.querySelectorAll('.edit-forum-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        showModal(`?? Konu Düzenle — #${f.id}`, `
          <div class="form-group"><label>Baþlýk</label><input id="ef-title" value="${escHtml(f.title)}" /></div>
          <div class="form-row">
    ...
    tbody.querySelectorAll('.del-forum-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!confirm('Bu konuyu silmek istediðine emin misin?')) return;
...
async function renderBooks(main) {
  ...
  main.innerHTML = `
    <div class="adm-section-header">
      ...
      <div class="adm-search"><i class="fas fa-search"></i><input type="text" id="book-search" placeholder="Baþlýk veya kullanýcý ara..." style="min-width:240px" /></div>
    </div>
    <div class="card">
      <div class="table-wrap">
        <table>
          <thead><tr><th>ID</th><th>Baþlýk</th><th>Yazar</th><th>Sayfa</th><th>Tarih</th><th>Ýþlem</th></tr></thead>
  ...
  const renderTable = (list) => {
    ...
    if (!list.length) { tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--text3);padding:32px">Kitap bulunamadý</td></tr>'; return; }
    ...
    tbody.querySelectorAll('.del-book-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!confirm('Bu kitabý silmek istediðine emin misin?')) return;
...
async function renderGroups(main) {
  ...
  main.innerHTML = `
    <div class="adm-section-header">
      ...
      <div class="adm-search"><i class="fas fa-search"></i><input type="text" id="group-search" placeholder="Grup adý veya sahibi ara..." style="min-width:240px" /></div>
    </div>
    <div class="card">
      <div class="table-wrap">
        <table>
          <thead><tr><th>ID</th><th>Grup Adý</th><th>Sahibi</th><th>Tarih</th><th>Ýþlem</th></tr></thead>
  ...
  const renderTable = (list) => {
    ...
    if (!list.length) { tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--text3);padding:32px">Grup bulunamadý</td></tr>'; return; }
    ...
    tbody.querySelectorAll('.del-group-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!confirm('Bu grubu silmek istediðine emin misin?')) return;
...
async function renderArtists(main) {
  ...
  const renderTable = (list) => {
    ...
    if (!list.length) {
      tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--text3);padding:32px"><i class="fas fa-microphone-slash" style="font-size:28px;margin-bottom:8px;display:block"></i>Artist bulunamadý</td></tr>`;
      return;
    }
    tbody.innerHTML = list.map(a => `<tr>
      ...
      <td>${a.banned ? '<span class="badge badge-red"><i class="fas fa-ban"></i> Banlý</span>' : '<span class="badge badge-green"><i class="fas fa-check"></i> Aktif</span>'}</td>
      <td>
        <div style="display:flex;gap:4px;flex-wrap:wrap">
          <button class="btn btn-blue btn-xs view-artist-songs-btn" data-id="${a.id}" data-name="${escHtml(a.username)}" title="Þarkýlarýný Gör"><i class="fas fa-music"></i> Þarkýlar</button>
          <button class="btn btn-outline btn-xs edit-artist-btn" data-id="${a.id}" title="Bilgileri Düzenle"><i class="fas fa-edit"></i></button>
          <button class="btn btn-danger btn-xs revoke-artist-btn" data-id="${a.id}" data-name="${escHtml(a.username)}" title="Artist Rozetini Kaldýr"><i class="fas fa-microphone-slash"></i></button>
        </div>
      </td>
    </tr>`).join('');
    ...
    tbody.querySelectorAll('.revoke-artist-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!confirm(`${btn.dataset.name} kullanýcýsýnýn artist rozeti kaldýrýlsýn mý?`)) return;
        try {
          await adminApi('/artists/'+btn.dataset.id, { method:'PUT', body:JSON.stringify({ is_artist: 0 }) });
          toast('Artist rozeti kaldýrýldý');
  ...
  main.innerHTML = `
    <div class="adm-section-header">
      ...
      <div class="adm-search"><i class="fas fa-search"></i><input type="text" id="artist-search" placeholder="Artist veya kullanýcý ara..." style="min-width:240px" /></div>
    </div>
    <div class="card">
      <div class="table-wrap">
        <table>
          <thead><tr><th>Artist</th><th>Tür</th><th>Þarkýlar</th><th>Artist'ten beri</th><th>Durum</th><th>Ýþlem</th></tr></thead>
...
async function showArtistSongsModal(artistId, artistName) {
  showModal(`?? ${artistName} — Þarkýlar`, `<div class="loading-center" style="padding:40px"><div class="spinner"></div></div>`);
  let songs = [];
  ...
  const renderSongs = (list) => {
    ...
    if (!list.length) {
      wrap.innerHTML = '<div style="text-align:center;color:var(--text3);padding:32px"><i class="fas fa-music" style="font-size:28px;margin-bottom:8px;display:block"></i>Þarký yok</div>';
      return;
    }
        ...
        <div style="flex:1;min-width:0">
          ...
          ${isBanned ? `<div style="font-size:10px;color:var(--red2);margin-top:2px">
            ...
            ${s.ban_until && !banExpired
              ...
              : s.ban_until ? ' <span style="color:var(--text3)">(süresi doldu)</span>' : ' <span style="color:var(--text3)">(kalýcý)</span>'}
          </div>` : ''}
        </div>
        <div style="display:flex;gap:4px;align-items:center;flex-shrink:0">
          ${isBanned && !banExpired
            ? `<button class="btn btn-green btn-xs song-unban-btn" data-id="${s.id}"><i class="fas fa-unlock"></i> Baný Kaldýr</button>`
    ...
    wrap.querySelectorAll('.song-unban-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!confirm('Baný kaldýrmak istediðine emin misin?')) return;
        try {
          await adminApi('/songs/'+btn.dataset.id+'/unban', { method:'POST' });
          toast('Ban kaldýrýldý');
  ...
  $('#modal-body').innerHTML = `
    <div style="margin-bottom:12px;display:flex;align-items:center;justify-content:space-between">
      <span style="font-size:12px;color:var(--text2)">${songs.length} þarký</span>
...
function showSongBanModal(songId, songTitle, songs, renderSongs) {
  showModal(`?? Þarký Banla — ${songTitle}`, `
    <div style="background:rgba(220,38,38,0.07);border:1px solid var(--border-red);border-radius:10px;padding:14px;margin-bottom:16px;font-size:12px;color:var(--text2)">
      ...
      Ban uygulanan þarký dinleyicilere gösterilmez. Süreli ban bitince otomatik aktife döner.
    </div>
    ...
    <div class="form-group">
      ...
      <select id="ban-duration">
        <option value="0">Kalýcý (elle kaldýrana kadar)</option>
        <option value="1">1 Gün</option>
        <option value="3">3 Gün</option>
        <option value="7">7 Gün</option>
        <option value="14">14 Gün</option>
        <option value="30">30 Gün</option>
        <option value="custom">Özel Gün Sayýsý...</option>
      </select>
    </div>
    <div id="custom-days-wrap" class="form-group hidden">
      <label>Gün Sayýsý</label>
    ...
    <div style="display:flex;gap:8px">
      <button class="btn btn-outline" id="ban-cancel-btn" style="flex:1;justify-content:center">Ýptal</button>
      <button class="btn btn-primary" id="ban-confirm-btn" style="flex:1;justify-content:center"><i class="fas fa-ban"></i> Baný Uygula</button>
    </div>
  ...
  const updatePreview = () => {
    ...
    if (!p) return;
    if (!days) {
      p.innerHTML = '<i class="fas fa-infinity" style="color:var(--red2);margin-right:6px"></i><span style="color:var(--red2)">Kalýcý ban — admin elle kaldýrana kadar devam eder</span>';
    } else {
      const until = new Date(Date.now() + days * 86400000);
      p.innerHTML = `<i class="fas fa-clock" style="color:var(--orange);margin-right:6px"></i><span style="color:var(--orange)">Bitiþ: ${until.toLocaleDateString('tr-TR', {day:'2-digit',month:'long',year:'numeric'})}</span>`;
    }
  };
  updatePreview();
  ...
  $('#ban-confirm-btn').addEventListener('click', async () => {
    ...
    } catch(e) { err.textContent=e.message; btn.disabled=false; btn.innerHTML='<i class="fas fa-ban"></i> Baný Uygula'; }
  });
}
...
function showEditArtistModal(artist, list, renderTable) {
  showModal(`?? Artist Düzenle — ${escHtml(artist.username)}`, `
    ...
    <div class="form-group">
      <label>Sahne Adý / Display Name</label>
      <input id="ea-display" value="${escHtml(artist.artist_display_name||'')}" placeholder="Kullanýcý adýndan farklý sanatçý adý..." />
    </div>
    <div class="form-row">
      <div class="form-group">
    ...
    <div class="form-group">
      ...
      <textarea id="ea-bio" rows="3" placeholder="Artist hakkýnda kýsa açýklama...">${escHtml(artist.artist_bio||'')}</textarea>
    </div>
    <div style="background:rgba(220,38,38,0.07);border:1px solid var(--border-red);border-radius:10px;padding:12px;margin-bottom:16px">
      ...
      <div style="font-size:11px;color:var(--text3);margin-top:6px;margin-left:26px">Rozeti kaldýrýrsan kullanýcý yeni þarký yükleyemez. Mevcut þarkýlar silinmez.</div>
    </div>
...
async function renderLevels(main) {
  ...
  const renderTable = () => {
    ...
    tbody.querySelectorAll('.edit-level-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        showModal(`?? Seviye Düzenle — ${l.name}`, `
          <div class="form-group"><label>Ýsim</label><input id="elv-name" value="${escHtml(l.name)}" /></div>
          <div class="form-row">
          ...
          <div class="form-row">
            ...
            <div class="form-group"><label>Min. Kitap Sayfasý</label><input id="elv-pages" type="number" value="${l.min_book_pages||0}" /></div>
          </div>
          <div class="form-group">
            <label class="checkbox-label" style="margin:0">
              <input type="checkbox" id="elv-require-any" style="width:auto" ${l.require_any ? 'checked' : ''} />
              <div>
                <span style="font-weight:600;font-size:13px">Herhangi birini tamamlayýnca atla</span>
                <div style="font-size:11px;color:var(--text3);margin-top:2px">Ýþaretliyse konu, kitap veya yorumdan birini yapan atlar.</div>
              </div>
            </label>
          </div>
    ...
    tbody.querySelectorAll('.del-level-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!confirm('Bu seviyeyi silmek istediðine emin misin?')) return;
  ...
  main.innerHTML = `
    ...
    <div class="card">
      <div class="table-wrap">
        <table>
          <thead><tr><th>Ýkon</th><th>Ýsim</th><th>Renk</th><th>Koþullar</th><th>Sýra</th><th>Ýþlem</th></tr></thead>
  ...
  $('#new-level-btn').addEventListener('click', () => {
    showModal('Yeni Seviye Ekle', `
      <div class="form-group"><label>Ýsim</label><input id="lv-name" /></div>
      <div class="form-row">
        <div class="form-group" style="flex:1">
          <label>Ýkon <span style="font-size:11px;color:var(--text3)">(FA class veya emoji)</span></label>
          ...
          <button type="button" class="btn btn-outline btn-sm" id="lv-icon-picker-btn" style="margin-top:6px;width:100%"><i class="fas fa-icons"></i> Ýkon Seç</button>
      ...
      <div class="form-row">
        <div class="form-group"><label>Min. Kitap Sayfasý</label><input id="lv-pages" type="number" value="0" /></div>
        <div class="form-group"><label>Sýra</label><input id="lv-order" type="number" value="${levels.length+1}" /></div>
      </div>
      <div class="form-group">
        <label class="checkbox-label" style="margin:0">
          ...
          <div>
            <span style="font-weight:600;font-size:13px">Herhangi birini tamamlayýnca atla</span>
            <div style="font-size:11px;color:var(--text3);margin-top:2px">Ýþaretliyse konu, kitap veya yorumdan birini yapan atlar. Ýþaretsizse hepsini yapmasý gerekir.</div>
          </div>
        </label>
      </div>
    ...
    $('#lv-save-btn').addEventListener('click', async () => {
      ...
      if (!name) { err.textContent='Ýsim zorunlu'; return; }
      try {
...
async function renderTags(main) {
  ...
  const renderTable = () => {
    ...
    tbody.querySelectorAll('.del-tag-btn:not([disabled])').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!confirm('Bu etiketi silmek istediðine emin misin?')) return;
  ...
  main.innerHTML = `
    ...
    <div class="card">
      <div class="table-wrap">
        <table>
          <thead><tr><th>Etiket</th><th>Renk</th><th>Tür</th><th>Ýþlem</th></tr></thead>
  ...
  $('#new-tag-btn').addEventListener('click', () => {
    showModal('Yeni Etiket Ekle', `
      <div class="form-group"><label>Ýsim</label><input id="tag-name" /></div>
...
async function renderLogs(main) {
  ...
  main.innerHTML = `
    <div class="adm-section-header">
      <div class="adm-section-title"><div class="icon-pill"><i class="fas fa-history"></i></div> Sistem Loglarý <span style="font-size:13px;font-weight:400;color:var(--text2)">(${logs.length})</span></div>
  ...
  $('#log-search').addEventListener('input', e => {
    ...
    $('#logs-list').innerHTML = filtered.length ? filtered.map(l => `
      <div style="padding:10px 16px;border-bottom:1px solid var(--border);font-size:13px;display:flex;align-items:center;gap:10px">
        ...
        <span style="flex:1">${escHtml(l.action)}${l.target?' <span style="color:var(--text3)">› '+escHtml(l.target)+'</span>':''}</span>
        <span style="color:var(--text3);font-size:11px;white-space:nowrap">${timeAgo(l.created_at)}</span>
      </div>`).join('') : '<div style="padding:32px;text-align:center;color:var(--text3)">Sonuç bulunamadý</div>';
  });
}
...
async function renderAdminMessages(main) {
  ...
  main.innerHTML = `
    ...
    <div class="card">
      <div class="table-wrap">
        <table>
          <thead><tr><th>ID</th><th>Kullanýcý 1</th><th>Kullanýcý 2</th><th>Son Mesaj</th><th>Ýþlem</th></tr></thead>
          <tbody>
            ...
            </tr>`).join('') : '<tr><td colspan="5" style="text-align:center;color:var(--text3);padding:32px">Konuþma bulunamadý</td></tr>'}
          </tbody>
        </table>
      </div>
    </div>`;
...
async function renderAnnouncements(main) {
  ...
  main.innerHTML = `
    ...
    <div class="card">
      <div class="table-wrap">
        <table>
          <thead><tr><th>Baþlýk</th><th>Konum</th><th>Boyut</th><th>Bitiþ</th><th>Durum</th><th>Ýþlem</th></tr></thead>
...
function showAnnModal(ann, anns) {
  ...
  showModal(isEdit ? 'Duyuru Düzenle' : 'Yeni Duyuru', `
    <div class="form-group"><label>Baþlýk</label><input id="ann-title" value="${escHtml(ann?.title||'')}" placeholder="Duyuru baþlýðý..." /></div>
    <div class="form-group"><label>Ýçerik</label><textarea id="ann-content" rows="3" placeholder="Duyuru metni...">${escHtml(ann?.content||'')}</textarea></div>
    <div class="form-row">
      ...
      <div class="form-group"><label>Yazý Rengi</label><input id="ann-text-color" type="color" value="${ann?.text_color||'#ffffff'}" style="height:38px;cursor:pointer" /></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label>Kenarlýk Rengi</label><input id="ann-border" type="color" value="${ann?.border_color||'#991b1b'}" style="height:38px;cursor:pointer" /></div>
    ...
    <div style="background:var(--bg4);border-radius:10px;padding:14px;margin-bottom:16px">
      <div style="font-size:11px;font-weight:600;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px">Süre Ayarý</div>
      <div class="form-row" style="margin-bottom:0">
        <div class="form-group" style="margin-bottom:0"><label>Deðer (0 = süresiz)</label><input id="ann-dur-val" type="number" min="0" value="0" /></div>
    ...
    <button class="btn btn-primary" id="ann-save-btn" style="width:100%;justify-content:center">${isEdit?'Güncelle':'Oluþtur'}</button>
  ...
  function updatePreview() {
    ...
    const title = $('#ann-title')?.value||'Baþlýk', content = $('#ann-content')?.value||'Ýçerik';
  ...
  $('#ann-save-btn').addEventListener('click', async () => {
    ...
    if (!body.title || !body.content) { $('#ann-error').textContent = 'Baþlýk ve içerik zorunlu'; return; }
    try {
      ...
      toast(isEdit ? 'Duyuru güncellendi' : 'Duyuru oluþturuldu'); hideModal(); loadSection('announcements');
...
async function renderAdminSongs(main) {
  ...
  main.innerHTML = `
    <div class="adm-section-header">
      ...
      <div class="adm-search"><i class="fas fa-search"></i><input type="text" id="song-search" placeholder="Þarký, sanatçý, daðýtýcý ara..." style="min-width:220px" /></div>
    </div>
    <div class="card">
      <div class="table-wrap">
        <table>
          <thead><tr><th>Kapak</th><th>Baþlýk / Sanatçý</th><th>Tür</th><th>Daðýtýcý</th><th>Dinlenme</th><th>Durum</th><th>Yükleyen</th><th>Tarih</th><th>Ýþlem</th></tr></thead>
  ...
  const render = (list) => {
    ...
    if (!list.length) { t.innerHTML = '<tr><td colspan="9" style="text-align:center;color:var(--text3);padding:32px">Þarký yok</td></tr>'; return; }
    t.innerHTML = list.map(s => `<tr>
      ...
      <td>${s.status === 'active' ? '<span class="badge badge-green">Aktif</span>' : s.status === 'suspended' ? '<span class="badge badge-red">Askýda</span>' : `<span class="badge badge-gray">${escHtml(s.status)}</span>`}</td>
      ...
      <td>
        <div style="display:flex;gap:4px;flex-wrap:wrap">
          ...
          ${s.status === 'active'
            ? `<button class="btn btn-danger btn-xs sus-btn" data-id="${s.id}" title="Askýya Al"><i class="fas fa-pause"></i></button>`
    ...
    t.addEventListener('click', async e => {
      ...
      if (sus) {
        if (!confirm('Þarký askýya alýnsýn mý?')) return;
        try { await adminApi('/songs/'+sus.dataset.id, {method:'PUT', body:JSON.stringify({status:'suspended'})}); toast('Askýya alýndý'); loadSection('songs'); } catch(e){toast(e.message,'error');}
      }
      if (unsus) {
        try { await adminApi('/songs/'+unsus.dataset.id, {method:'PUT', body:JSON.stringify({status:'active'})}); toast('Aktife alýndý'); loadSection('songs'); } catch(e){toast(e.message,'error');}
      }
      if (ds) { if (!confirm('Þarký kalýcý silinsin mi?')) return; try { await adminApi('/songs/'+ds.dataset.id, {method:'DELETE'}); toast('Silindi'); loadSection('songs'); } catch(e){toast(e.message,'error');} }
    });
  };
  render(songs);
...
function showSongEditModal(song) {
  showModal(`Þarký Düzenle — ${escHtml(song.title)}`, `
    <div class="form-row">
      <div class="form-group"><label>Þarký Adý</label><input id="se-title" value="${escHtml(song.title)}" /></div>
      <div class="form-group"><label>Sanatçý Adý</label><input id="se-artist" value="${escHtml(song.artist_name)}" /></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label>Daðýtýcý</label><input id="se-dist" value="${escHtml(song.distributor||'')}" /></div>
      <div class="form-group"><label>Tür</label><input id="se-genre" value="${escHtml(song.genre||'')}" /></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label>Dinlenme Sayýsý</label><input id="se-plays" type="number" value="${song.play_count}" /></div>
      <div class="form-group"><label>Durum</label>
        <select id="se-status">
          <option value="active" ${song.status==='active'?'selected':''}>Aktif</option>
          <option value="suspended" ${song.status==='suspended'?'selected':''}>Askýda</option>
          <option value="deleted" ${song.status==='deleted'?'selected':''}>Silindi</option>
        </select>
      </div>
    </div>
    <div class="form-group"><label>Þarký Sözleri</label><textarea id="se-lyrics" rows="6">${escHtml(song.lyrics||'')}</textarea></div>
    <div class="form-group"><label>Yeni Ses Dosyasý (boþ býrak = deðiþme)</label>
    ...
    <div class="form-group"><label>Yeni Kapak Fotoðrafý (boþ býrak = deðiþme)</label>
  ...
  document.getElementById('se-save').addEventListener('click', async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error||'Hata');
      toast('Þarký güncellendi'); hideModal(); loadSection('songs');
...
async function renderArtistApps(main) {
  ...
  main.innerHTML = `
    ...
    <div style="margin-bottom:24px">
      ...
      <div class="card">
        <div class="table-wrap">
          <table>
            <thead><tr><th>Kullanýcý</th><th>Tür</th><th>Örnek</th><th>Not</th><th>Tarih</th><th>Ýþlem</th></tr></thead>
            <tbody id="pending-tbody"></tbody>
          </table>
        </div>
      </div>
    </div>` : '<div class="card" style="margin-bottom:20px"><div class="card-body" style="text-align:center;color:var(--text3);padding:30px"><i class="fas fa-check-circle" style="font-size:28px;margin-bottom:8px;color:var(--green)"></i><div>Bekleyen baþvuru yok</div></div></div>'}
    ${others.length ? `
    <div>
      <div style="font-size:12px;font-weight:600;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px">Geçmiþ Baþvurular</div>
      <div class="card">
        <div class="table-wrap">
          <table>
            <thead><tr><th>Kullanýcý</th><th>Tür</th><th>Durum</th><th>Ýnceleme</th><th>Tarih</th></tr></thead>
            <tbody>${others.map(a => `<tr>
              ...
              <td>${a.status === 'accepted' ? '<span class="badge badge-green"><i class="fas fa-check"></i> Onaylandý</span>' : '<span class="badge badge-red"><i class="fas fa-times"></i> Reddedildi</span>'}</td>
  ...
  if (pt && pending.length) {
    ...
    pt.addEventListener('click', async e => {
      ...
      if (approve) {
        if (!confirm('Baþvuru onaylansýn mý? Kullanýcýya artist rozeti verilecek.')) return;
        try {
...
async function renderSettings(main) {
  ...
  main.innerHTML = `
    <div class="adm-section-header"><div class="adm-section-title"><div class="icon-pill"><i class="fas fa-cog"></i></div> Site Ayarlarý</div></div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
      <div class="card">
        <div class="card-header"><span><i class="fas fa-palette" style="color:var(--red2);margin-right:8px"></i>Genel</span></div>
        <div class="card-body">
          <div class="form-group"><label>Site Adý</label><input id="s-sitename" value="${escHtml(settings['site_name']||'TeaTube')}" /></div>
          <div class="form-group">
          ...
          <div class="form-group"><label>Site Açýklamasý</label><textarea id="s-desc" rows="3">${escHtml(settings['site_description']||'')}</textarea></div>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="form-group"><label>Yeni Admin Þifresi</label><input id="s-newpw" type="password" placeholder="Boþ býrakýrsan deðiþmez" /></div>
          <div class="form-group"><label>Þifreyi Onayla</label><input id="s-newpw2" type="password" placeholder="••••••" /></div>
          <button class="btn btn-primary" id="s-pw-save" style="width:100%;justify-content:center"><i class="fas fa-key"></i> Þifreyi Güncelle</button>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="form-group"><label>Footer Yazýsý</label><input id="s-footer" value="${escHtml(settings['footer_copyright_text']||'')}" placeholder="© Copyright 2026" /></div>
      ...
      <div class="card">
        <div class="card-header"><span><i class="fas fa-cogs" style="color:var(--red2);margin-right:8px"></i>Forum Ayarlarý</span></div>
        <div class="card-body">
          <div class="form-group"><label>Ayrýlmýþ Kullanýcý Adlarý</label><textarea id="s-reserved-usernames" rows="4" placeholder="Birer satýra yazýn">${escHtml(settings['reserved_usernames']||'')}</textarea></div>
          <div class="form-group"><label class="checkbox-label"><input type="checkbox" id="s-default-forum-comments" ${settings['forum_default_allow_comments']!=='0'?'checked':''} /> Yeni konular için varsayýlan olarak yorumlara izin ver</label></div>
          <div class="form-group"><label class="checkbox-label"><input type="checkbox" id="s-default-forum-likes" ${settings['forum_default_allow_likes']!=='0'?'checked':''} /> Yeni konular için varsayýlan olarak beðenilere izin ver</label></div>
      ...
      <div class="card">
        <div class="card-header"><span><i class="fas fa-music" style="color:var(--red2);margin-right:8px"></i>Þarký Yayýnlama Kurallarý</span></div>
        <div class="card-body">
          <div class="form-group"><label>Kendi Þarkým – Kurallar</label><textarea id="s-music-own" rows="4">${escHtml(settings['music_own_rules']||'')}</textarea></div>
          ...
          <div class="form-group"><label>Baþkasýnýn Þarkýsý – Kurallar</label><textarea id="s-music-other" rows="4">${escHtml(settings['music_other_rules']||'')}</textarea></div>
  ...
  document.getElementById('s-forum-settings-save')?.addEventListener('click', async () => {
    ...
    try {
      ...
      toast('Forum ayarlarý kaydedildi');
  ...
  document.getElementById('s-pw-save').addEventListener('click', async () => {
    ...
    if (!pw) { msg.textContent='Þifre boþ olamaz'; return; }
    if (pw !== pw2) { msg.textContent='Þifreler eþleþmiyor'; return; }
    ...
    msg.style.color='var(--green)'; msg.textContent='Þifre güncellendi';
  });
