...
function showMobilePostMenu() {
  showModal('Hýzlý Paylaþým', `
    <div style="display:flex;flex-direction:column;gap:12px">
      <button class="btn btn-primary btn-block" id="mobile-post-photo"><i class="fas fa-camera-retro"></i> Fotoðraf Paylaþ</button>
      <button class="btn btn-primary btn-block" id="mobile-post-forum"><i class="fas fa-comments"></i> Konu Aç</button>
      <button class="btn btn-primary btn-block" id="mobile-post-book"><i class="fas fa-book"></i> Kitap Paylaþ</button>
      <button class="btn btn-primary btn-block" id="mobile-post-group"><i class="fas fa-users"></i> Grup Oluþtur</button>
      <button class="btn btn-primary btn-block" id="mobile-post-music"><i class="fas fa-music"></i> Müzik Paylaþ</button>
    </div>
  `);
...
async function showNewPhotoModal() {
  showModal('Fotoðraf Yükle', `
    <div class="form-group"><label>Fotoðraf</label><input type="file" id="photo-file-input" accept="image/*" /></div>
    <div class="form-group"><label>Baþlýk / Açýklama (opsiyonel)</label><input type="text" id="photo-caption" /></div>
  ...
  $('#photo-submit-btn')?.addEventListener('click', async () => {
    ...
    if (!fileInput || !fileInput.files.length) { $('#photo-upload-error').textContent = 'Fotoðraf seçmelisiniz'; return; }
...
function showEntryAuthPopup() {
  ...
  showModal('TeaTube’a Katýl', `
    <div style="display:flex;flex-direction:column;gap:12px;text-align:center">
      <p>En iyi paylaþýmlar için hemen giriþ yap veya kaydol.</p>
      <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:center">
        <a href="/giris" data-link class="btn btn-primary btn-sm" style="flex:1;min-width:120px">Giriþ Yap</a>
        <a href="/kayit" data-link class="btn btn-ghost btn-sm" style="flex:1;min-width:120px">Kayýt Ol</a>
      </div>
...
function userDisplayName(u) {
  ...
  return `<span class="user-badge" ${color}>${escHtml(u.username)}${u.nickname ? ` <span class="user-nickname-inline">${escHtml(u.nickname)}</span>` : ''}${selectedBadge}${adminBadge}</span>`;
}

function brandProfileClick(e) {
...
function renderContent(text) {
  if (!text) return '';
  // XSS güvenli: önce escape, sonra pattern'lere dönüþtür
  const safe = String(text)
    .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
  return safe
    // #hashtag › mavi týklanabilir link
    .replace(/#([a-zA-Z0-9_çðýöþüÇÐÝÖÞÜ]+)/g, (_, tag) =>
      `<a href="/forum?tag=${encodeURIComponent(tag)}" data-link class="inline-hashtag">#${tag}</a>`)
    // @mention › profil link
    .replace(/@([a-zA-Z0-9_çðýöþüÇÐÝÖÞÜ]+)/g, (_, user) =>
...
async function renderVip(app) {
    ...
    <div class="container page">
      ...
      <div style="display:flex;gap:16px;flex-wrap:wrap">
        <div class="card" style="flex:1;min-width:260px">
          ...
          <div class="card-body">
            <p style="color:var(--text-secondary)">Profil renginizi özelleþtirin, özel rozet ve öncelikli destek.</p>
            <p style="font-size:18px;font-weight:700">?29 / aylýk</p>
            <div style="display:flex;gap:8px;margin-top:12px">
              <button class="btn btn-primary" id="buy-vip">Satýn Al</button>
        ...
        <div class="card" style="flex:1;min-width:260px">
          ...
          <div class="card-body">
            <p style="color:var(--text-secondary)">VIP özellikleri + ekstra görünürlük ve içerik artýlarý.</p>
            <p style="font-size:18px;font-weight:700">?49 / aylýk</p>
            <div style="display:flex;gap:8px;margin-top:12px">
              <button class="btn btn-primary" id="buy-plus">Satýn Al</button>
              <button class="btn btn-outline" id="gift-plus">Hediye Et</button>
            </div>
          </div>
        </div>
      </div>
      <div style="margin-top:18px;color:var(--text-muted)">Not: Bu sayfa demo amaçlýdýr — gerçek ödeme entegrasyonu yok, satýn alma anýnda hesabýnýza paket atanýr.</div>
  ...
  $('#buy-vip')?.addEventListener('click', async () => {
    ...
    showModal('Ödeme — VIP', `<div style="padding:12px"><p>VIP üyelik satýn alma iþlemi baþlatýlýyor...</p><div id="checkout-spinner" style="margin-top:12px"><div class="spinner"></div></div></div>`);
    try {
      ...
      hideModal(); showModal('Ödeme Hatasý', `<div style="padding:12px">Ödeme baþlatýlamadý: ${escHtml(resp && resp.error || 'Bilinmeyen hata')}</div>`);
    } catch (e) { hideModal(); showModal('Ödeme Hatasý', `<div style="padding:12px">Ödeme gerçekleþtirilemedi: ${escHtml(e.message)}</div>`); }
  });
  $('#buy-plus')?.addEventListener('click', async () => {
    showModal('Ödeme — Plus+', `<div style="padding:12px"><p>Plus+ üyelik satýn alma iþlemi baþlatýlýyor...</p><div id="checkout-spinner" style="margin-top:12px"><div class="spinner"></div></div></div>`);
    try {
  ...
  function openGiftModal(type) {
    showModal('Üyelik Hediye Et', `
      <div style="padding:12px;display:flex;flex-direction:column;gap:8px">
        <div>Kime hediye edeceksiniz? (kullanýcý adý)</div>
        <input id="gift-to-username" placeholder="alici_kullanici_adi" />
        <div style="font-size:13px;color:var(--text-muted)">Eðer kullanýcý henüz kayýtlý deðilse, hediye kodu oluþturulacak ve el ile paylaþabilirsiniz.</div>
        <div style="display:flex;gap:8px;margin-top:8px"><button class="btn btn-primary" id="gift-send">Gönder</button><button class="btn btn-ghost" id="gift-cancel">Ýptal</button></div>
      </div>
    `);
    $('#gift-cancel')?.addEventListener('click', hideModal);
    $('#gift-send')?.addEventListener('click', async () => {
      const to = ($('#gift-to-username').value || '').trim();
      if (!to) { toast('Alýcý kullanýcý adý girin', 'error'); return; }
      try {
        ...
        if (resp.assigned_to) {
          ...
          showModal('Hediye Kodu', `<div style="padding:12px">Hediye oluþturuldu. Kodu alýcýya iletin: <div style="margin-top:8px;padding:8px;background:var(--bg-card2);border:1px solid var(--border);border-radius:6px;font-weight:700">${escHtml(resp.code)}</div></div>`);
        }
      } catch (e) { hideModal(); showModal('Hata', `<div style="padding:12px">Hediye baþarýsýz: ${escHtml(e.message)}</div>`); }
    });
  }
  ...
  $('#redeem-btn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      currentUser = updated; updateNavUI(); toast('Hediye baþarýyla kullanýldý'); renderRoute(location.pathname);
  ...
  async function loadGiftHistory() {
    ...
    try {
      ...
      if (!gifts.length) {
        historyEl.innerHTML = '<div style="color:var(--text-muted);font-size:13px">Henüz gönderdiðiniz hediye yok.</div>';
        return;
      }
      historyEl.innerHTML = gifts.map(g => `
        <div style="padding:10px;border:1px solid var(--border);border-radius:10px;margin-bottom:10px;background:var(--bg-card2)">
          ...
          <div style="font-size:13px;color:var(--text-muted);margin:6px 0">Alýcý: ${escHtml(g.recipient_username || 'Belirsiz')} · Kod: ${escHtml(g.code)}</div>
          <div style="font-size:13px;color:var(--text-muted)">${escHtml(new Date(g.created_at).toLocaleString('tr-TR'))} · ${g.redeemed ? 'Kullanýldý' : 'Beklemede'}</div>
        </div>
      `).join('');
    } catch (e) {
...
function updateNavUI() {
  ...
  if (brand) {
    brand.removeEventListener('click', brandProfileClick);
    brand.addEventListener('click', brandProfileClick);
  }
  ...
  if (currentUser) {
    ...
    if (mobUserLinks) mobUserLinks.innerHTML = `
      ...
      <a href="/arkadaslar" data-link class="mobile-nav-link"><i class="fas fa-user-friends" style="width:18px"></i> Arkadaþlar</a>
      <a href="/ayarlar" data-link class="mobile-nav-link"><i class="fas fa-cog" style="width:18px"></i> Ayarlar</a>
      <button class="mobile-nav-link" id="mob-logout" style="background:none;border:none;width:100%;text-align:left;color:var(--accent-red2)"><i class="fas fa-sign-out-alt" style="width:18px"></i> Çýkýþ Yap</button>
    `;
    $('#mob-logout')?.addEventListener('click', async () => {
      try { await api('/auth/logout', { method: 'POST' }); } catch { }
      currentToken = null; currentUser = null;
      localStorage.removeItem('token');
      updateNavUI(); navigate('/'); toast('Çýkýþ yapýldý');
    ...
    if (mbbAuth) {
      ...
      const lbl = $('#mbb-auth-label'); if (lbl) lbl.textContent = 'Giriþ';
...
async function renderHome(app) {
  ...
  updatePageMeta('TeaTube – Topluluk Platformu', 'TeaTube her tür içeriðin paylaþýldýðý fotoðraf, yazý ve müzik platformu.', '');
  const isMobile = window.innerWidth <= 900;
  const homeTitle = isMobile ? 'Öne Çýkan Fotoðraflar' : 'Popüler Konular';
  const homeSubtitle = isMobile ? 'Topluluðun en yeni paylaþýmlarý' : 'Topluluðun en çok konuþulan baþlýklarý';
  const homeAction = isMobile
    ? currentUser ? `<button class="btn btn-primary btn-sm" id="home-new-photo-btn"><i class="fas fa-upload"></i> Fotoðraf Yükle</button>` : `<a href="/giris" data-link class="btn btn-primary btn-sm">Giriþ Yap</a>`
    : currentUser ? `<button class="btn btn-primary btn-sm" id="home-new-forum-btn"><i class="fas fa-plus"></i> Yeni Konu Aç</button>` : `<a href="/giris" data-link class="btn btn-primary btn-sm">Giriþ Yap</a>`;
  ...
  app.innerHTML = `
    <div class="container page">
      <div class="home-tabs-row">
        <a href="/fotograflar" data-link class="home-tab${photosTabActive}" id="photos-tab">Fotoðraflar</a>
  ...
  async function loadHomePhotos() {
    ...
    try {
      ...
      if (!photos.length) {
        contentEl.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-camera-retro"></i><p>Henüz fotoðraf yok.</p></div>';
        return;
      }
  ...
  async function loadHomeForums() {
    ...
    $('#home-forum-search')?.addEventListener('input', e => {
      ...
      const el = $('#home-forums');
      if (!el) return;
      if (!filtered.length) { el.innerHTML = '<div class="empty-state"><i class="fas fa-comments"></i><p>Konu bulunamadý.</p></div>'; return; }
    ...
    try {
      ...
      if (!friends.length) { el.innerHTML = '<div class="empty-state"><i class="fas fa-user-friends"></i><p>Henüz arkadaþ yok.</p></div>'; }
  ...
  async function renderForumList(app, queryString) {
    ...
    updatePageMeta('Konular – TeaTube', 'Toplulukla fikir paylaþ, tartýþ, keþfet.', '');
    ...
    <div class="container page">
      <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
        <div>
          ...
          <div class="page-subtitle">${activeTag ? `<i class="fas fa-hashtag" style="color:var(--accent-red2)"></i> <strong>${escHtml(activeTag)}</strong> etiketiyle filtreli &nbsp;<a href="/forum" data-link style="font-size:12px;color:var(--accent-red2)"><i class="fas fa-times"></i> Temizle</a>` : 'Toplulukla fikir paylaþ'}</div>
        </div>
  ...
  async function renderPhotoDetail(app, id, ownerUsername) {
    try {
      ...
      document.title = (photo.caption ? photo.caption + ' – ' : '') + 'Fotoðraf – TeaTube';
      updatePageMeta(photo.caption || 'Fotoðraf', photo.caption || '', photo.url || '');
      app.innerHTML = `
      <div class="container page">
        <div class="section">
          <div class="photo-detail">
            ...
            <div class="photo-detail-body">
              ...
              <div class="meta-row">Paylaþan: <a href="/profil/${escHtml(photo.username || ownerUsername || '')}" data-link>${escHtml(photo.username || ownerUsername || '')}</a></div>
      ...
      toast('Fotoðraf bulunamadý', 'error'); navigate('/fotograflar');
    }
  }
  ...
  function renderForumListItems(forums) {
    ...
    if (!el) return;
    if (!forums.length) { el.innerHTML = '<div class="empty-state"><i class="fas fa-comments"></i><p>Konu bulunamadý.</p></div>'; return; }
  ...
  function forumCardHTML(f) {
    ...
    const authorName = f.username || 'Silinmiþ Kullanýcý';
    ...
    const extraImgs = Array.isArray(f.system_tags) ? [] : []; // system_tags zaten ayrý parse ediliyor
  ...
  // Hashtag týklanýnca o etikete göre filtrele
  ...
  function photoCardHTML(p) {
    ...
    <div class="photo-card">
      <div class="photo-card-media" onclick="navigate('/profil/${escHtml(p.username)}/foto/${escHtml(p.id)}')" style="cursor:pointer">
        <img src="${escHtml(p.url)}" alt="${escHtml(p.caption || 'Fotoðraf')}" />
      ...
      <div class="photo-card-body">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;gap:8px">
          ...
          <div style="display:flex;align-items:center;gap:8px">
            <button class="btn btn-ghost btn-sm photo-like-btn" data-id="${escHtml(p.id)}" title="Beðen">${p.liked ? '<i class="fas fa-heart" style="color:var(--accent-red2)"></i>' : '<i class="far fa-heart"></i>'} <span class="photo-like-count">${p.like_count || 0}</span></button>
  ...
  async function editPhoto(photoId) {
    try {
      ...
      if (!currentUser || photo.username !== currentUser.username) return toast('Bu fotoðrafý düzenleyemezsiniz', 'error');
      showModal('Fotoðraf Düzenle', `
      <div class="form-group"><label>Fotoðraf URL</label><input id="edit-photo-url" type="text" value="${escHtml(photo.url)}" /></div>
      <div class="form-group"><label>Baþlýk / Açýklama</label><textarea id="edit-photo-caption" rows="4">${escHtml(photo.caption || '')}</textarea></div>
      <div class="form-group"><label>Yeni Fotoðraf (opsiyonel)</label><input type="file" id="edit-photo-file" accept="image/*" /></div>
      <div class="form-group"><label class="checkbox-label"><input type="checkbox" id="edit-show-likes" ${photo.show_likes == 1 ? 'checked' : ''} /> Beðeni sayýsýný göster</label></div>
      ...
      <div class="form-group"><label class="checkbox-label"><input type="checkbox" id="edit-allow-shares" ${photo.allow_shares == 1 ? 'checked' : ''} /> Ýletilere izin ver</label></div>
    ...
    `);
      ...
      $('#edit-photo-save')?.addEventListener('click', async () => {
        ...
        if (!url) { $('#edit-photo-error').textContent = 'Fotoðraf URL gerekli'; return; }
        let finalUrl = url;
  ...
  async function showPhotoComments(photoId) {
    try {
      ...
      <div style="display:flex;flex-direction:column;gap:12px">
        ...
        <button class="btn btn-primary" id="photo-comment-send">Gönder</button>` : `<a href="/giris" data-link class="btn btn-primary">Giriþ Yap</a>`}
      </div>
    `);
      ...
      $('#modal-body')?.addEventListener('click', async (e) => {
        ...
        if (del) {
          ...
          try {
            await api(`/photos/comments/${encodeURIComponent(del.dataset.id)}`, { method: 'DELETE' });
  ...
  async function deletePhoto(photoId) {
    if (!confirm('Bu fotoðrafý silmek istediðine emin misin?')) return;
    try {
      await api('/photos/' + encodeURIComponent(photoId), { method: 'DELETE' });
      toast('Fotoðraf silindi');
      renderRoute(location.pathname);
    } catch (e) {
      toast(e.message, 'error');
    }
  ...
  function showNewForumModal(existing = null) {
    ...
    <div class="form-group"><label>Baþlýk</label><input id="fm-title" type="text" placeholder="Konu baþlýðý" value="${existing ? escHtml(existing.title) : ''}" /></div>
    <div class="form-group"><label>Ýçerik</label><textarea id="fm-content" rows="8" placeholder="Yazýnýzý buraya girin...">${existing ? escHtml(existing.content) : ''}</textarea></div>
    <div class="form-group">
      ...
      <div style="margin-top:8px"><small style="color:var(--text-muted)">veya virgülle ayýrarak kendiniz ekleyin:</small></div>
      <input type="text" id="fm-custom-tags" placeholder="Örn: bilim, siyaset, teknoloji" style="margin-top:4px" />
    </div>
    <div class="form-group">
      <label>Kart Küçük Resmi <span style="font-size:11px;font-weight:400;color:var(--text-muted)">(opsiyonel — boþ býrakýrsan 1. ek resim ya da banner kullanýlýr)</span></label>
    ...
    <div class="form-group">
      ...
      <div style="margin-top:8px">
        <label style="font-size:11px;font-weight:600;color:var(--text-muted);display:block;margin-bottom:4px">FOTOÐRAF GÖRÜNÜMÜ</label>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          ...
          <label style="display:flex;align-items:center;gap:4px;font-size:12px;cursor:pointer;padding:4px 10px;border:1px solid var(--border);border-radius:6px">
            <input type="radio" name="fm-fit" value="contain" ${existing && existing.banner_fit === 'contain' ? 'checked' : ''} style="width:auto" /> Sýðdýr (Tam Görünsün)
          </label>
          <label style="display:flex;align-items:center;gap:4px;font-size:12px;cursor:pointer;padding:4px 10px;border:1px solid var(--border);border-radius:6px">
    ...
    <div class="form-group"><label class="checkbox-label"><input type="checkbox" id="fm-likes" ${!existing || existing.allow_likes ? 'checked' : ''} /> Beðenilere izin ver</label></div>
    <div class="form-group">
    ...
    <button class="btn btn-primary" id="fm-submit" style="width:100%">${existing ? 'Güncelle' : 'Yayýnla'}</button>
  ...
  `);
    ...
    // Enter tuþu ile input'lardan form submit tetiklenmesini önle
    ...
    $('#fm-submit').addEventListener('click', async () => {
      ...
      if (!title || !content) { $('#fm-error').textContent = 'Baþlýk ve içerik zorunlu'; return; }
      ...
      try {
          ...
          banner_image = await new Promise((resolve, reject) => {
            ...
            xhr.addEventListener('load', () => {
              ...
              try {
                ...
                if (xhr.status >= 400) return reject(new Error(data.error || 'Yükleme hatasý'));
                resolve(data.url);
              } catch (e) {
                reject(new Error('Sunucu yanýtý geçersiz: ' + xhr.responseText.substring(0, 100)));
              }
            });
            xhr.addEventListener('error', () => reject(new Error('Að hatasý, tekrar deneyin')));
        ...
        if (existing) {
          await api('/forum/' + existing.slug, { method: 'PUT', body: JSON.stringify({ title, content, banner_image, allow_comments: $('#fm-comments').checked, allow_likes: $('#fm-likes').checked, tagIds, customTags, banner_fit: document.querySelector('[name="fm-fit"]:checked')?.value || 'cover', images: uploadedExtraImages, thumbnail: thumbnailUrl }) });
          toast('Konu güncellendi');
        } else {
          const f = await api('/forums', { method: 'POST', body: JSON.stringify({ title, content, banner_image, allow_comments: $('#fm-comments').checked, allow_likes: $('#fm-likes').checked, tagIds, customTags, banner_fit: document.querySelector('[name="fm-fit"]:checked')?.value || 'cover', images: uploadedExtraImages, thumbnail: thumbnailUrl }) });
          toast('Konu oluþturuldu');
          hideModal();
        ...
        const submitBtn = $('#fm-submit');
        if (submitBtn) { submitBtn.disabled = false; submitBtn._submitting = false; submitBtn.innerHTML = existing ? 'Güncelle' : 'Yayýnla'; }
  ...
  async function renderForumDetail(app, slug) {
    ...
    } catch { app.innerHTML = '<div class="container page"><div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>Konu bulunamadý.</p></div></div>'; return; }
    ...
    <div class="forum-detail">
      ...
      <div class="forum-actions">
        ${forum.allow_likes ? `<button class="forum-action-btn ${liked ? 'liked' : ''}" id="like-btn">
          <i class="fas fa-heart"></i> <span id="like-count">${forum.like_count || 0}</span> Beðeni
        </button>` : `<button class="forum-action-btn disabled" disabled><i class="fas fa-heart-broken"></i> Beðeniler kapalý</button>`}
        <button class="forum-action-btn" id="share-btn"><i class="fas fa-share-alt"></i> Paylaþ</button>
        ${currentUser ? `<button class="forum-action-btn" id="forward-forum-btn"><i class="fas fa-paper-plane"></i> Ýlet</button>` : ''}
      </div>
      <hr class="divider" />
      <div class="comments-section">
        ...
        ${currentUser && forum.allow_comments ? `
          <div class="comment-form">
            ...
            <textarea id="comment-input" placeholder="Yorumunuzu yazýn..."></textarea>
            <button class="btn btn-primary btn-sm" id="comment-submit"><i class="fas fa-paper-plane"></i></button>
          </div>` : (!currentUser && forum.allow_comments ? `<p class="text-secondary" style="margin-bottom:16px">Yorum yapmak için <a href="/giris" data-link class="auth-link">giriþ yapýn</a>.</p>` : (!forum.allow_comments ? `<p class="text-muted" style="margin-bottom:16px">Yorumlar kapatýlmýþ.</p>` : ''))}
    ...
    if (isOwner) {
      ...
      $('#del-forum-btn').addEventListener('click', async () => {
        if (!confirm('Konuyu silmek istediðinize emin misiniz?')) return;
    ...
    $('#share-btn').addEventListener('click', () => {
      ...
      if (navigator.clipboard) { navigator.clipboard.writeText(url); toast('Link kopyalandý!'); }
      else { window.prompt('Linki kopyalayýn:', url); }
    });
  ...
  async function renderBookList(app) {
    ...
    updatePageMeta('Kitaplar – TeaTube', 'Topluluðun yazdýðý eserleri keþfet.', '');
    app.innerHTML = `
    <div class="container page">
      <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
        <div><div class="page-title">Kitaplar</div><div class="page-subtitle">Topluluðun eserleri</div></div>
        ${currentUser ? `<button class="btn btn-primary" id="new-book-btn"><i class="fas fa-plus"></i> Yeni Kitap</button>` : ''}
      </div>
      <div class="search-bar"><i class="fas fa-search"></i><input type="text" id="book-search" placeholder="Kitap adý, yazar veya konu ara..." /></div>
    ...
    $('#book-search')?.addEventListener('input', async e => {
      const q = e.target.value.trim().toLowerCase();
      if (!q) { renderBookGrid(books); return; }
      renderBookGrid(books.filter(b =>
        b.title.toLowerCase().includes(q) ||
        b.username?.toLowerCase().includes(q) ||
        b.topic?.toLowerCase().includes(q) ||
        b.author_nickname?.toLowerCase().includes(q)
      ));
    });
  }
  ...
  function renderBookGrid(books) {
    ...
    if (!books.length) { el.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-book-open"></i><p>Kitap bulunamadý.</p></div>'; return; }
  ...
  function bookCardHTML(b) {
    ...
    <div class="book-info">
      ...
      ${b.topic ? `<div class="book-topic">${escHtml(b.topic)}</div>` : ''}
      <div class="book-author"><i class="fas fa-user" style="color:var(--accent-red);font-size:11px"></i> ${escHtml(b.username || 'Bilinmiyor')}${b.author_nickname ? ` <span class="book-author-nick">(${escHtml(b.author_nickname)})</span>` : ''}</div>
  ...
  function showNewBookModal(existing = null) {
    showModal(existing ? 'Kitabý Düzenle' : 'Yeni Kitap', `
    <div class="form-group"><label>Baþlýk</label><input id="bk-title" type="text" value="${existing ? escHtml(existing.title) : ''}" /></div>
    <div class="form-group"><label>Konu / Tür</label><input id="bk-topic" type="text" value="${existing ? escHtml(existing.topic || '') : ''}" placeholder="Örn. Fantastik, Kiþisel Geliþim" /></div>
    ...
    <button class="btn btn-primary" id="bk-submit" style="width:100%">${existing ? 'Güncelle' : 'Oluþtur'}</button>
  ...
  `);
    ...
    $('#bk-submit').addEventListener('click', async () => {
      ...
      if (!title) { $('#bk-error').textContent = 'Baþlýk zorunlu'; return; }
      try {
        ...
        const topic = $('#bk-topic').value.trim();
        if (existing) {
          await api('/book/' + existing.slug, { method: 'PUT', body: JSON.stringify({ title, topic, preface: $('#bk-preface').value.trim(), cover_image }) });
          toast('Kitap güncellendi'); hideModal(); renderRoute(location.pathname);
        } else {
          const b = await api('/books', { method: 'POST', body: JSON.stringify({ title, topic, preface: $('#bk-preface').value.trim(), cover_image }) });
          toast('Kitap oluþturuldu'); hideModal(); navigate('/kitap/' + b.slug);
        }
  ...
  async function renderBookDetail(app, slug) {
    app.innerHTML = `<div class="container page"><div class="loading-center"><div class="spinner"></div></div></div>`;
    let data;
    try { data = await api('/book/' + slug); } catch { app.innerHTML = '<div class="container page"><div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>Kitap bulunamadý.</p></div></div>'; return; }

    const { book, chapters, pages } = data;
    document.title = book.title + ' – TeaTube';
    updatePageMeta(book.title + ' – TeaTube', book.preface ? book.preface.substring(0, 155) : book.title + ' – TeaTube\'da yayýnlanan kitap.', book.cover_image || '');
    ...
    <div class="book-detail-header">
      ...
      <div class="book-detail-info">
        ...
        ${book.topic ? `<div class="book-topic" style="margin-top:6px">${escHtml(book.topic)}</div>` : ''}
    ...
    <div class="book-pages-list">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:14px">
        ...
        <div>
          <button class="btn btn-primary" id="book-read-btn"><i class="fas fa-book-open"></i> ${localStorage.getItem('book_last_' + book.slug) ? 'Devam Et' : 'Oku'}</button>
        </div>
      </div>
      ${localStorage.getItem('book_last_' + book.slug) ? `<div style="margin-top:8px;font-size:13px;color:var(--text-muted)">Son okunan sayfa: ${escHtml(localStorage.getItem('book_last_' + book.slug))}</div>` : ''}
    ...
    if (isOwner) {
      ...
      $('#del-book-btn').addEventListener('click', async () => {
        if (!confirm('Kitabý ve tüm sayfalarýný silmek istediðinize emin misiniz?')) return;
      ...
      document.querySelectorAll('.del-chapter').forEach(btn => {
        btn.addEventListener('click', async e => {
          e.stopPropagation();
          if (!confirm('Bölümü silmek istediðinize emin misiniz?')) return;
  ...
  async function showAddPageModal(bookSlug, chapters) {
    ...
    <div class="form-group"><label>Sayfa Baþlýðý</label><input id="pg-title" type="text" placeholder="Sayfa baþlýðý..." /></div>
    ...
    <div class="form-group"><label>Ýçerik</label><textarea id="pg-content" rows="14" placeholder="Sayfanýn içeriðini buraya yazýn..."></textarea></div>
  ...
  `);
    ...
    $('#pg-submit').addEventListener('click', async () => {
      ...
      if (!title || !content) { $('#pg-error').textContent = 'Baþlýk ve içerik zorunlu'; return; }
  ...
  function showAddChapterModal(bookSlug) {
    ...
    <div class="form-group"><label>Bölüm Adý</label><input id="ch-title" type="text" /></div>
    <div class="form-group"><label>Sýra</label><input id="ch-order" type="number" value="0" /></div>
  ...
  `);
    $('#ch-submit').addEventListener('click', async () => {
      ...
      if (!title) { $('#ch-error').textContent = 'Baþlýk zorunlu'; return; }
      try {
  ...
  async function renderPageReader(app, bookSlug, pageSlug) {
    app.innerHTML = `<div class="container page"><div class="loading-center"><div class="spinner"></div></div></div>`;
    let data;
    try { data = await api(`/book/${bookSlug}/page/${pageSlug}`); } catch { app.innerHTML = '<div class="container page"><div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>Sayfa bulunamadý.</p></div></div>'; return; }

    const { page, book, prev, next } = data;
    document.title = page.title + ' - ' + book.title;
    updatePageMeta(page.title + ' - ' + book.title, page.content.substring(0, 155).replace(/\n/g, ' '), book.cover_image || '');
    const isOwner = currentUser && currentUser.id === book.user_id;

    // Kitabýn tüm sayfalarýný al (içindekiler için)
    let allPages = [];
    ...
    <div class="ebook-layout">
      ...
      <div class="ebook-toc" id="ebook-toc" style="display:none">
        <div style="padding:12px 16px;border-bottom:1px solid rgba(220,38,38,0.15);font-size:13px;font-weight:600;color:var(--text-secondary)">
          <i class="fas fa-list" style="color:var(--accent-red)"></i> Ýçindekiler
        </div>
      ...
      <div class="ebook-reader" style="flex:1">
        ...
        <div class="ebook-toolbar">
          <button class="btn btn-ghost btn-sm" id="toc-toggle" title="Ýçindekiler">
            <i class="fas fa-list"></i> <span class="hidden" id="toc-label">Ýçindekiler</span>
          </button>
    ...
    if (isOwner) {
      $('#edit-page-btn').addEventListener('click', () => {
        showModal('Sayfayý Düzenle', `
        <div class="form-group"><label>Baþlýk</label><input id="ep-title" type="text" value="${escHtml(page.title)}" /></div>
        <div class="form-group"><label>Ýçerik</label><textarea id="ep-content" rows="14">${escHtml(page.content)}</textarea></div>
  ...
  async function renderGroupList(app) {
    ...
    <div class="container page">
      <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
        <div><div class="page-title">Gruplar</div><div class="page-subtitle">Topluluða katýl</div></div>
      ...
      <div id="join-invite-section" style="margin-bottom:16px">
        ${currentUser ? `<div style="display:flex;gap:8px;max-width:400px">
          <input id="invite-code-input" type="text" placeholder="Davet kodu ile katýl..." />
          <button class="btn btn-outline" id="join-invite-btn">Katýl</button>
        </div>` : ''}
      </div>
      <div class="search-bar" style="margin-bottom:24px"><i class="fas fa-search"></i><input type="text" id="group-search" placeholder="Grup ara (isim veya açýklama)..." /></div>
    ...
    if (currentUser) {
      ...
      $('#join-invite-btn')?.addEventListener('click', async () => {
        ...
        if (!code) return;
        try { await api('/group/join-invite', { method: 'POST', body: JSON.stringify({ invite_code: code }) }); toast('Gruba katýldýnýz!'); renderRoute(location.pathname); } catch (e) { toast(e.message, 'error'); }
      });
    }
    ...
    function renderGroups(list) {
      ...
      if (!el) return;
      if (!list.length) { el.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-users"></i><p>Grup bulunamadý.</p></div>'; return; }
  ...
  async function renderPhotos(app) {
    document.title = 'Fotoðraflar - TeaTube';
    app.innerHTML = `<div class="container page">
    <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
      <div><div class="page-title">Fotoðraflar</div><div class="page-subtitle">Topluluk tarafýndan paylaþýlan fotoðraflar</div></div>
      ${currentUser ? `<button class="btn btn-primary" id="photo-upload-btn"><i class="fas fa-upload"></i> Fotoðraf Yükle</button>` : `<a href="/giris" data-link class="btn btn-primary btn-sm">Giriþ Yap</a>`}
    </div>
    ...
    async function loadPhotos() {
      ...
      try {
        const photos = await api('/photos');
        if (!photos.length) {
          grid.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-camera-retro"></i><p>Henüz fotoðraf yok.</p></div>';
          return;
        }
  ...
  function groupCardHTML(g) {
    const typeBadge = g.type === 'private' ? `<span class="badge badge-red"><i class="fas fa-lock"></i> Özel</span>` : `<span class="badge badge-green"><i class="fas fa-globe"></i> Açýk</span>`;
  ...
  function showNewGroupModal() {
    showModal('Yeni Grup', `
    <div class="form-group"><label>Grup Adý</label><input id="gr-name" type="text" /></div>
    <div class="form-group"><label>Açýklama</label><textarea id="gr-desc" rows="3"></textarea></div>
    <div class="form-group">
      <label>Kapak Resmi (opsiyonel)</label>
      <input type="file" id="gr-cover-file" accept="image/*" style="margin-bottom:8px" />
      <div id="gr-cover-preview" style="display:none"></div>
    </div>
    <div class="form-group"><label>Tür</label><select id="gr-type"><option value="public">Açýk</option><option value="private">Özel</option></select></div>
    <div class="form-group">
      <label class="checkbox-label"><input type="checkbox" id="gr-chat" checked /> Sohbete izin ver</label>
      <label class="checkbox-label" style="margin-top:8px"><input type="checkbox" id="gr-photos" checked /> Fotoðrafa izin ver</label>
      <label class="checkbox-label" style="margin-top:8px"><input type="checkbox" id="gr-invite" /> Sadece davet ile katýlým</label>
    </div>
    <button class="btn btn-primary" id="gr-submit" style="width:100%">Oluþtur</button>
  ...
  `);
    ...
    $('#gr-submit').addEventListener('click', async () => {
      ...
      if (!name) { $('#gr-error').textContent = 'Ýsim zorunlu'; return; }
      try {
        let cover_image = '';
        ...
        toast('Grup oluþturuldu'); hideModal(); navigate('/grup/' + g.slug);
  ...
  async function renderGroupDetail(app, slug) {
    ...
    } catch { app.innerHTML = '<div class="container page"><div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>Grup bulunamadý.</p></div></div>'; return; }
    ...
    <div style="margin-bottom:20px">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          ${!isMember && currentUser && group.type === 'public' && !group.invite_only ? `<button class="btn btn-primary" id="join-btn"><i class="fas fa-plus"></i> Katýl</button>` : ''}
          ${isMember && !isOwner ? `<button class="btn btn-outline" id="leave-btn"><i class="fas fa-sign-out-alt"></i> Ayrýl</button>` : ''}
        ...
        ${group.allow_chat ? `
          <div class="chat-container">
            ...
            ${canSend ? `<div class="chat-input-bar">
              ${group.allow_photos ? `<label class="btn btn-ghost btn-sm" for="chat-img-input" title="Fotoðraf gönder" style="flex-shrink:0"><i class="fas fa-image"></i></label><input id="chat-img-input" type="file" accept="image/*" style="display:none" />` : ''}
            ...
            </div>` : (currentUser && !isMember ? `<div style="padding:12px;text-align:center;color:var(--text-muted);font-size:13px">Mesaj göndermek için gruba katýlýn.</div>` : `<div style="padding:12px;text-align:center;color:var(--text-muted);font-size:13px">Giriþ yaparak katýlabilirsiniz.</div>`)}
          </div>` : `<div class="card card-body" style="text-align:center;color:var(--text-muted)"><i class="fas fa-comment-slash" style="font-size:32px;margin-bottom:8px;display:block"></i>Sohbet kapatýlmýþ.</div>`}
      </div>
      <div>
        <div class="group-sidebar-card">
          ...
          <div class="card-body" style="font-size:13px;color:var(--text-secondary)">
            ...
            <div style="margin-bottom:6px">${group.type === 'private' ? '<span class="badge badge-red"><i class="fas fa-lock"></i> Özel</span>' : '<span class="badge badge-green"><i class="fas fa-globe"></i> Açýk</span>'}</div>
    ...
    $('#join-btn')?.addEventListener('click', async () => {
      try { await api('/group/' + slug + '/join', { method: 'POST' }); toast('Gruba katýldýnýz!'); renderRoute(location.pathname); } catch (e) { toast(e.message, 'error'); }
    });
    $('#leave-btn')?.addEventListener('click', async () => {
      if (!confirm('Gruptan ayrýlmak istiyor musunuz?')) return;
      try { await api('/group/' + slug + '/leave', { method: 'POST' }); toast('Gruptan ayrýldýnýz.'); renderRoute(location.pathname); } catch (e) { toast(e.message, 'error'); }
    });
    ...
    $('#gen-invite-btn')?.addEventListener('click', async () => {
      try {
        ...
        <button class="btn btn-primary" onclick="navigator.clipboard && navigator.clipboard.writeText('${r.invite_code}'); toast('Kopyalandý!')">Kopyala</button>
      </div>`);
      } catch (e) { toast(e.message, 'error'); }
    });

    $('#group-settings-btn')?.addEventListener('click', () => {
      showModal('Grup Ayarlarý', `
      <div class="form-group"><label>Grup Adý</label><input id="gs-name" type="text" value="${escHtml(group.name)}" /></div>
      <div class="form-group"><label>Açýklama</label><textarea id="gs-desc" rows="3">${escHtml(group.description || '')}</textarea></div>
      <div class="form-group">
      ...
      <div class="form-group"><label>Tür</label><select id="gs-type"><option value="public" ${group.type === 'public' ? 'selected' : ''}>Açýk</option><option value="private" ${group.type === 'private' ? 'selected' : ''}>Özel</option></select></div>
      <div class="form-group">
        <label class="checkbox-label"><input type="checkbox" id="gs-chat" ${group.allow_chat ? 'checked' : ''} /> Sohbet</label>
        <label class="checkbox-label" style="margin-top:8px"><input type="checkbox" id="gs-photos" ${group.allow_photos ? 'checked' : ''} /> Fotoðraf</label>
      </div>
    ...
    `);
      ...
      $('#gs-delete').addEventListener('click', async () => {
        if (!confirm('Grubu silmek istediðinize emin misiniz?')) return;
    ...
    $('#members-list')?.addEventListener('click', async e => {
      ...
      if (banBtn && isOwner) {
        ...
        try { await api(`/group/${slug}/ban/${uid}`, { method: 'POST' }); toast('Üye atýldý'); renderRoute(location.pathname); } catch (e) { toast(e.message, 'error'); }
      }
      if (modBtn && isOwner) {
        const uid = modBtn.dataset.uid;
        try { await api(`/group/${slug}/moderator/${uid}`, { method: 'POST' }); toast('Moderatör yapýldý'); renderRoute(location.pathname); } catch (e) { toast(e.message, 'error'); }
      }
    });
  }
  ...
  async function renderProfile(app, username) {
    app.innerHTML = `<div class="container page"><div class="loading-center"><div class="spinner"></div></div></div>`;
    let data;
    try { data = await api('/profile/' + username); } catch { app.innerHTML = '<div class="container page"><div class="empty-state"><i class="fas fa-user-slash"></i><p>Kullanýcý bulunamadý.</p></div></div>'; return; }

    const { user, forums, books, groups, level, levels, book_page_count } = data;
    document.title = (user.nickname ? user.nickname + ' (' + user.username + ')' : user.username) + ' - TeaTube';
    ...
    const profilePhotosHTML = profilePhotos.length
      ...
      : '<div class="empty-state"><i class="fas fa-camera-retro"></i><p>Henüz fotoðraf yok.</p></div>';
    ...
    const profileFriendsHTML = profileFriends.length
      ...
      : '<div class="empty-state"><i class="fas fa-user-friends"></i><p>Arkadaþ yok.</p></div>';
    ...
    if (nextLevel) {
      ...
      if (nbp !== null && nbp > 0) { const left = Math.max(0, nbp - (book_page_count || 0)); if (left > 0) remaining.push(`${left} kitap sayfasý`); }
      ...
      if (remaining.length > 0) {
        if (reqAny) {
          ...
          <strong>Þunlardan birini tamamlayarak seviye atlayabilirsin:</strong>
          <ul style="margin:6px 0 0 16px;list-style:disc">
            ${remaining.map(r => `<li style="margin:2px 0">${r}</li>`).join('')}
          </ul>
        </div>`;
        } else {
          hint = `<div style="font-size:11px;color:var(--text-muted);margin-top:4px">${remaining.join(', ')} kaldý</div>`;
        }
      }

      progressHTML = `<div style="margin-top:12px">
      <div style="font-size:12px;color:var(--text-muted);margin-bottom:4px">
        ${escHtml(nextLevel.name)} seviyesine ${overallPct}% tamamlandý
      </div>
    ...
    <div class="profile-header">
      ...
      <div class="profile-info">
        ...
        ${user.nickname ? `<div class="profile-nickname">${escHtml(user.nickname)}</div>` : ''}
    ...
    <div class="section profile-section">
      <div class="section-header">
        <div class="section-title"><div class="section-title-bar"></div>Fotoðraflar</div>
      </div>
      ${profilePhotosHTML}
    </div>
    ...
    <div class="tabs">
      ...
      <button class="tab" data-tab="friends">Arkadaþlar</button>
  ...
  async function renderSettings(app) {
    ...
    <div class="settings-layout">
      <div class="settings-nav">
        ...
        <div class="settings-nav-item" data-section="password"><i class="fas fa-lock"></i> Þifre</div>
    ...
    if (urlParams.get('spotify') === 'ok') { toast('Spotify baðlandý! ??'); history.replaceState({}, '', '/ayarlar'); }
    if (urlParams.get('spotify') === 'error') { toast('Spotify baðlantýsý baþarýsýz', 'error'); history.replaceState({}, '', '/ayarlar'); }
    ...
    if (section === 'profile') {
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="form-group"><label>Ünvan <span style="color:var(--accent-red2)">*</span></label><input type="text" id="s-title" placeholder="Örn: Yazar, Öðrenci, Mühendis..." value="${escHtml(currentUser.title || '')}" /></div>
          <div class="form-group"><label>Konum (opsiyonel)</label><input type="text" id="s-location" placeholder="Örn: Ýstanbul, Türkiye" value="${escHtml(currentUser.location || '')}" /></div>
          <div class="form-row">
            <div class="form-group"><label>Ünvan <span style="color:var(--accent-red2)">*</span></label><input type="text" id="s-title" value="${escHtml(currentUser.title || '')}" placeholder="Örn: Yazýlým Geliþtirici, Öðrenci..." /></div>
            <div class="form-group"><label>Konum <span style="color:var(--text-muted);font-size:11px">(opsiyonel)</span></label><input type="text" id="s-location" value="${escHtml(currentUser.location || '')}" placeholder="Örn: Ýstanbul, Türkiye" /></div>
          </div>
          <div class="form-group">
            <label>Linkler</label>
      ...
      function renderLinkRows(linksArr) {
        ...
        <div class="link-row" data-idx="${i}" style="display:flex;gap:8px;align-items:center">
          <input type="text" placeholder="Baþlýk (örn: GitHub)" value="${escHtml(l.label || '')}" data-field="label" style="flex:1" />
      ...
      el.innerHTML = `
      <div class="card">
        <div class="card-header"><span>Þifre Deðiþtir</span></div>
        <div class="card-body">
          <div class="form-group"><label>Eski Þifre</label><input type="password" id="old-pw" /></div>
          <div class="form-group"><label>Yeni Þifre</label><input type="password" id="new-pw" /></div>
          <div class="form-group"><label>Yeni Þifre (Tekrar)</label><input type="password" id="new-pw2" /></div>
          <button class="btn btn-primary" id="save-pw-btn">Deðiþtir</button>
      ...
      $('#save-pw-btn').addEventListener('click', async () => {
        ...
        if (new_password !== $('#new-pw2').value) { $('#pw-msg').textContent = 'Þifreler uyuþmuyor'; return; }
        try {
          await api('/profile/password', { method: 'PUT', body: JSON.stringify({ old_password, new_password }) });
          toast('Þifre deðiþtirildi'); $('#old-pw').value = ''; $('#new-pw').value = ''; $('#new-pw2').value = '';
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="form-group"><label class="checkbox-label"><input type="checkbox" id="s-show-color" ${currentUser.show_level_color ? 'checked' : ''} /> Ýsim rengini göster</label></div>
          <div class="form-group"><label>Ýsim Rengi</label>
          ...
          ${(currentUser.is_vip || currentUser.is_plus) ? `<div id="custom-badge-controls" style="display:${currentUser.badge_display === 'custom' ? 'block' : 'none'}">
            <div class="form-group"><label>Rozet Adý</label><input type="text" id="s-badge-name" value="${escHtml(currentUser.badge_name || '')}" placeholder="Örn: VIP, Plus, Sanatçý..." /></div>
            <div class="form-group"><label>Rozet Ýkonu</label><input type="text" id="s-badge-icon" value="${escHtml(currentUser.badge_icon || 'fas fa-award')}" placeholder="fas fa-award veya ?" /></div>
      ...
      // VIP satýn alma butonu: non-VIP kullanýcýlarý VIP sayfasýna yönlendir
      $('#buy-vip-btn')?.addEventListener('click', () => {
        showModal('VIP Ol', `
        <div style="padding:12px">
          <p style="margin-bottom:12px">Ýsim rengini deðiþtirmek için VIP olmanýz gerekiyor. VIP ile profil renkleri, özel rozet ve daha fazlasýna eriþim saðlarsýnýz.</p>
          <div style="display:flex;gap:8px">
            <button class="btn btn-primary" id="vip-go">VIP Sayfasýna Git</button>
      ...
      el.innerHTML = `
      <div class="card">
        <div class="card-header"><span><i class="fas fa-bell" style="color:var(--accent-red2);margin-right:6px"></i>Bildirim Ayarlarý</span></div>
        <div class="card-body">
          <div class="form-group">
            <label class="checkbox-label" style="align-items:flex-start;gap:12px">
              ...
              <div>
                <div style="font-weight:600;font-size:14px">Beni etiketleyen kiþilere bildirim gönder</div>
                <div style="font-size:12px;color:var(--text-muted);margin-top:3px">
                  Kapatýrsan kimse seni @etiketleyemez ve bildirim almaz, profil linki de açýlmaz
                </div>
              </div>
            </label>
          </div>
      ...
      $('#save-notif-btn').addEventListener('click', async () => {
        ...
        try {
          const updated = await apiForm('/profile', fd, 'PUT');
          currentUser = updated; updateNavUI();
          toast('Bildirim ayarlarý kaydedildi');
        ...
        <div class="card-body">
          ${hasSpotify ? `
            <div style="display:flex;align-items:center;gap:10px;padding:12px;background:rgba(30,215,96,0.08);border:1px solid rgba(30,215,96,0.2);border-radius:8px;margin-bottom:16px">
              ...
              <div>
                <div style="font-weight:600;color:var(--text-primary)">Spotify Baðlý ?</div>
                <div style="font-size:13px;color:#1ED760;margin-top:2px">@${escHtml(currentUser.username)}</div>
                <div style="font-size:11px;color:var(--text-muted);margin-top:2px">Profil sayfanda "Þu an dinliyor" kutusu gösterilecek</div>
              </div>
            </div>
            ...
            <div class="form-group">
              <label class="checkbox-label">
                ...
                Þu an dinlediðimi profilimde göster
              </label>
            </div>
            <div style="display:flex;gap:8px">
              <button class="btn btn-primary" id="spotify-save-vis">Kaydet</button>
              <button class="btn btn-danger" id="spotify-disconnect">Baðlantýyý Kes</button>
            </div>
          ` : `
            <div style="font-size:14px;color:var(--text-secondary);margin-bottom:16px">
              Spotify hesabýný baðlayarak profilinde þu an dinlediðin müziði gösterebilirsin — týpký Discord gibi.
            </div>
            <a href="#" id="spotify-connect-btn" class="btn btn-primary" style="background:linear-gradient(135deg,#1ED760,#17a84a);border:none;text-decoration:none">
              <i class="fab fa-spotify"></i> Spotify Hesabýný Baðla
            </a>
          `}
          <div id="spotify-msg" class="form-error mt-4"></div>
        </div>
      </div>`;
      // Þu an çalan önizleme
      if (hasSpotify) {
        fetch('/api/spotify/now-playing/' + encodeURIComponent(currentUser.username))
          .then(r => r.json()).then(data => {
            ...
            if (data.playing) {
              ...
              <div style="flex:1;min-width:0">
                <div style="font-size:10px;color:#1ED760;font-weight:600;text-transform:uppercase;letter-spacing:.5px">Þu an çalýyor</div>
              ...
              pre.innerHTML = `<div style="font-size:12px;color:var(--text-muted);padding:8px 0">Þu an bir þey çalmýyor.</div>`;
            }
          }).catch(() => { });
      }
      ...
      $('#spotify-disconnect')?.addEventListener('click', async () => {
        try {
          ...
          toast('Spotify baðlantýsý kesildi');
      ...
      <div class="card" style="border-color:rgba(220,38,38,0.3)">
        ...
        <div class="card-body">
          <div style="font-size:14px;font-weight:600;margin-bottom:8px">Hesabý Sil</div>
          <ul style="font-size:13px;color:var(--text-secondary);margin:0 0 16px 18px;line-height:1.8">
            ...
            <li>Hesap <strong>10 gün</strong> içinde kalýcý olarak silinir</li>
            <li>10 gün içinde giriþ yaparak silme iþlemini iptal edebilirsin</li>
            <li>Bu iþlem geri alýnamazsa tüm veriler kalýcý silinir</li>
          </ul>
          <div class="form-group">
            <label>Þifreni Girerek Onayla</label>
          ...
          <div class="form-group">
            <label class="checkbox-label">
              ...
              <span>Hesabýmýn silineceðini ve bu iþlemin 10 gün içinde geri alýnabileceðini anlýyorum</span>
            </label>
          </div>
          <button class="btn btn-danger" id="delete-account-btn" style="width:100%;justify-content:center;background:rgba(220,38,38,0.15);border:1px solid rgba(220,38,38,0.4);color:var(--accent-red2)">
            <i class="fas fa-trash-alt"></i> Hesabýmý Silmek Ýstiyorum
          </button>
      ...
      $('#delete-account-btn').addEventListener('click', async () => {
        ...
        if (!pw) { msg.textContent = 'Þifrenizi girin'; return; }
        if (!confirmed) { msg.textContent = 'Onay kutusunu iþaretleyin'; return; }
        if (!confirm('Emin misiniz? Hesabýnýz ve içerikleriniz gizlenecek, 10 gün içinde kalýcý silinecek.')) return;
        ...
        try {
          ...
          updateNavUI();
          navigate('/');
          toast('Hesap silme talebiniz alýndý. 10 gün içinde giriþ yaparak iptal edebilirsiniz.');
        } catch (e) {
          msg.textContent = e.message;
          btn.disabled = false;
          btn.innerHTML = '<i class="fas fa-trash-alt"></i> Hesabýmý Silmek Ýstiyorum';
        }
      });
    }
  }
  ...
  function renderLogin(app) {
    ...
    document.title = 'Giriþ Yap - TeaTube';
    app.innerHTML = `<div class="auth-page">
    <div class="auth-card card card-body">
      <div class="auth-title">Giriþ Yap</div>
      <p class="auth-subtitle">Hesabýnýza eriþin</p>
      <div class="form-group"><label>Kullanýcý Adý</label><input type="text" id="login-id" placeholder="kullanýcý_adý" autocomplete="username" /></div>
      <div class="form-group">
        <label>Þifre</label>
        <div style="position:relative">
      ...
      <button class="btn btn-primary" style="width:100%;margin-top:4px" id="login-btn">Giriþ Yap</button>
      <div id="login-error" class="form-error mt-4" style="text-align:center"></div>
      <div class="auth-footer">Hesabýn yok mu? <a href="/kayit" data-link class="auth-link">Kayýt Ol</a></div>
    </div>
  </div>`;
    ...
    const doLogin = async () => {
      ...
      if (!login || !password) { $('#login-error').textContent = 'Tüm alanlarý doldurun'; return; }
      try {
        const data = await api('/auth/login', { method: 'POST', body: JSON.stringify({ login, password }) });
        // Silinme talebi verilmiþ hesap
        if (data.pending_delete) {
          ...
          <div class="auth-card card card-body" style="border-color:rgba(220,38,38,0.4)">
            <div style="text-align:center;margin-bottom:20px">
              ...
              <div style="font-size:18px;font-weight:700;color:var(--accent-red2)">Hesabýnýzýn Silinmesi Ýstendi</div>
              <p style="font-size:13px;color:var(--text-secondary);margin-top:8px">
                Hesabýnýz <strong>${daysLeft} gün</strong> içinde kalýcý olarak silinecek.
            ...
            <button class="btn btn-primary" id="cancel-delete-btn" style="width:100%;justify-content:center;margin-bottom:10px">
              <i class="fas fa-undo"></i> Vazgeç, Hesabýmý Geri Al
            </button>
            <button class="btn btn-outline" id="keep-delete-btn" style="width:100%;justify-content:center;color:var(--accent-red2);border-color:rgba(220,38,38,0.3)">
              <i class="fas fa-trash"></i> Hayýr, Silinsin
            </button>
          </div>
        </div>`;
          $('#cancel-delete-btn').addEventListener('click', async () => {
            try {
              // Geçici tokenla cancel-delete çaðýr
              ...
              updateNavUI(); toast('Hesabýn geri alýndý, hoþ geldin ' + currentUser.username + '!');
              navigate('/');
            } catch (e) { toast(e.message, 'error'); }
          });
        ...
        localStorage.setItem('token', currentToken);
        updateNavUI(); toast('Hoþ geldiniz, ' + currentUser.username + '!');
        navigate('/');
  ...
  function renderRegister(app) {
    if (currentUser) { navigate('/'); return; }
    document.title = 'Kayýt Ol - TeaTube';
    app.innerHTML = `<div class="auth-page">
    <div class="auth-card card card-body">
      <div class="auth-title">Kayýt Ol</div>
      <p class="auth-subtitle">Topluluða katýl</p>
      <div class="form-group"><label>Kullanýcý Adý</label><input type="text" id="reg-username" placeholder="..." autocomplete="username" /></div>
      <div class="form-group"><label>Görünen Ýsim / Takma Ad (opsiyonel)</label><input type="text" id="reg-nickname" placeholder="..." autocomplete="nickname" /></div>
      <div class="form-group">
        <label style="display:flex;align-items:center;gap:8px">
          E-posta
          <span style="font-size:11px;color:var(--text-muted);font-weight:400;font-style:italic">Sallayabilirsiniz. Zaten umursamýyoruz&nbsp;: )</span>
        </label>
      ...
      <div class="form-group">
        <label class="checkbox-label">
          ...
          <span>KVKK aydýnlatma metnini okudum ve kabul ediyorum. <button type="button" class="btn btn-ghost btn-sm" id="kvkk-btn" style="padding:0;color:var(--accent-red2);font-size:13px">Metni oku</button></span>
        </label>
      </div>
      <button class="btn btn-primary" style="width:100%;margin-top:4px" id="reg-btn">Kayýt Ol</button>
      <div id="reg-error" class="form-error mt-4" style="text-align:center"></div>
      <div class="auth-footer">Zaten hesabýn var mý? <a href="/giris" data-link class="auth-link">Giriþ Yap</a></div>
    </div>
  </div>`;
    ...
    $('#kvkk-btn').addEventListener('click', async () => {
      try {
        ...
        showModal('KVKK Aydýnlatma Metni', `<div style="white-space:pre-wrap;font-size:13px;line-height:1.7;color:var(--text-secondary);max-height:400px;overflow-y:auto">${escHtml(r.text)}</div>`);
      } catch { }
    });
    ...
    const doRegister = async () => {
      ...
      if (!username || !email || !password) { $('#reg-error').textContent = 'Tüm alanlarý doldurun'; return; }
      if (!kvkk_accepted) { $('#reg-error').textContent = 'KVKK onayý zorunludur'; return; }
      try {
        const data = await api('/auth/register', { method: 'POST', body: JSON.stringify({ username, nickname, email, password, kvkk_accepted }) });
        currentToken = data.token; currentUser = data.user;
        localStorage.setItem('token', currentToken);
        updateNavUI(); toast('Hoþ geldiniz, ' + currentUser.username + '!');
        navigate('/');
  ...
  function renderNotFound(app) {
    document.title = 'Sayfa Bulunamadý - TeaTube';
    ...
    <div style="font-size:24px;font-weight:700;margin-bottom:12px">Sayfa Bulunamadý</div>
    <p style="color:var(--text-secondary);margin-bottom:24px">O sayfa taze bitti abim, veremmi baþkasýný?</p>
  ...
  async function init() {
    ...
    try {
      ...
      if (footer) {
        ...
        footer.innerHTML = createdVisible ? `Created By. Ýsmail DEMÝRCAN &nbsp;${copyrightText}` : copyrightText;
      }
    } catch { }
    loadAnnouncements();
  ...
  async function showForwardForumModal(forum) {
    ...
    const listHTML = convs.length === 0
      ? `<div class="empty-state" style="padding:20px"><p>Henüz mesajlaþma yok. Bir kullanýcýya mesaj gönderin.</p></div>`
    ...
    showModal('Forumu Ýlet', `
    <div style="margin-bottom:12px">
      <input id="fwd-search" type="text" placeholder="Kullanýcý adý ara..." style="width:100%;padding:8px 12px;background:var(--bg-card2);border:1px solid var(--border);border-radius:8px;color:var(--text-primary);font-size:13px" />
    </div>
    ...
    <div style="margin-top:8px;display:flex;gap:8px">
      <input id="fwd-username" type="text" placeholder="veya direkt kullanýcý adý gir..." style="flex:1;padding:8px 12px;background:var(--bg-card2);border:1px solid var(--border);border-radius:8px;color:var(--text-primary);font-size:13px" />
      <button class="btn btn-primary" id="fwd-send-btn"><i class="fas fa-paper-plane"></i> Ýlet</button>
    </div>
  ...
  `);
    ...
    $('#fwd-send-btn').addEventListener('click', async () => {
      ...
      if (!username) { $('#fwd-error').textContent = 'Kullanýcý adý girin'; return; }
      try {
  ...
  async function renderMessages(app, targetUsername) {
    ...
    <div class="dm-sidebar">
      ...
      <div style="padding:10px;border-top:1px solid var(--border);">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          <strong style="font-size:13px">Arkadaþlar</strong>
    ...
    <div class="dm-main" id="dm-main">
      ${targetUsername ? '' : `<div class="dm-empty"><i class="fas fa-comments" style="font-size:48px;color:var(--text-muted);margin-bottom:16px"></i><p style="color:var(--text-muted)">Bir konuþma seçin</p></div>`}
    </div>
  </div>`;
    ...
    async function loadDmFriends() {
      ...
      try {
        ...
        <div style="display:flex;gap:8px;align-items:center;margin-bottom:8px">
          <button class="btn btn-ghost btn-sm" id="dm-friends-tab-friends">Arkadaþlar (${accepted.length})</button>
          <button class="btn btn-ghost btn-sm" id="dm-friends-tab-req">Ýstekler (${pending_in.length})</button>
        ...
        <div style="margin-top:8px"><input id="dm-friend-search" type="text" placeholder="Kullanýcý ara..." style="width:100%;padding:8px;border:1px solid var(--border);border-radius:8px;background:var(--bg-card2);color:var(--text-primary)" /></div>
      `;
        ...
        function renderTab(tab) {
          ...
          `).join('') : '<div style="color:var(--text-muted);padding:8px;text-align:center">Arkadaþ yok</div>';
          } else if (tab === 'req') {
          ...
          `).join('') : '<div style="color:var(--text-muted);padding:8px;text-align:center">Ýstek yok</div>';
          } else {
          ...
          `).join('') : '<div style="color:var(--text-muted);padding:8px;text-align:center">Gönderilen isteði yok</div>';
          }
        }
        renderTab('friends');
    ...
    $('#new-dm-btn')?.addEventListener('click', () => {
      ...
      <div class="form-group"><label>Kullanýcý adý</label><input id="new-dm-username" type="text" placeholder="kullanici_adi" /></div>
  ...
  async function renderDMChat(username) {
    ...
    // Karþý tarafýn mesajlarýný okundu iþaretle
    try { api(`/conversation/${encodeURIComponent(username)}/mark-read`, { method: 'POST' }); } catch { }

    // Sidebar'daki bu konuþmanýn unread badge'ini kaldýr
    ...
    if (isHidden) {
      ...
      <div class="dm-empty" style="flex:1">
        ...
        <p style="color:var(--text-muted)">Bu konuþma kilitli</p>
        ${hasPassword ? `<div style="margin-top:16px;display:flex;gap:8px">
          <input id="dm-unlock-pass" type="password" placeholder="Þifre" style="padding:8px 12px;background:var(--bg-card2);border:1px solid var(--border);border-radius:8px;color:var(--text-primary)" />
    ...
    <div class="dm-chat-header">
      ...
      <div style="display:flex;gap:8px;align-items:center">
        <div id="dm-sel-actions" style="display:none;gap:6px">
          ...
          <button class="btn btn-ghost btn-sm" id="dm-sel-cancel">Ýptal</button>
        </div>
    ...
    <div class="dm-input-bar">
      <label class="btn btn-ghost btn-sm" for="dm-img-input" title="Fotoðraf ekle" style="cursor:pointer;padding:6px"><i class="fas fa-image"></i></label>
    ...
    async function sendDmMessage() {
      ...
      try {
        ...
        const convItem = $(`.dm-conv-item[data-username="${CSS.escape(username)}"]`);
        if (convItem) convItem.querySelector('.dm-conv-last').textContent = content || '?? Fotoðraf';
      } catch (e) { toast(e.message, 'error'); }
    ...
    // Mesaj aksiyonlarý (üç nokta, seç, yanýtla, sil)
  ...
  function showDmMsgMenu(btn, msgId, isOwn, username, replyToId, setReply) {
    ...
    const items = [
      { label: '<i class="fas fa-reply"></i> Yanýtla', action: 'reply' },
  ...
  function showDmOptionsMenu(username, convId) {
    showModal('Konuþma Seçenekleri', `
    <div style="display:flex;flex-direction:column;gap:8px">
      <button class="btn btn-outline" id="dm-opt-hide"><i class="fas fa-lock"></i> Gizle / Kilitle</button>
      <button class="btn btn-outline" id="dm-opt-setpass"><i class="fas fa-key"></i> Þifre Deðiþtir</button>
      <button class="btn btn-danger" id="dm-opt-delete"><i class="fas fa-trash"></i> Konuþmayý Sil</button>
    </div>
  `);
    $('#dm-opt-hide').addEventListener('click', () => {
      hideModal();
      showModal('Konuþmayý Gizle', `
      <p style="font-size:13px;color:var(--text-secondary);margin-bottom:12px">Þifre koyarsanýz açmak için þifre gerekecek.</p>
      <div class="form-group"><label>Þifre (opsiyonel)</label><input id="dm-hide-pass" type="password" placeholder="Þifresiz býrakmak için boþ býrakýn" /></div>
      <div style="display:flex;gap:8px">
        <button class="btn btn-primary" id="dm-hide-confirm" style="flex:1">Gizle</button>
        <button class="btn btn-outline" onclick="hideModal()" style="flex:1">Ýptal</button>
      </div>
    `);
      $('#dm-hide-confirm').addEventListener('click', async () => {
        const pass = $('#dm-hide-pass').value;
        try { await api(`/conversation/${encodeURIComponent(username)}/hide`, { method: 'POST', body: JSON.stringify({ password: pass }) }); hideModal(); navigate('/mesajlar'); toast('Konuþma gizlendi'); } catch (e) { toast(e.message, 'error'); }
      });
    });
    $('#dm-opt-setpass').addEventListener('click', () => {
      hideModal();
      showModal('Þifre Deðiþtir', `
      <div class="form-group"><label>Yeni Þifre (boþ = þifresiz)</label><input id="dm-newpass" type="password" /></div>
    ...
    `);
      $('#dm-setpass-confirm').addEventListener('click', async () => {
        ...
        try { await api(`/conversation/${encodeURIComponent(username)}/set-password`, { method: 'POST', body: JSON.stringify({ password: pass }) }); hideModal(); toast('Þifre güncellendi'); } catch (e) { toast(e.message, 'error'); }
      });
    });
    $('#dm-opt-delete').addEventListener('click', async () => {
      if (!confirm('Konuþma silinsin mi?')) return;
      try { await api(`/conversation/${encodeURIComponent(username)}`, { method: 'DELETE' }); hideModal(); navigate('/mesajlar'); toast('Konuþma silindi'); } catch (e) { toast(e.message, 'error'); }
    });
  }
  ...
  async function renderFriends(app) {
    if (!currentUser) { navigate('/giris'); return; }
    document.title = 'Arkadaþlar - TeaTube';
    let friends = [];
    ...
    app.innerHTML = `<div class="container page">
    <div class="page-header"><div class="page-title"><i class="fas fa-user-friends" style="color:var(--accent-red)"></i> Arkadaþlar</div></div>
    <div class="friends-layout">
      <div>
        <div class="tabs" style="margin-bottom:16px">
          <button class="tab active" id="tab-friends" onclick="showFriendsTab('friends')">Arkadaþlar (${accepted.length})</button>
          <button class="tab" id="tab-requests" onclick="showFriendsTab('requests')">Ýstekler ${pending_in.length > 0 ? `<span style="background:var(--accent-red);color:#fff;font-size:10px;padding:1px 5px;border-radius:10px;margin-left:4px">${pending_in.length}</span>` : ''}</button>
        ...
        <div id="friends-content">
          <div id="tab-content-friends">
            ${accepted.length === 0 ? '<div class="empty-state"><i class="fas fa-user-friends"></i><p>Henüz arkadaþýn yok</p></div>'
      ...
      <div>
        <div class="card card-body">
          <div style="font-size:14px;font-weight:600;margin-bottom:12px"><i class="fas fa-search" style="color:var(--accent-red)"></i> Kullanýcý Ara</div>
          <div style="display:flex;gap:8px">
            <input id="friend-search-input" type="text" placeholder="Kullanýcý adý..." style="flex:1;padding:9px 12px;background:var(--bg-card2);border:1px solid var(--border);border-radius:8px;color:var(--text-primary);font-size:13px" />
    ...
    async function doFriendSearch() {
      ...
      try {
        const users = await api(`/search/users?q=${encodeURIComponent(q)}`);
        if (!users.length) { res.innerHTML = '<p style="color:var(--text-muted);font-size:13px;text-align:center">Sonuç bulunamadý</p>'; return; }
    ...
    app.addEventListener('click', async e => {
      ...
      if (remove) { if (!confirm('Arkadaþlýktan çýkart?')) return; try { await api(`/friends/${remove.dataset.id}`, { method: 'DELETE' }); renderFriends(app); } catch (e) { toast(e.message, 'error'); } }
  ...
  function friendItemHTML(f, type, myId) {
    ...
    <div style="flex:1">
      ...
      ${type === 'incoming' ? `<div style="font-size:11px;color:var(--accent-red2)"><i class="fas fa-user-plus"></i> Arkadaþlýk isteði gönderdi</div>` : ''}
    </div>
    <div style="display:flex;gap:6px">
      ...
      ${type === 'accepted' || type === 'outgoing' ? `<button class="btn btn-ghost btn-sm friend-remove" data-id="${f.id}" title="${type === 'outgoing' ? 'Ýptal' : 'Sil'}"><i class="fas fa-user-minus"></i></button>` : ''}
    </div>
  </div>`;
  }
  ...
  function blockItemHTML(b) {
    ...
    ${!isDeleted ? `<button class="btn btn-outline btn-sm friend-unblock" data-username="${escHtml(b.username)}">Engeli Kaldýr</button>` : ''}
  </div>`;
  }
  ...
  (function () {
    ...
    // Auth deðiþince toggle butonunu güncelle
      ...
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <div style="flex:1;min-width:0">
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px">
            ...
            <span style="font-size:10px;color:#1ED760;font-weight:600;text-transform:uppercase;letter-spacing:0.5px">Þu an dinliyor</span>
          </div>
  ...
  async function renderMusicList(app) {
    ...
    <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:20px">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
        ${currentUser && !currentUser.is_artist ? `<a href="/artist-basvuru" data-link class="btn btn-outline btn-sm"><i class="fas fa-microphone"></i> Artist Baþvurusu</a>` : ''}
        ${currentUser?.is_artist ? `<a href="/artist-panel" data-link class="btn btn-primary btn-sm"><i class="fas fa-upload"></i> Þarký Yükle</a>` : ''}
        ${currentUser && !currentUser.is_artist ? `<a href="/sarki-yukle" data-link class="btn btn-outline btn-sm"><i class="fas fa-share"></i> Þarký Paylaþ</a>` : ''}
      </div>
    </div>
    <div class="music-search-bar" style="margin-bottom:20px">
      <div class="search-bar" style="margin:0">
        <i class="fas fa-search"></i>
        <input type="text" id="music-search" placeholder="Þarký adý, sanatçý, tür, daðýtýcý, þarký sözü ara..." style="width:100%" />
      </div>
    </div>
    ...
    const loadSongs = async (q = '') => {
      ...
      try {
        ...
        songs = await api(url);
        if (!songs.length) { el.innerHTML = '<div class="empty-state"><i class="fas fa-music"></i><p>Henüz þarký yok.</p></div>'; return; }
        el.innerHTML = `<div class="music-table">
        <div class="music-table-header">
          <div style="width:40px">#</div>
          <div style="flex:1">Baþlýk</div>
          <div style="width:160px;display:none" class="col-dist">Daðýtýcý</div>
  ...
  function openMiniPlayer(audioUrl, slug, song) {
    ...
    if (currentAudio) { currentAudio.pause(); currentAudio = null; currentAudioSlug = null; }
    currentAudio = audio;
    currentAudioSlug = slug;
    ...
    const savedVol = parseFloat(localStorage.getItem('teatube_volume') ?? localStorage.getItem('TeaTube_volume') ?? '0.8');
    ...
    audio.addEventListener('ended', () => {
      ...
      currentAudio = null; currentAudioSlug = null;
      document.querySelectorAll('.music-play-mini').forEach(b => b.innerHTML = '<i class="fas fa-play"></i>');
    });

    document.getElementById('gp-play').addEventListener('click', () => {
      const btn = document.getElementById('gp-play');
      if (audio.paused) { audio.play(); if (btn) btn.innerHTML = '<i class="fas fa-pause"></i>'; }
      else { audio.pause(); if (btn) btn.innerHTML = '<i class="fas fa-play"></i>'; }
    });
  ...
  async function renderMusicDetail(app, slug) {
    ...
    try { song = await api('/songs/' + slug); } catch {
      app.innerHTML = '<div class="container page"><div class="empty-state"><i class="fas fa-music"></i><p>Þarký bulunamadý.</p></div></div>'; return;
    }
    ...
    <div class="music-detail-header">
      ...
      <div class="music-detail-info">
        <div class="music-detail-header-row" style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap">
          <div>
            <div class="music-detail-type-badge">${isOwn ? '<i class="fas fa-microphone"></i> Sanatçý Þarkýsý' : '<i class="fas fa-share"></i> Paylaþýlan Þarký'}</div>
            <div class="music-detail-title">${escHtml(song.title)}</div>
            <div class="music-detail-artist">${escHtml(song.artist_name)}</div>
          </div>
          ${isUploader ? `<div><button class="music-edit-top-btn btn btn-outline btn-sm" id="song-edit-btn"><i class="fas fa-edit"></i> Düzenle</button></div>` : ''}
        </div>
        ...
        ${!isOwn && song.share_reason ? `
          <div class="music-share-reason">
            <div class="music-share-reason-label"><i class="fas fa-question-circle"></i> Neden paylaþtýnýz?</div>
    ...
    ${hasLyrics ? `
      <div class="music-lyrics-box">
        <div class="music-lyrics-title"><i class="fas fa-align-left"></i> Þarký Sözleri</div>
    ...
    // Detail page ses kontrolü (localStorage'dan baþlat)
    const savedVol = parseFloat(localStorage.getItem('teatube_volume') ?? localStorage.getItem('TeaTube_volume') ?? '0.8');
    audio.volume = savedVol;
    ...
    // Þarký düzenleme butonu (sadece yükleyene gösterilir)
    const editBtn = document.getElementById('song-edit-btn');
    if (editBtn) {
      editBtn.addEventListener('click', () => {
        showModal(`?? Þarkýyý Düzenle — ${escHtml(song.title)}`, `
        <div class="form-group"><label>Þarký Adý</label><input id="ue-title" value="${escHtml(song.title)}" /></div>
        ${song.song_type === 'own' ? `
        <div class="form-row">
          <div class="form-group"><label>Sanatçý Adý</label><input id="ue-artist" value="${escHtml(song.artist_name)}" /></div>
          <div class="form-group"><label>Daðýtýcý</label><input id="ue-dist" value="${escHtml(song.distributor || '')}" /></div>
        </div>` : `
        <div class="form-group"><label>Sanatçý Adý</label><input id="ue-artist" value="${escHtml(song.artist_name)}" /></div>`}
        <div class="form-group"><label>Müzik Türü</label><input id="ue-genre" value="${escHtml(song.genre || '')}" /></div>
        <div class="form-group"><label>Þarký Sözleri</label><textarea id="ue-lyrics" rows="5">${escHtml(song.lyrics || '')}</textarea></div>
        ${song.song_type === 'other' ? `<div class="form-group"><label>Paylaþma Sebebi</label><textarea id="ue-reason" rows="2">${escHtml(song.share_reason || '')}</textarea></div>` : ''}
        <div class="form-group"><label>Yeni Kapak Fotoðrafý <span style="font-size:11px;color:var(--text-muted)">(boþ býrak = deðiþmez)</span></label>
        ...
        <div class="form-group"><label>Yeni Ses Dosyasý <span style="font-size:11px;color:var(--text-muted)">(boþ býrak = deðiþmez)</span></label>
  ...
  async function renderArtistApply(app) {
    if (!currentUser) { navigate('/giris'); return; }
    document.title = 'Artist Baþvurusu – TeaTube';
    let existing = null;
    ...
    if (isAccepted || currentUser.is_artist) {
      ...
      <div style="text-align:center;padding:60px 20px">
        ...
        <p style="color:var(--text-secondary);margin-bottom:24px">Þarký yüklemek için artist paneline gidin.</p>
    ...
    app.innerHTML = `<div class="container page" style="max-width:600px;margin:0 auto">
    <div class="page-title"><i class="fas fa-microphone" style="color:var(--accent-red2);margin-right:8px"></i>Artist Rozeti Baþvurusu</div>
    ${isPending ? `
      <div class="card" style="margin-bottom:20px">
        <div class="card-body" style="text-align:center;padding:40px">
          <div style="font-size:40px;margin-bottom:12px">?</div>
          <div style="font-size:18px;font-weight:700">Baþvurunuz Bekliyor</div>
          <p style="color:var(--text-secondary);margin-top:8px">Ekibimiz baþvurunuzu inceliyor. Onaylanýnca bildirim alacaksýnýz.</p>
        </div>
      </div>` : ''}
    ${isRejected ? `
      <div class="card" style="margin-bottom:20px;border-color:rgba(220,38,38,0.4)">
        <div class="card-body" style="padding:16px">
          <div style="color:var(--accent-red2);font-weight:600"><i class="fas fa-times-circle"></i> Baþvurunuz Reddedildi</div>
          <p style="font-size:13px;color:var(--text-secondary);margin-top:6px">Yeniden baþvurabilirsiniz.</p>
        </div>
      </div>` : ''}
    ${!isPending ? `
    <div class="card">
      <div class="card-body">
        <p style="font-size:14px;color:var(--text-secondary);margin-bottom:20px">
          Artist rozeti alarak kendi þarkýlarýnýzý TeaTube'da yayýnlayabilirsiniz.
        </p>
        <div class="form-group"><label>Müzik Türünüz *</label>
          <input id="apply-genre" placeholder="Pop, Rock, Hip-Hop, Elektronik..." />
        </div>
        <div class="form-group"><label>Örnek Þarký URL (SoundCloud, YouTube vb.)</label>
          <input id="apply-url" placeholder="https://soundcloud.com/..." />
        </div>
        <div class="form-group"><label>veya Örnek Þarký Dosyasý Yükle</label>
        ...
        <div class="form-group"><label>Notunuz (isteðe baðlý)</label>
          <textarea id="apply-note" rows="3" placeholder="Kendinizi kýsaca tanýtýn..."></textarea>
        </div>
        <button class="btn btn-primary" id="apply-submit" style="width:100%;justify-content:center">
          <i class="fas fa-paper-plane"></i> Baþvuruyu Gönder
        </button>
    ...
    document.getElementById('apply-submit')?.addEventListener('click', async () => {
      ...
      try {
        ...
        <div style="font-size:22px;font-weight:700;margin-top:12px">Baþvurunuz Alýndý!</div>
        <p style="color:var(--text-secondary);margin-top:8px">Ekibimiz inceleyecek, onaylanýnca bildirim alýrsýnýz.</p>
        <a href="/" data-link class="btn btn-outline" style="margin-top:20px">Ana Sayfaya Dön</a>
      </div>`;
      } catch (e) { msg.style.color = 'var(--accent-red2)'; msg.textContent = e.message; btn.disabled = false; btn.innerHTML = '<i class="fas fa-paper-plane"></i> Baþvuruyu Gönder'; }
    });
  }
  ...
  async function renderArtistPanel(app) {
        ...
        <div class="form-group">
          ...
          <div style="display:flex;gap:10px">
            <label class="checkbox-label" style="flex:1;padding:12px;background:var(--bg-card2);border:1px solid var(--border);border-radius:8px;cursor:pointer">
              <input type="radio" name="song-type" id="st-own" value="own" checked style="width:auto" /> Kendi Þarkým
            </label>
            <label class="checkbox-label" style="flex:1;padding:12px;background:var(--bg-card2);border:1px solid var(--border);border-radius:8px;cursor:pointer">
              <input type="radio" name="song-type" id="st-other" value="other" style="width:auto" /> Baþkasýnýn Þarkýsý
            </label>
          </div>
        </div>
        <div id="own-fields">
          <div class="form-group"><label>Yayýmlayýcý / Daðýtýcý Ýsmi</label><input id="s-distributor" placeholder="Kendi adýn ya da þirket adý" /></div>
          <div class="form-group"><label>Þarký Adý *</label><input id="s-title" /></div>
          <div class="form-group"><label>Þarký Türü</label><input id="s-genre" placeholder="Pop, Rock, Elektronik..." /></div>
          <div class="form-group"><label>Þarký Dosyasý * (MP3/WAV)</label><input type="file" id="s-audio" accept="audio/*" style="background:var(--bg-card2);border:1px dashed var(--border);padding:10px;cursor:pointer" /></div>
          <div class="form-group"><label>Kapak Fotoðrafý</label><input type="file" id="s-cover" accept="image/*" style="background:var(--bg-card2);border:1px dashed var(--border);padding:10px;cursor:pointer" /></div>
          <div class="form-group"><label>Þarký Sözleri (isteðe baðlý)</label><textarea id="s-lyrics" rows="6" placeholder="Þarký sözlerini buraya yapýþtýrýn..."></textarea></div>
          ...
          <label class="checkbox-label" style="margin-bottom:16px"><input type="checkbox" id="s-rules-own" style="width:auto" /> Þarký yayýnlama kurallarýný okudum ve kabul ediyorum</label>
        </div>
        <div id="other-fields" style="display:none">
          <div class="form-group"><label>Þarký Adý *</label><input id="s-title-o" /></div>
          <div class="form-group"><label>Þarký Sahibi (Sanatçý) *</label><input id="s-artist-o" /></div>
          <div class="form-group"><label>Kapak Fotoðrafý</label><input type="file" id="s-cover-o" accept="image/*" style="background:var(--bg-card2);border:1px dashed var(--border);padding:10px;cursor:pointer" /></div>
          <div class="form-group"><label>Þarký Sözleri (isteðe baðlý)</label><textarea id="s-lyrics-o" rows="6" placeholder="Þarký sözlerini buraya yapýþtýrýn..."></textarea></div>
          <div class="form-group"><label>Þarký Dosyasý * (MP3/WAV)</label><input type="file" id="s-audio-o" accept="audio/*" style="background:var(--bg-card2);border:1px dashed var(--border);padding:10px;cursor:pointer" /></div>
          <div class="form-group"><label>Neden paylaþtýnýz? *</label><textarea id="s-reason" rows="3" placeholder="Bu þarkýyý neden topluluðumuzla paylaþmak istediniz?"></textarea></div>
          ...
          <label class="checkbox-label" style="margin-bottom:16px"><input type="checkbox" id="s-rules-other" style="width:auto" /> Baþkasýnýn þarkýsýný paylaþma kurallarýný okudum ve kabul ediyorum</label>
        </div>
        <button class="btn btn-primary" id="song-upload-btn" style="width:100%;justify-content:center"><i class="fas fa-upload"></i> Þarkýyý Yayýnla</button>
    ...
    document.getElementById('song-upload-btn').addEventListener('click', async () => {
      ...
      if (!rules_ok) { msg.style.color = 'var(--accent-red2)'; msg.textContent = 'Kurallarý kabul etmelisiniz'; return; }
      const fd = new FormData();
      fd.append('song_type', isOwn ? 'own' : 'other');
      fd.append('rules_accepted', '1');
      if (isOwn) {
        const title = document.getElementById('s-title')?.value.trim();
        if (!title) { msg.style.color = 'var(--accent-red2)'; msg.textContent = 'Þarký adý gerekli'; return; }
        const audio = document.getElementById('s-audio')?.files[0];
        if (!audio) { msg.style.color = 'var(--accent-red2)'; msg.textContent = 'Ses dosyasý gerekli'; return; }
        ...
        if (!title || !artist) { msg.style.color = 'var(--accent-red2)'; msg.textContent = 'Baþlýk ve sanatçý adý gerekli'; return; }
      ...
      } catch (e) { msg.style.color = 'var(--accent-red2)'; msg.textContent = e.message; btn.disabled = false; btn.innerHTML = '<i class="fas fa-upload"></i> Þarkýyý Yayýnla'; }
    });
  }

  // ===== BAÞKASININ ÞARKISINI PAYLAÞ (artist rozeti gerekmez) =====
  async function renderShareSong(app) {
    if (!currentUser) { navigate('/giris'); return; }
    // Artist olanlar kendi panelini kullansýn
    if (currentUser.is_artist) { navigate('/artist-panel'); return; }
    document.title = 'Þarký Paylaþ – TeaTube';

    let rules = { other_rules: '' };
    try { rules = await api('/music-rules'); } catch { }

    app.innerHTML = `<div class="container page" style="max-width:680px;margin:0 auto">
    <div class="page-title"><i class="fas fa-share-alt" style="color:var(--accent-red2);margin-right:8px"></i>Þarký Paylaþ</div>
    <div class="card" style="margin-bottom:16px">
      <div class="card-body" style="font-size:13px;color:var(--text-secondary);display:flex;align-items:flex-start;gap:10px">
        ...
        <div>
          Bu sayfa <strong>baþkasýna ait þarkýlarý</strong> topluluða paylaþmak içindir.
          Kendi þarkýný yüklemek istiyorsan önce
          <a href="/artist-basvuru" data-link style="color:var(--accent-red2)">artist baþvurusu</a> yapman gerekir.
        </div>
      </div>
    </div>
    <div class="card">
      <div class="card-body">
        <div class="form-group">
          <label>Þarký Adý *</label>
          <input id="ss-title" placeholder="Þarkýnýn adý" />
        </div>
        <div class="form-group">
          <label>Sanatçý (Þarký Sahibi) *</label>
          <input id="ss-artist" placeholder="Sanatçýnýn adý" />
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          <label>Þarký Dosyasý * (MP3/WAV, max 50MB)</label>
        ...
        <div class="form-group">
          <label>Kapak Fotoðrafý (isteðe baðlý)</label>
        ...
        <div class="form-group">
          <label>Þarký Sözleri (isteðe baðlý)</label>
          <textarea id="ss-lyrics" rows="5" placeholder="Þarký sözlerini buraya yapýþtýrýn..."></textarea>
        </div>
        <div class="form-group">
          <label>Neden paylaþýyorsunuz? *</label>
          <textarea id="ss-reason" rows="2" placeholder="Bu þarkýyý neden topluluðumuzla paylaþmak istediniz?"></textarea>
        </div>
        ${rules.other_rules ? `
        ...
        <label class="checkbox-label" style="margin-bottom:16px">
          ...
          <span>Baþkasýnýn þarkýsýný paylaþma kurallarýný okudum ve kabul ediyorum</span>
        </label>
    ...
    document.getElementById('ss-submit').addEventListener('click', async () => {
      ...
      if (!title) { msg.style.color = 'var(--accent-red2)'; msg.textContent = 'Þarký adý zorunlu'; return; }
      if (!artist) { msg.style.color = 'var(--accent-red2)'; msg.textContent = 'Sanatçý adý zorunlu'; return; }
      if (!audio) { msg.style.color = 'var(--accent-red2)'; msg.textContent = 'Ses dosyasý zorunlu'; return; }
      if (!reason) { msg.style.color = 'var(--accent-red2)'; msg.textContent = 'Paylaþma sebebi zorunlu'; return; }
      if (!rules_ok) { msg.style.color = 'var(--accent-red2)'; msg.textContent = 'Kurallarý kabul etmelisiniz'; return; }
      ...
      try {
        ...
        msg.textContent = e.message;
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-share"></i> Paylaþ';
      }
    });
  }
}
