...
function timeAgo(dt) {
  ...
  if (sec < 3600) return Math.floor(sec / 60) + ' dk �nce';
  if (sec < 86400) return Math.floor(sec / 3600) + ' sa �nce';
  if (sec < 604800) return Math.floor(sec / 86400) + ' g�n �nce';
...
function renderContent(text) {
  if (!text) return '';
  // XSS g�venli: �nce escape, sonra pattern'lere d�n��t�r
...
function renderRoute(fullPath) {
  // Video observer temizle (sayfa değişiminde)
  if (window._videoAutoplayObserver) {
    window._videoAutoplayObserver.disconnect();
    window._videoAutoplayObserver = null;
  }
  // Query string'i ay�r
...
function updateNavUI() {
  ...
  if (currentUser) {
    ...
    if (mobUserLinks) mobUserLinks.innerHTML = `
      ...
      <a href="/arkadaslar" data-link class="mobile-nav-link"><i class="fas fa-user-friends" style="width:18px"></i> Arkada�lar</a>
...
async function renderHome(app) {
  ...
  updatePageMeta('Demlik � Topluluk Platformu', '�ay kadar s�cak topluluk platformu.', '');
  app.innerHTML = `
    <div class="container page">
      <div class="section">
        <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:16px">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
            ...
            <a href="/forum" data-link class="btn btn-ghost btn-sm">T�m� <i class="fas fa-arrow-right"></i></a>
          </div>
        </div>
      ...
      <div class="section">
        <div class="section-header">
          <div class="section-title"><div class="section-title-bar"></div>�ne ��kan Kitaplar</div>
          <a href="/kitaplar" data-link class="btn btn-ghost btn-sm">T�m� <i class="fas fa-arrow-right"></i></a>
        </div>
  ...
  try {
    ...
    if (!allForums.length) { el.innerHTML = '<div class="empty-state"><i class="fas fa-comments"></i><p>Hen�z konu yok.</p></div>'; }
  ...
  $('#home-forum-search')?.addEventListener('input', e => {
    ...
    const el = $('#home-forums');
    if (!el) return;
    if (!filtered.length) { el.innerHTML = '<div class="empty-state"><i class="fas fa-comments"></i><p>Konu bulunamad�.</p></div>'; return; }
  ...
  try {
    ...
    const el = $('#home-books');
    if (!books.length) { el.innerHTML = '<div class="empty-state"><i class="fas fa-book"></i><p>Hen�z kitap yok.</p></div>'; }
...
async function renderForumList(app, queryString) {
  ...
  updatePageMeta('Konular � Demlik', 'Toplulukla fikir payla�, tart��, ke�fet.', '');

  // URL'den ?tag= parametresini oku � �nce arg�man, yoksa location.search
    ...
    <div class="container page">
      <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
        <div>
          ...
          <div class="page-subtitle">${activeTag ? `<i class="fas fa-hashtag" style="color:var(--accent-red2)"></i> <strong>${escHtml(activeTag)}</strong> etiketiyle filtreli &nbsp;<a href="/forum" data-link style="font-size:12px;color:var(--accent-red2)"><i class="fas fa-times"></i> Temizle</a>` : 'Toplulukla fikir payla�'}</div>
        </div>
...
function renderForumListItems(forums) {
  ...
  if (!el) return;
  if (!forums.length) { el.innerHTML = '<div class="empty-state"><i class="fas fa-comments"></i><p>Konu bulunamad�.</p></div>'; return; }
...
function forumCardHTML(f) {
  ...
  // Kart thumbnail: �nce thumbnail, yoksa 1. ek resim, yoksa banner
...
function showNewForumModal(existing = null) {
  showModal(existing ? 'Konuyu D�zenle' : 'Yeni Konu A�', `
    <div class="form-group"><label>Ba�l�k</label><input id="fm-title" type="text" placeholder="Konu ba�l���" value="${existing ? escHtml(existing.title) : ''}" /></div>
    <div class="form-group"><label>��erik</label><textarea id="fm-content" rows="8" placeholder="Yaz�n�z� buraya girin...">${existing ? escHtml(existing.content) : ''}</textarea></div>
    <div class="form-group">
      <label>Konu T�rleri</label>
      <div id="fm-tags-loading" style="color:var(--text-muted);padding:8px">Y�kleniyor...</div>
    ...
    <button class="btn btn-primary" id="fm-submit" style="width:100%">${existing ? 'G�ncelle' : 'Yay�nla'}</button>
  ...
  api('/tags').then(tags => {
    ...
    $('#fm-tags-loading').textContent = 'Tag y�klenemedi';
  });
  ...
  let extraImageFiles = []; // yeni y�klenecekler
  ...
  $('#fm-submit').addEventListener('click', async () => {
    ...
    if (!title || !content) { $('#fm-error').textContent = 'Ba�l�k ve i�erik zorunlu'; return; }
    ...
    submitBtn.innerHTML = '<div class="spinner" style="width:16px;height:16px;border-width:2px;display:inline-block;margin-right:6px"></div> Y�kleniyor...';
    ...
    try {
        ...
        banner_image = await new Promise((resolve, reject) => {
          ...
          xhr.addEventListener('load', () => {
            ...
            try {
              ...
              if (xhr.status >= 400) return reject(new Error(data.error || 'Y�kleme hatas�'));
              resolve(data.url);
            } catch (e) {
      ...
      const submitBtn = $('#fm-submit');
      if (submitBtn) { submitBtn.disabled = false; submitBtn._submitting = false; submitBtn.innerHTML = existing ? 'G�ncelle' : 'Yay�nla'; }
...
async function renderForumDetail(app, slug) {
  ...
  } catch { app.innerHTML = '<div class="container page"><div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>Konu bulunamad�.</p></div></div>'; return; }
    ...
    <div class="forum-detail">
      ...
      <div class="forum-detail-header">
        ...
        <div class="forum-detail-meta">
          ...
          <span><i class="fas fa-eye" style="color:var(--accent-red)"></i> ${forum.views || 0} g�r�nt�lenme</span>
      ...
      <div class="forum-actions">
        ...
        <button class="forum-action-btn" id="share-btn"><i class="fas fa-share-alt"></i> Payla�</button>
      ...
      <div class="comments-section">
        ...
        ${currentUser && forum.allow_comments ? `
          <div class="comment-form">
            ...
            <textarea id="comment-input" placeholder="Yorumunuzu yaz�n..."></textarea>
...
async function renderBookList(app) {
  ...
  updatePageMeta('Kitaplar � Demlik', 'Toplulu�un yazd��� eserleri ke�fet.', '');
  app.innerHTML = `
    <div class="container page">
      <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
        <div><div class="page-title">Kitaplar</div><div class="page-subtitle">Toplulu�un eserleri</div></div>
...
function renderBookGrid(books) {
  ...
  if (!books.length) { el.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-book-open"></i><p>Kitap bulunamad�.</p></div>'; return; }
...
function showNewBookModal(existing = null) {
  showModal(existing ? 'Kitab� D�zenle' : 'Yeni Kitap', `
    <div class="form-group"><label>Ba�l�k</label><input id="bk-title" type="text" value="${existing ? escHtml(existing.title) : ''}" /></div>
    ...
    <button class="btn btn-primary" id="bk-submit" style="width:100%">${existing ? 'G�ncelle' : 'Olu�tur'}</button>
  ...
  $('#bk-submit').addEventListener('click', async () => {
    ...
    if (!title) { $('#bk-error').textContent = 'Ba�l�k zorunlu'; return; }
    try {
      ...
      if (existing) {
        ...
        toast('Kitap g�ncellendi'); hideModal(); renderRoute(location.pathname);
      } else {
        const b = await api('/books', { method: 'POST', body: JSON.stringify({ title, preface: $('#bk-preface').value.trim(), cover_image }) });
        toast('Kitap olu�turuldu'); hideModal(); navigate('/kitap/' + b.slug);
      }
...
async function renderBookDetail(app, slug) {
  app.innerHTML = `<div class="container page"><div class="loading-center"><div class="spinner"></div></div></div>`;
  let data;
  try { data = await api('/book/' + slug); } catch { app.innerHTML = '<div class="container page"><div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>Kitap bulunamad�.</p></div></div>'; return; }

  const { book, chapters, pages } = data;
  document.title = book.title + ' � Demlik';
  updatePageMeta(book.title + ' � Demlik', book.preface ? book.preface.substring(0,155) : book.title + ' � Demlik\'te yay�nlanan kitap.', book.cover_image || '');
  ...
  $('#download-pdf-btn')?.addEventListener('click', async () => {
    ...
    } catch (e) { toast('PDF olu�turulamad�: ' + e.message, 'error'); }
  });
}
...
async function showAddPageModal(bookSlug, chapters) {
  ...
  showModal('Yeni Sayfa', `
    ...
    <div class="form-group"><label>Sayfa Ba�l���</label><input id="pg-title" type="text" placeholder="Sayfa ba�l���..." /></div>
    ${chapters.length ? `<div class="form-group"><label>B�l�m (opsiyonel)</label><select id="pg-chap"><option value="">-- B�l�m se�in --</option>${chapOptions}</select></div>` : ''}
    <div class="form-group">
    ...
    <div class="form-group"><label>��erik</label><textarea id="pg-content" rows="14" placeholder="Sayfan�n i�eri�ini buraya yaz�n..."></textarea></div>
  ...
  $('#pg-submit').addEventListener('click', async () => {
    ...
    if (!title || !content) { $('#pg-error').textContent = 'Ba�l�k ve i�erik zorunlu'; return; }
...
function showAddChapterModal(bookSlug) {
  ...
  $('#ch-submit').addEventListener('click', async () => {
    ...
    if (!title) { $('#ch-error').textContent = 'Ba�l�k zorunlu'; return; }
    try {
...
async function renderPageReader(app, bookSlug, pageSlug) {
  app.innerHTML = `<div class="container page"><div class="loading-center"><div class="spinner"></div></div></div>`;
  let data;
  try { data = await api(`/book/${bookSlug}/page/${pageSlug}`); } catch { app.innerHTML = '<div class="container page"><div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>Sayfa bulunamad�.</p></div></div>'; return; }
      ...
      <div class="ebook-reader" style="flex:1">
        ...
        <div class="ebook-nav">
          ${prev ? `<a href="/kitap/${escHtml(bookSlug)}/sayfa/${escHtml(prev.slug)}" data-link class="ebook-nav-btn">
            ...
            <div><div style="font-size:11px;color:var(--text-muted);text-transform:uppercase;letter-spacing:1px">�nceki</div><div style="font-size:13px;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escHtml(prev.title)}</div></div>
          </a>` : `<div></div>`}
          <div class="ebook-page-counter">${page.page_num} / ${book.page_count}</div>
  ...
  if (isOwner) {
    $('#edit-page-btn').addEventListener('click', () => {
      showModal('Sayfay� D�zenle', `
        <div class="form-group"><label>Ba�l�k</label><input id="ep-title" type="text" value="${escHtml(page.title)}" /></div>
      ...
      $('#ep-submit').addEventListener('click', async () => {
        ...
        try {
          ...
          toast('Sayfa g�ncellendi'); hideModal(); renderRoute(location.pathname);
...
async function renderGroupList(app) {
  ...
  app.innerHTML = `
    <div class="container page">
      ...
      <div class="search-bar" style="margin-bottom:24px"><i class="fas fa-search"></i><input type="text" id="group-search" placeholder="Grup ara (isim veya a��klama)..." /></div>
  ...
  function renderGroups(list) {
    ...
    if (!el) return;
    if (!list.length) { el.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-users"></i><p>Grup bulunamad�.</p></div>'; return; }
...
function showNewGroupModal() {
  showModal('Yeni Grup', `
    ...
    <div class="form-group">
      ...
      <label class="checkbox-label" style="margin-top:8px"><input type="checkbox" id="gr-photos" checked /> Foto�rafa izin ver</label>
      <label class="checkbox-label" style="margin-top:8px"><input type="checkbox" id="gr-invite" /> Sadece davet ile kat�l�m</label>
    </div>
    <button class="btn btn-primary" id="gr-submit" style="width:100%">Olu�tur</button>
  ...
  $('#gr-submit').addEventListener('click', async () => {
    ...
    try {
      ...
      toast('Grup olu�turuldu'); hideModal(); navigate('/grup/' + g.slug);
...
async function renderGroupDetail(app, slug) {
  ...
  } catch { app.innerHTML = '<div class="container page"><div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>Grup bulunamad�.</p></div></div>'; return; }
        ...
        ${group.allow_chat ? `
          <div class="chat-container">
            <div id="load-more-msgs-wrap" style="text-align:center;padding:8px;display:${messages.length >= 60 ? 'block' : 'none'}">
              <button class="btn btn-outline btn-sm" id="load-more-msgs"><i class="fas fa-history"></i> �nceki Mesajlar</button>
            </div>
            ...
            ${canSend ? `<div class="chat-input-bar">
              ${group.allow_photos ? `<label class="btn btn-ghost btn-sm" for="chat-img-input" title="Foto�raf g�nder" style="flex-shrink:0"><i class="fas fa-image"></i></label><input id="chat-img-input" type="file" accept="image/*" style="display:none" />` : ''}
            ...
            </div>` : (currentUser && !isMember ? `<div style="padding:12px;text-align:center;color:var(--text-muted);font-size:13px">Mesaj g�ndermek i�in gruba kat�l�n.</div>` : `<div style="padding:12px;text-align:center;color:var(--text-muted);font-size:13px">Giri� yaparak kat�labilirsiniz.</div>`)}
      ...
      <div>
        <div class="group-sidebar-card">
          ...
          <div class="card-body" style="font-size:13px;color:var(--text-secondary)">
            <div style="margin-bottom:6px"><i class="fas fa-users"></i> ${group.member_count} �ye</div>
        ...
        <div class="group-sidebar-card">
          <div class="card-header"><span><i class="fas fa-users" style="color:var(--accent-red)"></i> �yeler</span></div>
  ...
  $('#load-more-msgs')?.addEventListener('click', async () => {
    ...
    finally { btn.disabled = false; btn.innerHTML = '<i class="fas fa-history"></i> �nceki Mesajlar'; }
  });
  ...
  $('#group-settings-btn')?.addEventListener('click', () => {
    showModal('Grup Ayarlar�', `
      ...
      <div class="form-group">
        ...
        <label class="checkbox-label" style="margin-top:8px"><input type="checkbox" id="gs-photos" ${group.allow_photos ? 'checked' : ''} /> Foto�raf</label>
      </div>
    ...
    $('#gs-submit').addEventListener('click', async () => {
      try {
        ...
        toast('Grup g�ncellendi'); hideModal(); renderRoute(location.pathname);
  ...
  $('#members-list')?.addEventListener('click', async e => {
    ...
    if (banBtn && isOwner) {
      ...
      if (!confirm('�yeyi gruptan at?')) return;
      try { await api(`/group/${slug}/ban/${uid}`, { method: 'POST' }); toast('�ye at�ld�'); renderRoute(location.pathname); } catch (e) { toast(e.message, 'error'); }
    }
    if (modBtn && isOwner) {
...
async function renderProfile(app, username) {
  app.innerHTML = `<div class="container page"><div class="loading-center"><div class="spinner"></div></div></div>`;
  let data;
  try { data = await api('/profile/' + username); } catch { app.innerHTML = '<div class="container page"><div class="empty-state"><i class="fas fa-user-slash"></i><p>Kullan�c� bulunamad�.</p></div></div>'; return; }
...
async function renderSettings(app) {
  ...
  if (urlParams.get('spotify') === 'ok') { toast('Spotify ba�land�! ??'); history.replaceState({}, '', '/ayarlar'); }
  if (urlParams.get('spotify') === 'error') { toast('Spotify ba�lant�s� ba�ar�s�z', 'error'); history.replaceState({}, '', '/ayarlar'); }
  ...
  if (section === 'profile') {
    ...
    el.innerHTML = `
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="form-row">
            <div class="form-group"><label>�nvan <span style="color:var(--accent-red2)">*</span></label><input type="text" id="s-title" value="${escHtml(currentUser.title || '')}" placeholder="�rn: Yaz�l�m Geli�tirici, ��renci..." /></div>
    ...
    function renderLinkRows(linksArr) {
      ...
      container.innerHTML = linksArr.map((l, i) => `
        <div class="link-row" data-idx="${i}" style="display:flex;gap:8px;align-items:center">
          <input type="text" placeholder="Ba�l�k (�rn: GitHub)" value="${escHtml(l.label || '')}" data-field="label" style="flex:1" />
    ...
    el.innerHTML = `
      <div class="card">
        ...
        <div class="card-body">
          <div class="form-group">
            <label style="font-size:14px;font-weight:600;margin-bottom:10px;display:block"><i class="fas fa-palette" style="color:var(--accent-red2);margin-right:6px"></i>Tema</label>
            <button class="btn btn-outline btn-sm" onclick="showThemeSelector()" style="gap:8px"><i class="fas fa-palette"></i> Tema Seç</button>
            <div style="font-size:12px;color:var(--text-muted);margin-top:6px">Aktif tema: <strong id="cur-theme-label" style="color:var(--accent-red2)">${escHtml(currentUser.theme || 'dark')}</strong></div>
          </div>
        ...
        <div class="card-body">
          <div class="form-group">
            <label class="checkbox-label" style="align-items:flex-start;gap:12px">
              ...
              <div>
                <div style="font-weight:600;font-size:14px">Beni etiketleyen ki�ilere bildirim g�nder</div>
                <div style="font-size:12px;color:var(--text-muted);margin-top:3px">
                  Kapat�rsan kimse seni @etiketleyemez ve bildirim almaz, profil linki de a��lmaz
                </div>
              </div>
            </label>
          </div>
        ...
        <div class="card-body">
          ${hasSpotify ? `
            <div style="display:flex;align-items:center;gap:10px;padding:12px;background:rgba(30,215,96,0.08);border:1px solid rgba(30,215,96,0.2);border-radius:8px;margin-bottom:16px">
              ...
              <div>
                <div style="font-weight:600;color:var(--text-primary)">Spotify Ba�l� ?</div>
            ...
            <div style="display:flex;gap:8px">
              ...
              <button class="btn btn-danger" id="spotify-disconnect">Ba�lant�y� Kes</button>
            </div>
          ` : `
            <div style="font-size:14px;color:var(--text-secondary);margin-bottom:16px">
              Spotify hesab�n� ba�layarak profilinde �u an dinledi�in m�zi�i g�sterebilirsin � t�pk� Discord gibi.
            </div>
            <a href="#" id="spotify-connect-btn" class="btn btn-primary" style="background:linear-gradient(135deg,#1ED760,#17a84a);border:none;text-decoration:none">
              <i class="fab fa-spotify"></i> Spotify Hesab�n� Ba�la
            </a>
          `}
    ...
    $('#spotify-disconnect')?.addEventListener('click', async () => {
      try {
        ...
        toast('Spotify ba�lant�s� kesildi');
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            <label class="checkbox-label">
              ...
              <span>Hesab�m�n silinece�ini ve bu i�lemin 10 g�n i�inde geri al�nabilece�ini anl�yorum</span>
            </label>
          </div>
          <button class="btn btn-danger" id="delete-account-btn" style="width:100%;justify-content:center;background:rgba(220,38,38,0.15);border:1px solid rgba(220,38,38,0.4);color:var(--accent-red2)">
            <i class="fas fa-trash-alt"></i> Hesab�m� Silmek �stiyorum
          </button>
    ...
    $('#delete-account-btn').addEventListener('click', async () => {
      ...
      try {
        ...
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-trash-alt"></i> Hesab�m� Silmek �stiyorum';
      }
    });
  }
}
...
function renderLogin(app) {
  ...
  document.title = 'Giri� Yap - Demlik';
  app.innerHTML = `<div class="auth-page">
    <div class="auth-card card card-body">
      <div class="auth-title">Giri� Yap</div>
      <p class="auth-subtitle">Hesab�n�za eri�in</p>
      <div class="form-group"><label>Kullan�c� Ad�</label><input type="text" id="login-id" placeholder="kullan�c�_ad�" autocomplete="username" /></div>
      <div class="form-group">
        <label>�ifre</label>
        <div style="position:relative">
      ...
      <button class="btn btn-primary" style="width:100%;margin-top:4px" id="login-btn">Giri� Yap</button>
      <div id="login-error" class="form-error mt-4" style="text-align:center"></div>
      <div class="auth-footer">Hesab�n yok mu? <a href="/kayit" data-link class="auth-link">Kay�t Ol</a></div>
    </div>
  </div>`;
  ...
  const doLogin = async () => {
    ...
    if (!login || !password) { $('#login-error').textContent = 'T�m alanlar� doldurun'; return; }
    try {
      const data = await api('/auth/login', { method: 'POST', body: JSON.stringify({ login, password }) });
      // Silinme talebi verilmi� hesap
        ...
        app.innerHTML = `<div class="auth-page">
          <div class="auth-card card card-body" style="border-color:rgba(220,38,38,0.4)">
            <div style="text-align:center;margin-bottom:20px">
              ...
              <div style="font-size:18px;font-weight:700;color:var(--accent-red2)">Hesab�n�z�n Silinmesi �stendi</div>
              <p style="font-size:13px;color:var(--text-secondary);margin-top:8px">
                Hesab�n�z <strong>${daysLeft} g�n</strong> i�inde kal�c� olarak silinecek.
            ...
            <button class="btn btn-primary" id="cancel-delete-btn" style="width:100%;justify-content:center;margin-bottom:10px">
              <i class="fas fa-undo"></i> Vazge�, Hesab�m� Geri Al
            </button>
            <button class="btn btn-outline" id="keep-delete-btn" style="width:100%;justify-content:center;color:var(--accent-red2);border-color:rgba(220,38,38,0.3)">
              <i class="fas fa-trash"></i> Hay�r, Silinsin
            </button>
          </div>
        </div>`;
        $('#cancel-delete-btn').addEventListener('click', async () => {
          try {
            // Ge�ici tokenla cancel-delete �a��r
            ...
            updateNavUI(); toast('Hesab�n geri al�nd�, ho� geldin ' + currentUser.username + '!');
            navigate('/');
          } catch(e) { toast(e.message, 'error'); }
        });
      ...
      localStorage.setItem('token', currentToken);
      updateNavUI(); toast('Ho� geldiniz, ' + currentUser.username + '!');
      navigate('/');
...
function renderRegister(app) {
  if (currentUser) { navigate('/'); return; }
  document.title = 'Kay�t Ol - Demlik';
  app.innerHTML = `<div class="auth-page">
    <div class="auth-card card card-body">
      <div class="auth-title">Kay�t Ol</div>
      <p class="auth-subtitle">Toplulu�a kat�l</p>
      <div class="form-group"><label>Kullan�c� Ad�</label><input type="text" id="reg-username" placeholder="..." autocomplete="username" /></div>
      <div class="form-group">
        <label style="display:flex;align-items:center;gap:8px">
          E-posta
          <span style="font-size:11px;color:var(--text-muted);font-weight:400;font-style:italic">Sallayabilirsiniz. Zaten umursam�yoruz&nbsp;: )</span>
        </label>
      ...
      <div class="form-group">
        <label class="checkbox-label">
          ...
          <span>KVKK ayd�nlatma metnini okudum ve kabul ediyorum. <button type="button" class="btn btn-ghost btn-sm" id="kvkk-btn" style="padding:0;color:var(--accent-red2);font-size:13px">Metni oku</button></span>
        </label>
      </div>
      <button class="btn btn-primary" style="width:100%;margin-top:4px" id="reg-btn">Kay�t Ol</button>
      <div id="reg-error" class="form-error mt-4" style="text-align:center"></div>
      <div class="auth-footer">Zaten hesab�n var m�? <a href="/giris" data-link class="auth-link">Giri� Yap</a></div>
    </div>
  </div>`;
  ...
  $('#kvkk-btn').addEventListener('click', async () => {
    try {
      ...
      showModal('KVKK Ayd�nlatma Metni', `<div style="white-space:pre-wrap;font-size:13px;line-height:1.7;color:var(--text-secondary);max-height:400px;overflow-y:auto">${escHtml(r.text)}</div>`);
    } catch {}
  });
  ...
  const doRegister = async () => {
    ...
    if (!username || !email || !password) { $('#reg-error').textContent = 'T�m alanlar� doldurun'; return; }
    if (!kvkk_accepted) { $('#reg-error').textContent = 'KVKK onay� zorunludur'; return; }
    try {
...
function renderNotFound(app) {
  document.title = 'Sayfa Bulunamad� - Demlik';
  app.innerHTML = `<div class="container page" style="text-align:center;padding:80px 20px">
    ...
    <div style="font-size:24px;font-weight:700;margin-bottom:12px">Sayfa Bulunamad�</div>
    <p style="color:var(--text-secondary);margin-bottom:24px">O sayfa taze bitti abim, veremmi ba�kas�n�?</p>
...
function applyTheme(theme) {
  ...
  const themes = {
    'dark':          { bg:'#080808', card:'#1a1a1a', card2:'#242424', accent:'#dc2626', accentHov:'#f05252', text:'#f0f0f0', textSec:'#b0b0b0', border:'rgba(220,38,38,0.18)' },
    'darkmorp':      { bg:'#000000', card:'#0a0a0a', card2:'#141414', accent:'#8b5cf6', accentHov:'#a78bfa', text:'#ffffff', textSec:'#a0a0a0', border:'rgba(139,92,246,0.2)', grad:'linear-gradient(135deg,#000 0%,#1a0033 25%,#000 50%,#001a33 75%,#000 100%)' },
    'neon-purple':   { bg:'#0d0d1a', card:'#1a1a2e', card2:'#22223f', accent:'#9b59b6', accentHov:'#b57bee', text:'#f0e6ff', textSec:'#b39ddb', border:'rgba(155,89,182,0.2)' },
    'ocean-blue':    { bg:'#0a0e1a', card:'#0d1b2a', card2:'#15273a', accent:'#1e90ff', accentHov:'#63b3ed', text:'#e3f2fd', textSec:'#90caf9', border:'rgba(30,144,255,0.2)' },
    'fire-red':      { bg:'#1a0a0a', card:'#2d1010', card2:'#3d1818', accent:'#ff4500', accentHov:'#ff7043', text:'#fff3e0', textSec:'#ffab91', border:'rgba(255,69,0,0.2)' },
    'forest-green':  { bg:'#0a1a0a', card:'#0d2b0d', card2:'#163816', accent:'#2ecc71', accentHov:'#55d98d', text:'#e8f5e9', textSec:'#a5d6a7', border:'rgba(46,204,113,0.2)' },
    'gold':          { bg:'#1a1500', card:'#2a2200', card2:'#363000', accent:'#f1c40f', accentHov:'#f9e03b', text:'#fffde7', textSec:'#fff176', border:'rgba(241,196,15,0.2)' },
    'midnight-blue': { bg:'#050a1a', card:'#0a1428', card2:'#121e38', accent:'#3ea6ff', accentHov:'#7ec8e3', text:'#e8f4fd', textSec:'#90caf9', border:'rgba(62,166,255,0.2)' },
    'orange-fire':   { bg:'#1a0f00', card:'#2d1a00', card2:'#3d2400', accent:'#ff6b00', accentHov:'#ff9940', text:'#fff8e1', textSec:'#ffcc80', border:'rgba(255,107,0,0.2)' },
    'pink-dream':    { bg:'#1a0a14', card:'#2d1020', card2:'#3d162c', accent:'#e91e8c', accentHov:'#f06bb0', text:'#fce4ec', textSec:'#f48fb1', border:'rgba(233,30,140,0.2)' },
    'aurora':        { bg:'#050d1a', card:'#0d1f35', card2:'#152c4a', accent:'#00e5ff', accentHov:'#67f0ff', text:'#e0f7fa', textSec:'#80deea', border:'rgba(0,229,255,0.2)', grad:'linear-gradient(160deg,#050d1a 0%,#0d2137 50%,#1a0d37 100%)' },
    'sunset-glow':   { bg:'#1a0a00', card:'#2d1500', card2:'#3d1d00', accent:'#ff6b35', accentHov:'#ff9a6c', text:'#fff3e0', textSec:'#ffcc80', border:'rgba(255,107,53,0.2)', grad:'linear-gradient(160deg,#1a0a00 0%,#2d1500 40%,#1a0a1a 100%)' },
    'deep-space':    { bg:'#020408', card:'#0d0520', card2:'#170a30', accent:'#7c3aed', accentHov:'#a78bfa', text:'#ede9fe', textSec:'#c4b5fd', border:'rgba(124,58,237,0.2)', grad:'linear-gradient(160deg,#020408 0%,#0d0520 50%,#020408 100%)' },
    'emerald-night': { bg:'#020d0a', card:'#051a12', card2:'#0a2d20', accent:'#10b981', accentHov:'#34d399', text:'#d1fae5', textSec:'#6ee7b7', border:'rgba(16,185,129,0.2)', grad:'linear-gradient(160deg,#020d0a 0%,#051a12 50%,#020d0a 100%)' },
    'rose-gold':     { bg:'#1a0a0f', card:'#2d0f1a', card2:'#3d1525', accent:'#f43f5e', accentHov:'#fb7185', text:'#ffe4e6', textSec:'#fda4af', border:'rgba(244,63,94,0.2)', grad:'linear-gradient(160deg,#1a0a0f 0%,#2d0f1a 40%,#1a0a0f 100%)' },
    'light':         { bg:'#f5f5f5', card:'#ffffff', card2:'#eeeeee', accent:'#dc2626', accentHov:'#f05252', text:'#111111', textSec:'#555555', border:'rgba(0,0,0,0.1)', light:true },
    'light-blue':    { bg:'#e3f2fd', card:'#ffffff', card2:'#bbdefb', accent:'#2196f3', accentHov:'#64b5f6', text:'#0d47a1', textSec:'#546e7a', border:'rgba(33,150,243,0.2)', light:true, grad:'linear-gradient(160deg,#e3f2fd 0%,#bbdefb 50%,#e3f2fd 100%)' },
    'light-purple':  { bg:'#f3e5f5', card:'#ffffff', card2:'#e1bee7', accent:'#9c27b0', accentHov:'#ce93d8', text:'#4a148c', textSec:'#6a1b9a', border:'rgba(156,39,176,0.2)', light:true, grad:'linear-gradient(160deg,#f3e5f5 0%,#e1bee7 50%,#f3e5f5 100%)' },
    'light-green':   { bg:'#e8f5e9', card:'#ffffff', card2:'#c8e6c9', accent:'#4caf50', accentHov:'#81c784', text:'#1b5e20', textSec:'#558b2f', border:'rgba(76,175,80,0.2)', light:true, grad:'linear-gradient(160deg,#e8f5e9 0%,#c8e6c9 50%,#e8f5e9 100%)' },
    'light-orange':  { bg:'#fff3e0', card:'#ffffff', card2:'#ffe0b2', accent:'#ff9800', accentHov:'#ffb74d', text:'#e65100', textSec:'#ef6c00', border:'rgba(255,152,0,0.2)', light:true, grad:'linear-gradient(160deg,#fff3e0 0%,#ffe0b2 50%,#fff3e0 100%)' },
    'light-pink':    { bg:'#fce4ec', card:'#ffffff', card2:'#f8bbd0', accent:'#e91e63', accentHov:'#f48fb1', text:'#880e4f', textSec:'#ad1457', border:'rgba(233,30,99,0.2)', light:true, grad:'linear-gradient(160deg,#fce4ec 0%,#f8bbd0 50%,#fce4ec 100%)' },
    'light-teal':    { bg:'#e0f2f1', card:'#ffffff', card2:'#b2dfdb', accent:'#009688', accentHov:'#4db6ac', text:'#004d40', textSec:'#00695c', border:'rgba(0,150,136,0.2)', light:true, grad:'linear-gradient(160deg,#e0f2f1 0%,#b2dfdb 50%,#e0f2f1 100%)' }
  };

  const t = themes[theme] || themes['dark'];

  root.style.setProperty('--bg-primary', t.bg);
  root.style.setProperty('--bg-secondary', t.light ? t.card2 : (t.card || '#111'));
  root.style.setProperty('--bg-card', t.card);
  root.style.setProperty('--bg-card2', t.card2);
  root.style.setProperty('--bg-tertiary', t.card);
  root.style.setProperty('--bg-active', `${t.accent}14`);
  root.style.setProperty('--bg-hover', t.light ? 'rgba(0,0,0,0.04)' : 'rgba(255,255,255,0.04)');
  root.style.setProperty('--accent-red', t.accent);
  root.style.setProperty('--accent-red2', t.accentHov);
  root.style.setProperty('--accent-red-dark', t.accent);
  root.style.setProperty('--grad-red', `linear-gradient(135deg, ${t.accent}, ${t.bg})`);
  root.style.setProperty('--text-primary', t.text);
  root.style.setProperty('--text-secondary', t.textSec);
  root.style.setProperty('--text-muted', t.light ? '#888888' : '#606060');
  root.style.setProperty('--border', t.border);
  root.style.setProperty('--border-hover', t.accent + '88');
  root.style.setProperty('--shadow', `0 4px 24px rgba(0,0,0,${t.light ? 0.15 : 0.5})`);
  root.style.setProperty('--glow', `0 0 20px ${t.accent}26`);
  ...
  if (navbar) {
    navbar.style.background = t.light
      ? 'rgba(255,255,255,0.97)'
      : `linear-gradient(180deg, ${t.bg}ff 0%, ${t.bg}f7 100%)`;
    navbar.style.borderBottomColor = t.accent + '44';
  }
}

function showThemeSelector() {
  const darkThemes = [
    {id:'dark',name:'Gece Karanlığı',prev:'linear-gradient(135deg,#080808,#1a1a1a)',acc:'#dc2626'},
    {id:'darkmorp',name:'DarkMorp',prev:'linear-gradient(135deg,#000,#1a0033)',acc:'#8b5cf6'},
    {id:'neon-purple',name:'Elektrik Moru',prev:'linear-gradient(135deg,#0d0d1a,#1a1a2e)',acc:'#9b59b6'},
    {id:'ocean-blue',name:'Okyanus Mavisi',prev:'linear-gradient(135deg,#0a0e1a,#0d1b2a)',acc:'#1e90ff'},
    {id:'fire-red',name:'Ateş Kırmızısı',prev:'linear-gradient(135deg,#1a0a0a,#2d1010)',acc:'#ff4500'},
    {id:'forest-green',name:'Yeşil Orman',prev:'linear-gradient(135deg,#0a1a0a,#0d2b0d)',acc:'#2ecc71'},
    {id:'gold',name:'Altın Sarısı',prev:'linear-gradient(135deg,#1a1500,#2a2200)',acc:'#f1c40f'},
    {id:'midnight-blue',name:'Gece Mavisi',prev:'linear-gradient(135deg,#050a1a,#0a1428)',acc:'#3ea6ff'},
    {id:'orange-fire',name:'Turuncu Ateş',prev:'linear-gradient(135deg,#1a0f00,#2d1a00)',acc:'#ff6b00'},
    {id:'pink-dream',name:'Pembe Rüya',prev:'linear-gradient(135deg,#1a0a14,#2d1020)',acc:'#e91e8c'},
    {id:'aurora',name:'Aurora',prev:'linear-gradient(135deg,#050d1a,#0d2137)',acc:'#00e5ff'},
    {id:'sunset-glow',name:'Gün Batımı',prev:'linear-gradient(135deg,#1a0a00,#2d1500)',acc:'#ff6b35'},
    {id:'deep-space',name:'Derin Uzay',prev:'linear-gradient(135deg,#020408,#0d0520)',acc:'#7c3aed'},
    {id:'emerald-night',name:'Zümrüt Gece',prev:'linear-gradient(135deg,#020d0a,#051a12)',acc:'#10b981'},
    {id:'rose-gold',name:'Gül Altını',prev:'linear-gradient(135deg,#1a0a0f,#2d0f1a)',acc:'#f43f5e'}
  ];
  const lightThemes = [
    {id:'light',name:'Gün Işığı',prev:'linear-gradient(135deg,#f5f5f5,#eeeeee)',acc:'#dc2626'},
    {id:'light-blue',name:'Açık Mavi',prev:'linear-gradient(135deg,#e3f2fd,#bbdefb)',acc:'#2196f3'},
    {id:'light-purple',name:'Açık Mor',prev:'linear-gradient(135deg,#f3e5f5,#e1bee7)',acc:'#9c27b0'},
    {id:'light-green',name:'Açık Yeşil',prev:'linear-gradient(135deg,#e8f5e9,#c8e6c9)',acc:'#4caf50'},
    {id:'light-orange',name:'Açık Turuncu',prev:'linear-gradient(135deg,#fff3e0,#ffe0b2)',acc:'#ff9800'},
    {id:'light-pink',name:'Açık Pembe',prev:'linear-gradient(135deg,#fce4ec,#f8bbd0)',acc:'#e91e63'},
    {id:'light-teal',name:'Açık Turkuaz',prev:'linear-gradient(135deg,#e0f2f1,#b2dfdb)',acc:'#009688'}
  ];

  const cur = currentUser?.theme || localStorage.getItem('demlik_theme') || 'dark';

  const themeGrid = (list) => list.map(t => `
    <div class="theme-opt${cur===t.id?' active':''}" onclick="selectTheme('${t.id}')"
      style="cursor:pointer;border-radius:10px;overflow:hidden;border:2px solid ${cur===t.id?t.acc:'transparent'};transition:all .2s"
      onmouseover="this.style.transform='translateY(-3px)';this.style.boxShadow='0 6px 16px rgba(0,0,0,0.3)'"
      onmouseout="this.style.transform='';this.style.boxShadow=''">
      <div style="background:${t.prev};padding:14px;min-height:64px;position:relative">
        <div style="height:5px;background:${t.acc};border-radius:3px;margin-bottom:5px"></div>
        <div style="display:flex;gap:3px"><div style="flex:1;height:16px;background:rgba(128,128,128,0.2);border-radius:3px"></div><div style="width:24px;height:16px;background:${t.acc};border-radius:3px;opacity:.8"></div></div>
        ${cur===t.id?`<div style="position:absolute;top:6px;right:6px;width:20px;height:20px;background:${t.acc};border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:10px;color:#fff"><i class="fas fa-check"></i></div>`:''}
      </div>
      <div style="padding:8px;background:var(--bg-card2);text-align:center;font-size:12px;font-weight:600;color:var(--text-primary)">${t.name}</div>
    </div>`).join('');

  showModal('Tema Seç', `
    <h4 style="font-size:14px;font-weight:700;margin-bottom:12px;display:flex;align-items:center;gap:6px;color:var(--text-secondary)"><i class="fas fa-moon"></i> Karanlık Temalar</h4>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:10px;margin-bottom:24px">${themeGrid(darkThemes)}</div>
    <h4 style="font-size:14px;font-weight:700;margin-bottom:12px;display:flex;align-items:center;gap:6px;color:var(--text-secondary)"><i class="fas fa-sun"></i> Aydınlık Temalar</h4>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:10px">${themeGrid(lightThemes)}</div>
    <div style="margin-top:20px;text-align:right"><button class="btn btn-ghost btn-sm" onclick="hideModal()">Kapat</button></div>
  `);
}

async function selectTheme(theme) {
  applyTheme(theme);
  localStorage.setItem('demlik_theme', theme);
  if (currentUser) {
    try {
      await api('/user/theme', { method: 'PUT', body: JSON.stringify({ theme }) });
      currentUser.theme = theme;
    } catch {}
  }
  // label güncelle (ayarlar sayfasındaysa)
  const lbl = document.getElementById('cur-theme-label');
...
async function init() {
  ...
  // Tema uygula: önce localStorage, yoksa sunucu ayarı (initAuth sonrası currentUser.theme)
  const savedTheme = currentUser?.theme || localStorage.getItem('demlik_theme') || 'dark';
...
async function showForwardForumModal(forum) {
  ...
  const listHTML = convs.length === 0
    ? `<div class="empty-state" style="padding:20px"><p>Hen�z mesajla�ma yok. Bir kullan�c�ya mesaj g�nderin.</p></div>`
...
async function renderMessages(app, targetUsername) {
  ...
  app.innerHTML = `<div class="dm-layout">
    ...
    <div class="dm-main" id="dm-main">
      ${targetUsername ? '' : `<div class="dm-empty"><i class="fas fa-comments" style="font-size:48px;color:var(--text-muted);margin-bottom:16px"></i><p style="color:var(--text-muted)">Bir konu�ma se�in</p></div>`}
    </div>
  </div>`;
...
async function renderDMChat(username) {
  ...
  // Mesajlar okundu � badge'i hemen g�ncelle
  ...
  mainEl.innerHTML = `<div class="dm-chat">
    ...
    <div class="dm-input-bar">
      <label class="btn btn-ghost btn-sm" for="dm-img-input" title="Foto�raf ekle" style="cursor:pointer;padding:6px"><i class="fas fa-image"></i></label>
  ...
  async function sendDmMessage() {
    ...
    try {
      ...
      const convItem = $(`.dm-conv-item[data-username="${CSS.escape(username)}"]`);
      if (convItem) convItem.querySelector('.dm-conv-last').textContent = content || '?? Foto�raf';
    } catch (e) { toast(e.message, 'error'); }
  ...
  // Otomatik b�y�yen textarea
  $('#dm-input')?.addEventListener('input', e => {
    e.target.style.height = 'auto';
    e.target.style.height = Math.min(e.target.scrollHeight, 120) + 'px';
  });

  // Mesaj aksiyonlar� (�� nokta, se�, yan�tla, sil)
  ...
  return `<div class="dm-msg-wrap ${isOwn ? 'dm-own' : ''}" data-id="${m.id}">
    ...
    <div class="dm-msg-content">
      ...
      <div class="dm-msg-meta">
        ...
        ${isOwn && !deleted ? `<span style="font-size:11px;margin-left:3px">${m.read_at ? '<i class="fas fa-check-double" style="color:#1ED760" title="Okundu"></i>' : '<i class="fas fa-check" style="color:var(--text-muted)" title="G�nderildi"></i>'}</span>` : ''}
...
function showDmMsgMenu(btn, msgId, isOwn, username, replyToId, setReply) {
  ...
  const items = [
    ...
    { label: '<i class="fas fa-check-square"></i> Se�', action: 'select' },
...
function showDmOptionsMenu(username, convId) {
  ...
  $('#dm-opt-hide').addEventListener('click', () => {
    ...
    showModal('Konu�may� Gizle', `
      <p style="font-size:13px;color:var(--text-secondary);margin-bottom:12px">�ifre koyarsan�z a�mak i�in �ifre gerekecek.</p>
  ...
  $('#dm-opt-setpass').addEventListener('click', () => {
    ...
    $('#dm-setpass-confirm').addEventListener('click', async () => {
      ...
      try { await api(`/conversation/${encodeURIComponent(username)}/set-password`, { method: 'POST', body: JSON.stringify({ password: pass }) }); hideModal(); toast('�ifre g�ncellendi'); } catch (e) { toast(e.message, 'error'); }
    });
  });
...
async function renderFriends(app) {
  if (!currentUser) { navigate('/giris'); return; }
  document.title = 'Arkada�lar - Demlik';
  let friends = [];
  ...
  app.innerHTML = `<div class="container page">
    <div class="page-header"><div class="page-title"><i class="fas fa-user-friends" style="color:var(--accent-red)"></i> Arkada�lar</div></div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px">
      <div>
        <div class="tabs" style="margin-bottom:16px">
          <button class="tab active" id="tab-friends" onclick="showFriendsTab('friends')">Arkada�lar (${accepted.length})</button>
          ...
          <button class="tab" id="tab-sent" onclick="showFriendsTab('sent')">G�nderilenler (${pending_out.length})</button>
        ...
        <div id="friends-content">
          <div id="tab-content-friends">
            ${accepted.length === 0 ? '<div class="empty-state"><i class="fas fa-user-friends"></i><p>Hen�z arkada��n yok</p></div>'
          ...
          <div id="tab-content-sent" style="display:none">
            ${pending_out.length === 0 ? '<div class="empty-state"><i class="fas fa-paper-plane"></i><p>G�nderilen istek yok</p></div>'
  ...
  async function doFriendSearch() {
    ...
    try {
      ...
      if (!users.length) { res.innerHTML = '<p style="color:var(--text-muted);font-size:13px;text-align:center">Sonu� bulunamad�</p>'; return; }
      ...
      $$('.send-req-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
          try { await api(`/friends/request/${encodeURIComponent(btn.dataset.username)}`, { method: 'POST' }); btn.textContent = '? G�nderildi'; btn.disabled = true; btn.classList.remove('btn-primary'); } catch (e) { toast(e.message, 'error'); }
        });
      });
  ...
  app.addEventListener('click', async e => {
    ...
    if (remove) { if (!confirm('Arkada�l�ktan ��kart?')) return; try { await api(`/friends/${remove.dataset.id}`, { method: 'DELETE' }); renderFriends(app); } catch (e) { toast(e.message,'error'); } }
...
function friendItemHTML(f, type, myId) {
  ...
  return `<div class="card card-body" style="margin-bottom:8px;display:flex;align-items:center;gap:10px">
    ...
    <div style="flex:1">
      ...
      ${type === 'incoming' ? `<div style="font-size:11px;color:var(--accent-red2)"><i class="fas fa-user-plus"></i> Arkada�l�k iste�i g�nderdi</div>` : ''}
    </div>
...
(function() {
  ...
  // Auth de�i�ince toggle butonunu g�ncelle
...
async function renderMusicList(app) {
  document.title = 'M�zikler � Demlik';
  app.innerHTML = `<div class="container page">
    <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:20px">
      <div class="page-title" style="display:flex;align-items:center;gap:10px">
        <i class="fas fa-music" style="color:var(--accent-red2)"></i> M�zikler
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
        ${currentUser && !currentUser.is_artist ? `<a href="/artist-basvuru" data-link class="btn btn-outline btn-sm"><i class="fas fa-microphone"></i> Artist Ba�vurusu</a>` : ''}
        ${currentUser?.is_artist ? `<a href="/artist-panel" data-link class="btn btn-primary btn-sm"><i class="fas fa-upload"></i> �ark� Y�kle</a>` : ''}
        ${currentUser && !currentUser.is_artist ? `<a href="/sarki-yukle" data-link class="btn btn-outline btn-sm"><i class="fas fa-share"></i> �ark� Payla�</a>` : ''}
      </div>
    </div>
    <div class="music-search-bar" style="margin-bottom:20px">
      <div class="search-bar" style="margin:0">
        <i class="fas fa-search"></i>
        <input type="text" id="music-search" placeholder="�ark� ad�, sanat��, t�r, da��t�c�, �ark� s�z� ara..." style="width:100%" />
      </div>
    </div>
  ...
  const loadSongs = async (q = '') => {
    ...
    try {
      ...
      songs = await api(url);
      if (!songs.length) { el.innerHTML = '<div class="empty-state"><i class="fas fa-music"></i><p>Hen�z �ark� yok.</p></div>'; return; }
      el.innerHTML = `<div class="music-table">
        <div class="music-table-header">
          <div style="width:40px">#</div>
          <div style="flex:1">Ba�l�k</div>
          <div style="width:160px;display:none" class="col-dist">Da��t�c�</div>
  ...
  player.innerHTML = `
    <div class="gplayer-inner">
      ...
      <div class="gplayer-controls">
        <button class="gplayer-btn" id="gp-prev" title="�nceki"><i class="fas fa-step-backward"></i></button>
...
async function renderMusicDetail(app, slug) {
  ...
  try { song = await api('/songs/' + slug); } catch {
    app.innerHTML = '<div class="container page"><div class="empty-state"><i class="fas fa-music"></i><p>�ark� bulunamad�.</p></div></div>'; return;
  }
  ...
  app.innerHTML = `<div class="container page">
    <div class="music-detail-header">
      ...
      <div class="music-detail-info">
        <div class="music-detail-type-badge">${isOwn ? '<i class="fas fa-microphone"></i> Sanat�� �ark�s�' : '<i class="fas fa-share"></i> Payla��lan �ark�'}</div>
        ...
        ${!isOwn && song.share_reason ? `
          <div class="music-share-reason">
            <div class="music-share-reason-label"><i class="fas fa-question-circle"></i> Neden payla�t�n�z?</div>
    ...
    ${hasLyrics ? `
      <div class="music-lyrics-box">
        <div class="music-lyrics-title"><i class="fas fa-align-left"></i> �ark� S�zleri</div>
  ...
  // Detail page ses kontrol� (localStorage'dan ba�lat)
  const savedVol = parseFloat(localStorage.getItem('demlik_volume') ?? '0.8');
  audio.volume = savedVol;
  ...
  // �ark� d�zenleme butonu (sadece y�kleyene g�sterilir)
  const editBtn = document.getElementById('song-edit-btn');
  if (editBtn) {
    editBtn.addEventListener('click', () => {
      showModal(`?? �ark�y� D�zenle � ${escHtml(song.title)}`, `
        <div class="form-group"><label>�ark� Ad�</label><input id="ue-title" value="${escHtml(song.title)}" /></div>
        ${song.song_type === 'own' ? `
        <div class="form-row">
          <div class="form-group"><label>Sanat�� Ad�</label><input id="ue-artist" value="${escHtml(song.artist_name)}" /></div>
          <div class="form-group"><label>Da��t�c�</label><input id="ue-dist" value="${escHtml(song.distributor||'')}" /></div>
        </div>` : `
        <div class="form-group"><label>Sanat�� Ad�</label><input id="ue-artist" value="${escHtml(song.artist_name)}" /></div>`}
        <div class="form-group"><label>M�zik T�r�</label><input id="ue-genre" value="${escHtml(song.genre||'')}" /></div>
        <div class="form-group"><label>�ark� S�zleri</label><textarea id="ue-lyrics" rows="5">${escHtml(song.lyrics||'')}</textarea></div>
        ${song.song_type === 'other' ? `<div class="form-group"><label>Payla�ma Sebebi</label><textarea id="ue-reason" rows="2">${escHtml(song.share_reason||'')}</textarea></div>` : ''}
        <div class="form-group"><label>Yeni Kapak Foto�raf� <span style="font-size:11px;color:var(--text-muted)">(bo� b�rak = de�i�mez)</span></label>
...
async function renderArtistApply(app) {
  if (!currentUser) { navigate('/giris'); return; }
  document.title = 'Artist Ba�vurusu � Demlik';
  let existing = null;
  ...
  if (isAccepted || currentUser.is_artist) {
    app.innerHTML = `<div class="container page" style="max-width:600px;margin:0 auto">
      <div style="text-align:center;padding:60px 20px">
        ...
        <p style="color:var(--text-secondary);margin-bottom:24px">�ark� y�klemek i�in artist paneline gidin.</p>
  ...
  app.innerHTML = `<div class="container page" style="max-width:600px;margin:0 auto">
    <div class="page-title"><i class="fas fa-microphone" style="color:var(--accent-red2);margin-right:8px"></i>Artist Rozeti Ba�vurusu</div>
    ${isPending ? `
      <div class="card" style="margin-bottom:20px">
        <div class="card-body" style="text-align:center;padding:40px">
          <div style="font-size:40px;margin-bottom:12px">?</div>
          <div style="font-size:18px;font-weight:700">Ba�vurunuz Bekliyor</div>
          <p style="color:var(--text-secondary);margin-top:8px">Ekibimiz ba�vurunuzu inceliyor. Onaylan�nca bildirim alacaks�n�z.</p>
        </div>
      </div>` : ''}
    ${isRejected ? `
      <div class="card" style="margin-bottom:20px;border-color:rgba(220,38,38,0.4)">
        <div class="card-body" style="padding:16px">
          <div style="color:var(--accent-red2);font-weight:600"><i class="fas fa-times-circle"></i> Ba�vurunuz Reddedildi</div>
          <p style="font-size:13px;color:var(--text-secondary);margin-top:6px">Yeniden ba�vurabilirsiniz.</p>
        </div>
      </div>` : ''}
    ${!isPending ? `
    <div class="card">
      <div class="card-body">
        <p style="font-size:14px;color:var(--text-secondary);margin-bottom:20px">
          Artist rozeti alarak kendi �ark�lar�n�z� Demlik'te yay�nlayabilirsiniz.
        </p>
        <div class="form-group"><label>M�zik T�r�n�z *</label>
          <input id="apply-genre" placeholder="Pop, Rock, Hip-Hop, Elektronik..." />
        </div>
        <div class="form-group"><label>�rnek �ark� URL (SoundCloud, YouTube vb.)</label>
          <input id="apply-url" placeholder="https://soundcloud.com/..." />
        </div>
        <div class="form-group"><label>veya �rnek �ark� Dosyas� Y�kle</label>
        ...
        <div class="form-group"><label>Notunuz (iste�e ba�l�)</label>
        ...
        <button class="btn btn-primary" id="apply-submit" style="width:100%;justify-content:center">
          <i class="fas fa-paper-plane"></i> Ba�vuruyu G�nder
        </button>
  ...
  document.getElementById('apply-submit')?.addEventListener('click', async () => {
    ...
    if (!genre) { msg.style.color='var(--accent-red2)'; msg.textContent='M�zik t�r� zorunlu'; return; }
    ...
    try {
      ...
      app.innerHTML = `<div class="container page" style="max-width:600px;margin:0 auto;text-align:center;padding:60px 20px">
        ...
        <div style="font-size:22px;font-weight:700;margin-top:12px">Ba�vurunuz Al�nd�!</div>
    ...
    } catch(e) { msg.style.color='var(--accent-red2)'; msg.textContent=e.message; btn.disabled=false; btn.innerHTML='<i class="fas fa-paper-plane"></i> Ba�vuruyu G�nder'; }
  });
}
...
async function renderArtistPanel(app) {
        ...
        <div class="form-group">
          ...
          <div style="display:flex;gap:10px">
            <label class="checkbox-label" style="flex:1;padding:12px;background:var(--bg-card2);border:1px solid var(--border);border-radius:8px;cursor:pointer">
              <input type="radio" name="song-type" id="st-own" value="own" checked style="width:auto" /> Kendi �ark�m
            </label>
            <label class="checkbox-label" style="flex:1;padding:12px;background:var(--bg-card2);border:1px solid var(--border);border-radius:8px;cursor:pointer">
              <input type="radio" name="song-type" id="st-other" value="other" style="width:auto" /> Ba�kas�n�n �ark�s�
            </label>
          </div>
        </div>
        <div id="own-fields">
          <div class="form-group"><label>Yay�mlay�c� / Da��t�c� �smi</label><input id="s-distributor" placeholder="Kendi ad�n ya da �irket ad�" /></div>
          <div class="form-group"><label>�ark� Ad� *</label><input id="s-title" /></div>
          <div class="form-group"><label>�ark� T�r�</label><input id="s-genre" placeholder="Pop, Rock, Elektronik..." /></div>
          <div class="form-group"><label>�ark� Dosyas� * (MP3/WAV)</label><input type="file" id="s-audio" accept="audio/*" style="background:var(--bg-card2);border:1px dashed var(--border);padding:10px;cursor:pointer" /></div>
          <div class="form-group"><label>Kapak Foto�raf�</label><input type="file" id="s-cover" accept="image/*" style="background:var(--bg-card2);border:1px dashed var(--border);padding:10px;cursor:pointer" /></div>
          <div class="form-group"><label>�ark� S�zleri (iste�e ba�l�)</label><textarea id="s-lyrics" rows="6" placeholder="�ark� s�zlerini buraya yap��t�r�n..."></textarea></div>
          ...
          <label class="checkbox-label" style="margin-bottom:16px"><input type="checkbox" id="s-rules-own" style="width:auto" /> �ark� yay�nlama kurallar�n� okudum ve kabul ediyorum</label>
        </div>
        <div id="other-fields" style="display:none">
          <div class="form-group"><label>�ark� Ad� *</label><input id="s-title-o" /></div>
          <div class="form-group"><label>�ark� Sahibi (Sanat��) *</label><input id="s-artist-o" /></div>
          <div class="form-group"><label>Kapak Foto�raf�</label><input type="file" id="s-cover-o" accept="image/*" style="background:var(--bg-card2);border:1px dashed var(--border);padding:10px;cursor:pointer" /></div>
          <div class="form-group"><label>�ark� S�zleri (iste�e ba�l�)</label><textarea id="s-lyrics-o" rows="6" placeholder="�ark� s�zlerini buraya yap��t�r�n..."></textarea></div>
          <div class="form-group"><label>�ark� Dosyas� * (MP3/WAV)</label><input type="file" id="s-audio-o" accept="audio/*" style="background:var(--bg-card2);border:1px dashed var(--border);padding:10px;cursor:pointer" /></div>
          <div class="form-group"><label>Neden payla�t�n�z? *</label><textarea id="s-reason" rows="3" placeholder="Bu �ark�y� neden toplulu�umuzla payla�mak istediniz?"></textarea></div>
          ...
          <label class="checkbox-label" style="margin-bottom:16px"><input type="checkbox" id="s-rules-other" style="width:auto" /> Ba�kas�n�n �ark�s�n� payla�ma kurallar�n� okudum ve kabul ediyorum</label>
        </div>
        <button class="btn btn-primary" id="song-upload-btn" style="width:100%;justify-content:center"><i class="fas fa-upload"></i> �ark�y� Yay�nla</button>
  ...
  document.getElementById('song-upload-btn').addEventListener('click', async () => {
    ...
    if (isOwn) {
      ...
      if (!title) { msg.style.color='var(--accent-red2)'; msg.textContent='�ark� ad� gerekli'; return; }
      ...
      if (!title||!artist) { msg.style.color='var(--accent-red2)'; msg.textContent='Ba�l�k ve sanat�� ad� gerekli'; return; }
    ...
    btn.disabled=true; btn.innerHTML='<div class="spinner" style="width:14px;height:14px"></div> Y�kleniyor...';
    try {
      const data = await apiForm('/songs', fd);
      navigate('/muzik/' + data.slug);
    } catch(e) { msg.style.color='var(--accent-red2)'; msg.textContent=e.message; btn.disabled=false; btn.innerHTML='<i class="fas fa-upload"></i> �ark�y� Yay�nla'; }
  });
}
...
async function renderShareSong(app) {
  ...
  document.title = '�ark� Payla� � Demlik';

  let rules = { other_rules: '' };
  try { rules = await api('/music-rules'); } catch {}

  app.innerHTML = `<div class="container page" style="max-width:680px;margin:0 auto">
    <div class="page-title"><i class="fas fa-share-alt" style="color:var(--accent-red2);margin-right:8px"></i>�ark� Payla�</div>
    <div class="card" style="margin-bottom:16px">
      <div class="card-body" style="font-size:13px;color:var(--text-secondary);display:flex;align-items:flex-start;gap:10px">
        ...
        <div>
          Bu sayfa <strong>ba�kas�na ait �ark�lar�</strong> toplulu�a payla�mak i�indir.
          Kendi �ark�n� y�klemek istiyorsan �nce
          <a href="/artist-basvuru" data-link style="color:var(--accent-red2)">artist ba�vurusu</a> yapman gerekir.
        </div>
      </div>
    </div>
    <div class="card">
      <div class="card-body">
        <div class="form-group">
          <label>�ark� Ad� *</label>
          <input id="ss-title" placeholder="�ark�n�n ad�" />
        </div>
        <div class="form-group">
          <label>Sanat�� (�ark� Sahibi) *</label>
          <input id="ss-artist" placeholder="Sanat��n�n ad�" />
        </div>
        <div class="form-group">
          <label>M�zik T�r�</label>
          <input id="ss-genre" placeholder="Pop, Rock, Hip-Hop..." />
        </div>
        <div class="form-group">
          <label>�ark� Dosyas� * (MP3/WAV, max 50MB)</label>
        ...
        <div class="form-group">
          <label>Kapak Foto�raf� (iste�e ba�l�)</label>
        ...
        <div class="form-group">
          <label>�ark� S�zleri (iste�e ba�l�)</label>
          <textarea id="ss-lyrics" rows="5" placeholder="�ark� s�zlerini buraya yap��t�r�n..."></textarea>
        </div>
        <div class="form-group">
          <label>Neden payla��yorsunuz? *</label>
          <textarea id="ss-reason" rows="2" placeholder="Bu �ark�y� neden toplulu�umuzla payla�mak istediniz?"></textarea>
        </div>
        ${rules.other_rules ? `
        ...
        <label class="checkbox-label" style="margin-bottom:16px">
          ...
          <span>Ba�kas�n�n �ark�s�n� payla�ma kurallar�n� okudum ve kabul ediyorum</span>
        </label>
  ...
  document.getElementById('ss-submit').addEventListener('click', async () => {
    ...
    if (!title)    { msg.style.color='var(--accent-red2)'; msg.textContent='�ark� ad� zorunlu'; return; }
    if (!artist)   { msg.style.color='var(--accent-red2)'; msg.textContent='Sanat�� ad� zorunlu'; return; }
    if (!audio)    { msg.style.color='var(--accent-red2)'; msg.textContent='Ses dosyas� zorunlu'; return; }
    if (!reason)   { msg.style.color='var(--accent-red2)'; msg.textContent='Payla�ma sebebi zorunlu'; return; }
    if (!rules_ok) { msg.style.color='var(--accent-red2)'; msg.textContent='Kurallar� kabul etmelisiniz'; return; }
    ...
    btn.disabled = true;
    btn.innerHTML = '<div class="spinner" style="width:14px;height:14px"></div> Y�kleniyor...';
    try {
      const data = await apiForm('/songs', fd);
      navigate('/muzik/' + data.slug);
    } catch(e) {
      ...
      msg.textContent = e.message;
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-share"></i> Payla�';
    }
  });
}
...
async function renderVideoFeed(app, initialSlug = null) {
  document.title = 'Videolar � Demlik';
  updatePageMeta('Videolar � Demlik', 'K�sa videolar ke�fet ve payla�', '');

  videoFeedState = { videos: [], currentIdx: 0, loading: false, hasMore: true, activeSlug: initialSlug, observer: null, uploadQueue: [] };

  app.innerHTML = `
    <div class="video-feed-wrap" id="video-feed-wrap">
      <div class="video-sidebar-left">
        <a href="/" data-link class="video-side-btn" title="Ana Sayfa"><i class="fas fa-home"></i></a>
        <a href="/forum" data-link class="video-side-btn" title="Forum"><i class="fas fa-comments"></i></a>
        ${currentUser ? `<button class="video-side-btn" id="vf-upload-btn" title="Video Y�kle"><i class="fas fa-plus-circle"></i></button>` : ''}
      </div>

      <div class="video-scroll-container" id="video-scroll-container">
        <div class="video-feed-list" id="video-feed-list">
          <div class="video-loading-wrap" id="vf-initial-load">
      ...
      ${currentUser ? `
      <div id="vf-upload-progress-bar-wrap" style="display:none;position:fixed;bottom:0;left:0;right:0;height:3px;z-index:9999;background:rgba(255,255,255,0.1)">
        <div id="vf-upload-progress-bar" style="height:100%;background:var(--grad-red);width:0%;transition:width 0.3s"></div>
      </div>
      <div id="vf-upload-toast" style="display:none;position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:#1a1a1a;border:1px solid var(--border);border-radius:12px;padding:10px 20px;font-size:13px;color:var(--text-primary);z-index:9999;display:flex;align-items:center;gap:10px;min-width:200px;max-width:90vw">
        <div class="spinner" style="width:14px;height:14px;border-width:2px;flex-shrink:0"></div>
        <span id="vf-upload-toast-text">Y�kleniyor...</span>
      </div>` : ''}
    </div>`;

  // Upload butonu
  $('#vf-upload-btn')?.addEventListener('click', showVideoUploadModal);
  ...
  sentinel.id = 'vf-sentinel';
  sentinel.style.height = '1px';
  $('#video-feed-list')?.appendChild(sentinel);

  videoFeedState.observer = new IntersectionObserver(entries => {
    if (entries[0].isIntersecting && !videoFeedState.loading && videoFeedState.hasMore) {
      loadMoreVideos();
    }
  }, { threshold: 0.1 });
  videoFeedState.observer.observe(sentinel);

  // Ba�lang�� slug'� varsa o videoya kayd�r
  if (initialSlug) {
    setTimeout(() => {
      const el = document.querySelector(`[data-video-slug="${initialSlug}"]`);
...
async function loadMoreVideos() {
  if (videoFeedState.loading || !videoFeedState.hasMore) return;
  videoFeedState.loading = true;

  const list = $('#video-feed-list');
  if (!list) return;

  const lastId = videoFeedState.videos.length > 0
    ? videoFeedState.videos[videoFeedState.videos.length - 1].id
    : null;

  try {
    const url = '/videos' + (lastId ? `?before_id=${lastId}` : '');
    const newVideos = await api(url);

    // Initial loading spinner'� kald�r
    $('#vf-initial-load')?.remove();

    if (newVideos.length === 0) {
      videoFeedState.hasMore = false;
      if (videoFeedState.videos.length === 0) {
        const sentinel = $('#vf-sentinel');
        if (sentinel) list.insertBefore(createEmptyVideoState(), sentinel);
      }
      return;
    }

    videoFeedState.videos.push(...newVideos);
    const sentinel = $('#vf-sentinel');
    ...
    // Video autoplay intersect observer
    setupVideoAutoplay();

  } catch (e) {
    console.error('Video y�kleme hatas�:', e);
    $('#vf-initial-load')?.remove();
  } finally {
    videoFeedState.loading = false;
  }
}

function createEmptyVideoState() {
  const d = document.createElement('div');
  d.style.cssText = 'padding:80px 20px;text-align:center;color:var(--text-muted)';
  d.innerHTML = `
    <i class="fas fa-video" style="font-size:48px;margin-bottom:16px;display:block;opacity:0.4"></i>
    <p style="font-size:15px">Hen�z video yok.</p>
    ${currentUser ? `<button class="btn btn-primary" onclick="showVideoUploadModal()" style="margin-top:16px"><i class="fas fa-plus"></i> �lk videoyu y�kle</button>` : ''}`;
  return d;
}
...
function createVideoCard(v) {
  ...
  wrap.dataset.videoSlug = v.slug;
  wrap.dataset.videoId = v.id;

  const isOwn = currentUser && currentUser.id == v.user_id;
  const liked = v.liked_by_me;

  wrap.innerHTML = `
    <div class="video-player-wrap">
      <video class="video-player" preload="none" playsinline webkit-playsinline loop
        data-src="${escHtml(v.video_url)}"
        ${v.thumbnail_url ? `poster="${escHtml(v.thumbnail_url)}"` : ''}>
      </video>
      <div class="video-play-overlay" data-slug="${escHtml(v.slug)}">
        <i class="fas fa-play video-play-icon"></i>
      </div>
      <div class="video-top-bar">
        <button class="video-back-btn" onclick="history.back()"><i class="fas fa-arrow-left"></i></button>
        ${isOwn ? `<button class="video-del-btn" data-slug="${escHtml(v.slug)}" title="Sil"><i class="fas fa-trash"></i></button>` : ''}
      </div>
      <div class="video-mute-btn" data-slug="${escHtml(v.slug)}" title="Sesi A�/Kapat">
        <i class="fas fa-volume-mute"></i>
      </div>
    </div>
    <div class="video-right-actions">
      <div class="video-action">
        <button class="video-like-btn ${liked ? 'liked' : ''}" data-slug="${escHtml(v.slug)}" data-liked="${liked ? 1 : 0}">
          <i class="fas fa-heart"></i>
        </button>
        <span class="video-action-count" id="vlc-${v.id}">${v.like_count || 0}</span>
      </div>
      <div class="video-action">
        <button class="video-comment-btn" data-slug="${escHtml(v.slug)}">
          <i class="fas fa-comment"></i>
        </button>
        <span class="video-action-count">${v.comment_count || 0}</span>
      </div>
      <div class="video-action">
        <button class="video-share-btn" data-slug="${escHtml(v.slug)}" data-title="${escHtml(v.title || '')}">
          <i class="fas fa-share-alt"></i>
        </button>
        <span class="video-action-count">${v.share_count || 0}</span>
      </div>
    </div>
    <div class="video-info-bar">
      <div class="video-info-user">
        ${v.avatar ? `<img src="${escHtml(v.avatar)}" class="video-avatar" onclick="navigate('/profil/${escHtml(v.username)}')" />` : `<div class="video-avatar avatar-placeholder" onclick="navigate('/profil/${escHtml(v.username)}')"><i class="fas fa-user"></i></div>`}
        <span class="video-username" onclick="navigate('/profil/${escHtml(v.username)}')">${escHtml(v.username)}</span>
      </div>
      ${v.title ? `<div class="video-title-text">${escHtml(v.title)}</div>` : ''}
      ${v.description ? `<div class="video-description-text">${escHtml(v.description.substring(0, 100))}${v.description.length > 100 ? '...' : ''}</div>` : ''}
      <div class="video-views-text"><i class="fas fa-eye"></i> ${v.view_count || 0}</div>
    </div>`;

  // Video oynat/durdur
  const videoEl = wrap.querySelector('.video-player');
  const overlay = wrap.querySelector('.video-play-overlay');
  const muteBtn = wrap.querySelector('.video-mute-btn');

  overlay.addEventListener('click', () => toggleVideoPlay(videoEl, overlay));
  muteBtn.addEventListener('click', () => {
    videoEl.muted = !videoEl.muted;
    muteBtn.querySelector('i').className = videoEl.muted ? 'fas fa-volume-mute' : 'fas fa-volume-up';
  });

  // Be�en
  wrap.querySelector('.video-like-btn').addEventListener('click', async (e) => {
    if (!currentUser) { navigate('/giris'); return; }
    const btn = e.currentTarget;
    const slug = btn.dataset.slug;
    const liked = btn.dataset.liked === '1';
    try {
      const r = await api(`/videos/${slug}/like`, { method: 'POST' });
      btn.dataset.liked = r.liked ? '1' : '0';
      btn.classList.toggle('liked', r.liked);
      const cnt = document.getElementById(`vlc-${v.id}`);
      if (cnt) cnt.textContent = r.like_count;
    } catch(err) { toast(err.message, 'error'); }
  });

  // Yorum
  wrap.querySelector('.video-comment-btn').addEventListener('click', () => {
    showVideoCommentPanel(v.slug, v.id);
  });

  // Payla�
  wrap.querySelector('.video-share-btn').addEventListener('click', () => {
    showVideoShareModal(v.slug, v.title);
  });

  // Sil
  wrap.querySelector('.video-del-btn')?.addEventListener('click', async () => {
    if (!confirm('Bu videoyu silmek istiyor musunuz?')) return;
    try {
      await api(`/videos/${v.slug}`, { method: 'DELETE' });
      wrap.remove();
      toast('Video silindi');
    } catch(err) { toast(err.message, 'error'); }
  });
...
function toggleVideoPlay(videoEl, overlay) {
  if (videoEl.paused) {
    // Lazy load src
    ...
    overlay.querySelector('.video-play-icon').style.display = 'none';
  } else {
    videoEl.pause();
    overlay.style.opacity = '1';
    overlay.querySelector('.video-play-icon').style.display = 'block';
  }
}

function setupVideoAutoplay() {
  // Intersection observer � ekrana giren video otomatik �al���r, ��kan durur
  if (window._videoAutoplayObserver) window._videoAutoplayObserver.disconnect();

  window._videoAutoplayObserver = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      ...
      if (entry.isIntersecting && entry.intersectionRatio > 0.6) {
        ...
        video.muted = true; // autoplay i�in ses kapal� ba�la
        video.play().catch(() => {});
        if (overlay) { overlay.style.opacity = '0'; overlay.querySelector('.video-play-icon').style.display = 'none'; }
        const muteBtn = card.querySelector('.video-mute-btn i');
        if (muteBtn) muteBtn.className = 'fas fa-volume-mute';
      } else {
        video.pause();
        if (overlay) { overlay.style.opacity = '1'; overlay.querySelector('.video-play-icon').style.display = 'block'; }
      }
    });
  }, { threshold: [0.6] });

  document.querySelectorAll('.video-card').forEach(card => {
    window._videoAutoplayObserver.observe(card);
  });
}

async function showVideoCommentPanel(slug, videoId) {
  let comments = [];
  try { comments = await api(`/videos/${slug}/comments`); } catch {}

  showModal(`Yorumlar`, `
    <div id="vcmt-list" style="max-height:340px;overflow-y:auto;margin-bottom:12px;display:flex;flex-direction:column;gap:10px">
      ${comments.length === 0 ? `<div style="text-align:center;color:var(--text-muted);padding:20px;font-size:13px">Hen�z yorum yok.</div>` :
        comments.map(c => `
          <div class="comment" id="vcmt-${c.id}">
            ${c.avatar ? `<img src="${escHtml(c.avatar)}" class="comment-avatar" />` : `<div class="comment-avatar avatar-placeholder"><i class="fas fa-user"></i></div>`}
            <div class="comment-body">
              <div class="comment-header">
              ...
              <div style="display:flex;align-items:center;gap:8px;margin-top:6px">
                <button class="like-vcmt-btn forum-action-btn" data-id="${c.id}" data-slug="${escHtml(slug)}" style="font-size:12px;padding:3px 10px">
                  <i class="fas fa-heart"></i> <span class="vcmt-lc">${c.like_count || 0}</span>
                </button>
                ${currentUser && currentUser.id == c.user_id ? `<button class="del-vcmt-btn btn btn-ghost btn-sm" data-id="${c.id}" data-slug="${escHtml(slug)}" style="color:var(--accent-red2);padding:2px 6px"><i class="fas fa-trash"></i></button>` : ''}
              </div>
            </div>
          </div>`).join('')}
    </div>
    ...
    <div style="display:flex;gap:8px;align-items:flex-end">
      <textarea id="vcmt-input" rows="2" placeholder="Yorumunuzu yaz�n..." style="flex:1;resize:none"></textarea>
      <button class="btn btn-primary btn-sm" id="vcmt-submit"><i class="fas fa-paper-plane"></i></button>
    </div>` : `<p style="font-size:13px;color:var(--text-muted)">Yorum yapmak i�in <a href="/giris" data-link>giri� yap�n</a>.</p>`}
    <div id="vcmt-error" style="color:var(--accent-red2);font-size:12px;margin-top:6px"></div>
  `);

  $('#vcmt-submit')?.addEventListener('click', async () => {
    const content = $('#vcmt-input').value.trim();
    if (!content) return;
    try {
      const c = await api(`/videos/${slug}/comments`, { method: 'POST', body: JSON.stringify({ content }) });
      const list = $('#vcmt-list');
      if (list) {
        ...
        list.insertAdjacentHTML('afterbegin', `
          <div class="comment" id="vcmt-${c.id}">
            ${c.avatar ? `<img src="${escHtml(c.avatar)}" class="comment-avatar" />` : `<div class="comment-avatar avatar-placeholder"><i class="fas fa-user"></i></div>`}
            <div class="comment-body">
              <div class="comment-header"><span class="comment-author">${escHtml(c.username)}</span><span class="comment-time">az �nce</span></div>
              <div class="comment-content">${escHtml(c.content)}</div>
              <div style="display:flex;gap:8px;margin-top:6px">
                <button class="like-vcmt-btn forum-action-btn" data-id="${c.id}" data-slug="${escHtml(slug)}" style="font-size:12px;padding:3px 10px"><i class="fas fa-heart"></i> <span class="vcmt-lc">0</span></button>
                <button class="del-vcmt-btn btn btn-ghost btn-sm" data-id="${c.id}" data-slug="${escHtml(slug)}" style="color:var(--accent-red2);padding:2px 6px"><i class="fas fa-trash"></i></button>
              </div>
            </div>
          </div>`);
      }
      $('#vcmt-input').value = '';
    } catch(err) { $('#vcmt-error').textContent = err.message; }
  });

  // Yorum be�en/sil delegasyonu
  $('#modal-body').addEventListener('click', async e => {
    const likeBtn = e.target.closest('.like-vcmt-btn');
    const delBtn = e.target.closest('.del-vcmt-btn');
    if (likeBtn) {
      if (!currentUser) return;
      const id = likeBtn.dataset.id, sl = likeBtn.dataset.slug;
      try {
        const r = await api(`/videos/${sl}/comments/${id}/like`, { method: 'POST' });
        const cnt = likeBtn.querySelector('.vcmt-lc');
        cnt.textContent = parseInt(cnt.textContent) + (r.liked ? 1 : -1);
        likeBtn.classList.toggle('liked', r.liked);
      } catch {}
    }
    if (delBtn) {
      if (!confirm('Yorum silinsin mi?')) return;
      const id = delBtn.dataset.id, sl = delBtn.dataset.slug;
      try {
        await api(`/videos/${sl}/comments/${id}`, { method: 'DELETE' });
...
async function showVideoShareModal(slug, title) {
  const shareUrl = `${location.origin}/video/${slug}`;
  let convs = [];
  if (currentUser) {
    try { convs = await api('/conversations'); } catch {}
  }
  ...
  showModal('Payla� / �let', `
    <div style="margin-bottom:16px">
      <div style="font-size:13px;color:var(--text-muted);margin-bottom:8px">Ba�lant�y� kopyala</div>
      <div style="display:flex;gap:8px">
        <input id="vs-link" type="text" value="${escHtml(shareUrl)}" readonly style="flex:1;font-size:12px" />
        <button class="btn btn-outline btn-sm" id="vs-copy"><i class="fas fa-copy"></i></button>
      </div>
    </div>
    ${currentUser ? `
    <div style="font-size:13px;color:var(--text-muted);margin-bottom:8px">Birini se� veya ad� yaz</div>
    <div id="vs-conv-list" style="max-height:200px;overflow-y:auto;border:1px solid var(--border);border-radius:8px;margin-bottom:8px">
      ${convs.length === 0 ? `<div style="padding:12px;text-align:center;color:var(--text-muted);font-size:12px">Hen�z konu�ma yok</div>` :
        convs.map(c => `<div class="vs-conv-row" data-username="${escHtml(c.other_username)}" style="display:flex;align-items:center;gap:10px;padding:8px 12px;cursor:pointer;border-bottom:1px solid var(--border)" onmouseover="this.style.background='var(--bg-hover)'" onmouseout="this.style.background=''">
          ${c.other_avatar ? `<img src="${escHtml(c.other_avatar)}" class="avatar-sm" />` : `<div class="avatar-sm avatar-placeholder"><i class="fas fa-user"></i></div>`}
          <span style="font-size:13px">${escHtml(c.other_username)}</span>
        </div>`).join('')}
    </div>
    <div style="display:flex;gap:8px">
      <input id="vs-username" type="text" placeholder="veya kullan�c� ad� gir..." style="flex:1" />
      <button class="btn btn-primary btn-sm" id="vs-send"><i class="fas fa-paper-plane"></i> �let</button>
    </div>
    <div id="vs-msg" style="font-size:12px;margin-top:6px;color:var(--accent-red2)"></div>` : ''}
  `);

  $('#vs-copy')?.addEventListener('click', () => {
    if (navigator.clipboard) navigator.clipboard.writeText(shareUrl);
    else { const inp = $('#vs-link'); inp.select(); document.execCommand('copy'); }
    toast('Ba�lant� kopyaland�!');
  });

  $$('.vs-conv-row').forEach(el => {
    el.addEventListener('click', () => { const u = $('#vs-username'); if (u) u.value = el.dataset.username; });
  });

  $('#vs-send')?.addEventListener('click', async () => {
    const username = $('#vs-username').value.trim();
    if (!username) { $('#vs-msg').textContent = 'Kullan�c� ad� girin'; return; }
    try {
      await api(`/videos/${slug}/share`, { method: 'POST', body: JSON.stringify({ username }) });
      hideModal();
      toast('Video iletildi!');
      navigate('/mesajlar/' + username);
    } catch(err) { $('#vs-msg').textContent = err.message; }
  });
}

function showVideoUploadModal() {
  if (!currentUser) { navigate('/giris'); return; }
  showModal('Video Y�kle', `
    <div style="font-size:13px;color:var(--text-muted);margin-bottom:16px">
      <i class="fas fa-info-circle" style="color:var(--accent-red2)"></i>
      Video y�klerken sayfada kalabilirsiniz � y�kleme arka planda devam eder.
    </div>
    <div class="form-group">
      <label>Video Dosyas� * (MP4, MOV, WebM � max 200MB)</label>
      <input type="file" id="vu-file" accept="video/*" style="background:var(--bg-card2);border:2px dashed var(--border);padding:20px;cursor:pointer;border-radius:8px;text-align:center" />
    </div>
    <div class="form-group">
      <label>Ba�l�k (opsiyonel)</label>
      <input type="text" id="vu-title" placeholder="Videonu anlat..." maxlength="100" />
    </div>
    <div class="form-group">
      <label>A��klama (opsiyonel)</label>
      <textarea id="vu-desc" rows="3" placeholder="Ek bilgi ekle..." maxlength="500"></textarea>
    </div>
    <div class="form-group">
      <label>K���k Resim (opsiyonel � yoksa video'dan al�n�r)</label>
      <input type="file" id="vu-thumb" accept="image/*" style="background:var(--bg-card2);border:1px dashed var(--border);padding:8px;cursor:pointer;border-radius:8px" />
    </div>
    <div id="vu-preview" style="display:none;margin-bottom:12px">
      <video id="vu-video-preview" style="width:100%;max-height:180px;border-radius:8px;object-fit:contain;background:#000" muted></video>
    </div>
    <button class="btn btn-primary" id="vu-submit" style="width:100%;justify-content:center">
      <i class="fas fa-upload"></i> Y�kle
    </button>
    <div id="vu-error" style="color:var(--accent-red2);font-size:12px;margin-top:8px;text-align:center"></div>
  `);

  // Video �nizleme
  $('#vu-file')?.addEventListener('change', e => {
    const file = e.target.files[0];
    if (!file) return;
    const preview = $('#vu-preview');
    const vid = $('#vu-video-preview');
    if (preview && vid) {
      vid.src = URL.createObjectURL(file);
      preview.style.display = 'block';
    }
    // Dosya ad�ndan ba�l�k �ner
    const titleInput = $('#vu-title');
    if (titleInput && !titleInput.value) {
      titleInput.value = file.name.replace(/\.[^.]+$/, '').replace(/[_-]/g, ' ').substring(0, 60);
    }
  });

  $('#vu-submit')?.addEventListener('click', () => {
    const file = $('#vu-file')?.files[0];
    if (!file) { $('#vu-error').textContent = 'Video dosyas� se�in'; return; }
    if (file.size > 200 * 1024 * 1024) { $('#vu-error').textContent = 'Video 200MB\'dan b�y�k olamaz'; return; }

    const title = $('#vu-title').value.trim();
    const desc = $('#vu-desc').value.trim();
    const thumbFile = $('#vu-thumb')?.files[0];

    hideModal();
    startBackgroundVideoUpload(file, title, desc, thumbFile);
  });
}

function startBackgroundVideoUpload(videoFile, title, description, thumbFile) {
  // Progress bar g�ster
  const progressWrap = $('#vf-upload-progress-bar-wrap');
  const progressBar = $('#vf-upload-progress-bar');
  const uploadToast = $('#vf-upload-toast');
  const toastText = $('#vf-upload-toast-text');

  if (progressWrap) progressWrap.style.display = 'block';
  if (uploadToast) uploadToast.style.display = 'flex';
  if (toastText) toastText.textContent = 'Haz�rlan�yor...';
  ...
  xhr.upload.addEventListener('progress', e => {
    ...
    if (toastText) toastText.textContent = `Y�kleniyor: ${mb}/${total} MB (${pct}%)`;
  });
  ...
  xhr.addEventListener('load', () => {
    ...
    try {
      const data = JSON.parse(xhr.responseText);
      if (xhr.status >= 400) {
        toast('Y�kleme hatas�: ' + (data.error || 'Bilinmeyen hata'), 'error');
      } else {
        if (toastText) toastText.textContent = 'Video y�klendi! ?';
        setTimeout(() => {
          if (progressWrap) progressWrap.style.display = 'none';
          if (uploadToast) uploadToast.style.display = 'none';
          if (progressBar) progressBar.style.width = '0%';
        }, 2000);
        // Feed'deyse yeni videoyu ekle
        if (location.pathname === '/videolar' || location.pathname.startsWith('/video/')) {
          toast('Video y�klendi, feed yenileniyor...');
          setTimeout(() => {
            navigate('/videolar');
          }, 1500);
        } else {
          toast('Video y�klendi! <a href="/videolar" data-link>Videolara git</a>');
        }
      }
    } catch {
      toast('Sunucu yan�t� okunamad�', 'error');
      if (progressWrap) progressWrap.style.display = 'none';
      if (uploadToast) uploadToast.style.display = 'none';
    }
  });
  ...
  xhr.addEventListener('error', () => {
    toast('A� hatas�, y�kleme ba�ar�s�z', 'error');
    if (progressWrap) progressWrap.style.display = 'none';
    if (uploadToast) uploadToast.style.display = 'none';
    if (progressBar) progressBar.style.width = '0%';
  });
  ...
  // Toast'a iptal butonu
  if (uploadToast && !uploadToast.querySelector('.vu-cancel-btn')) {
    const cancelBtn = document.createElement('button');
    cancelBtn.className = 'vu-cancel-btn btn btn-ghost btn-sm';
    cancelBtn.innerHTML = '<i class="fas fa-times"></i>';
    cancelBtn.style.cssText = 'padding:2px 6px;font-size:11px;flex-shrink:0';
    cancelBtn.title = '�ptal';
    cancelBtn.onclick = () => { xhr.abort(); toast('Y�kleme iptal edildi'); };
    uploadToast.appendChild(cancelBtn);
  }
}

// window.showVideoUploadModal global olarak eri�ilebilsin
window.showVideoUploadModal = showVideoUploadModal;
